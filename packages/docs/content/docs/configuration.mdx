...
LLM `models` entries can remain bare IDs or declare `vision` and `thinking` capabilities for models outside the static catalog. A model declaration must use supported thinking values; invalid declarations are ignored with a configuration warning. When `<PREFIX>_MODELS` is set, declarations for IDs absent from that allowlist are ignored with a configuration warning.
