...
LLM `models` entries can remain bare IDs or declare `vision` and `thinking` capabilities for models outside the static catalog. A declaration must use supported thinking values; an invalid declaration is ignored with a configuration warning, but its valid model ID remains pinned. When `<PREFIX>_MODELS` is set, declarations for IDs absent from that allowlist are ignored with a configuration warning.
