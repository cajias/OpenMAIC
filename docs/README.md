...
([`packages/@openmaic/dsl/src/action.ts:235-256`](packages/@openmaic/dsl/src/action.ts#L235-L256)). Four moves operate on that
...
`appendix/`, and two at the root — this page and [`glossary.md`](docs/glossary.md). 837 Mermaid
...
([`16-development-view/07-quality-gates.md`](docs/16-development-view/07-quality-gates.md) §Gate 6).

**Read [`glossary.md`](docs/glossary.md) first if you are new.** One word, `stage`, has four
...
| --- | --- |
| **Anyone, before anything** | [`glossary.md`](docs/glossary.md) — 5 minutes, and it is the difference between reading this set and mis-reading it |
| **Architect, or a new tech lead** | [`18-decisions/index.md`](docs/18-decisions/index.md) → the two records that constrain everything else, [`18-decisions/03-dsl-as-the-serialized-contract.md`](docs/18-decisions/03-dsl-as-the-serialized-contract.md) and [`18-decisions/05-client-first-persistence-with-a-postgres-cutover.md`](docs/18-decisions/05-client-first-persistence-with-a-postgres-cutover.md) → [`02-container-view/index.md`](docs/02-container-view/index.md) → [`14-code-quality/12-remediation-backlog.md`](docs/14-code-quality/12-remediation-backlog.md) |
| **New engineer, week one** | [`01-system-context/index.md`](docs/01-system-context/index.md) → [`01-system-context/01-what-openmaic-is.md`](docs/01-system-context/01-what-openmaic-is.md) → [`02-container-view/index.md`](docs/02-container-view/index.md) → [`11-data-flows/00-index-of-flows.md`](docs/11-data-flows/00-index-of-flows.md) → [`11-data-flows/02-topic-to-classroom.md`](docs/11-data-flows/02-topic-to-classroom.md) → [`03-app-and-api/index.md`](docs/03-app-and-api/index.md) → [`16-development-view/05-local-development.md`](docs/16-development-view/05-local-development.md) → the component topic for your area (03–10) → [`14-code-quality/08-complexity-hotspots.md`](docs/14-code-quality/08-complexity-hotspots.md) |
| **API / skill integrator** | [`12-api-reference/index.md`](docs/12-api-reference/index.md) → [`12-api-reference/09-conventions.md`](docs/12-api-reference/09-conventions.md) (error envelopes, identity, SSE frames) → [`12-api-reference/00-all-routes.md`](docs/12-api-reference/00-all-routes.md) → [`05-agent-runtime/07-skill-package.md`](docs/05-agent-runtime/07-skill-package.md) → [`11-data-flows/09-external-workbench.md`](docs/11-data-flows/09-external-workbench.md) → [`15-cross-cutting/05-auth-and-access-control.md`](docs/15-cross-cutting/05-auth-and-access-control.md) |
| **Self-hoster / operator** | [`17-deployment-view/01-topologies-overview.md`](docs/17-deployment-view/01-topologies-overview.md) → [`17-deployment-view/03-docker-compose.md`](docs/17-deployment-view/03-docker-compose.md) → [`15-cross-cutting/06-configuration.md`](docs/15-cross-cutting/06-configuration.md) → [`04-ai-provider-layer/08-env-vars.md`](docs/04-ai-provider-layer/08-env-vars.md) → [`13-dependencies/06-runtime-prerequisites.md`](docs/13-dependencies/06-runtime-prerequisites.md) → [`15-cross-cutting/05-auth-and-access-control.md`](docs/15-cross-cutting/05-auth-and-access-control.md) (read before exposing a deployment) → [`17-deployment-view/08-operations-runbook.md`](docs/17-deployment-view/08-operations-runbook.md) |
| **Quality or security auditor** | [`14-code-quality/01-method.md`](docs/14-code-quality/01-method.md) (what the measurements can and cannot support) → [`15-cross-cutting/01-trust-boundaries.md`](docs/15-cross-cutting/01-trust-boundaries.md) → [`11-data-flows/12-trust-boundaries-in-flight.md`](docs/11-data-flows/12-trust-boundaries-in-flight.md) → [`15-cross-cutting/02-threat-ssrf.md`](docs/15-cross-cutting/02-threat-ssrf.md) → [`15-cross-cutting/03-threat-injection.md`](docs/15-cross-cutting/03-threat-injection.md) → [`15-cross-cutting/04-threat-secrets-and-uploads.md`](docs/15-cross-cutting/04-threat-secrets-and-uploads.md) → [`13-dependencies/07-licences.md`](docs/13-dependencies/07-licences.md) → [`13-dependencies/08-supply-chain.md`](docs/13-dependencies/08-supply-chain.md) → [`14-code-quality/08-complexity-hotspots.md`](docs/14-code-quality/08-complexity-hotspots.md) |
| **Extending the slide DSL or renderer** | [`07-dsl-renderer-editor/01-dsl-schema.md`](docs/07-dsl-renderer-editor/01-dsl-schema.md) → [`07-dsl-renderer-editor/02-dsl-invariants.md`](docs/07-dsl-renderer-editor/02-dsl-invariants.md) → [`07-dsl-renderer-editor/03-renderer.md`](docs/07-dsl-renderer-editor/03-renderer.md) → [`07-dsl-renderer-editor/10-public-package-api.md`](docs/07-dsl-renderer-editor/10-public-package-api.md) → [`16-development-view/07b-version-and-release-gates.md`](docs/16-development-view/07b-version-and-release-gates.md) (the version-bump gate that protects the serialized format) → [`06-generation-pipeline/05-scene-generation.md`](docs/06-generation-pipeline/05-scene-generation.md) (who produces it) → [`09-media-and-export/06-video-export-pipeline.md`](docs/09-media-and-export/06-video-export-pipeline.md) (the second consumer) |
| **Adding an agent tool or changing the loop** | [`05-agent-runtime/index.md`](docs/05-agent-runtime/index.md) (the two-runtimes table first) → [`05-agent-runtime/01-agent-loop.md`](docs/05-agent-runtime/01-agent-loop.md) → [`05-agent-runtime/01b-loop-dispatch-and-settle.md`](docs/05-agent-runtime/01b-loop-dispatch-and-settle.md) → [`05-agent-runtime/03-tool-catalogue.md`](docs/05-agent-runtime/03-tool-catalogue.md) → [`04-ai-provider-layer/03-stage-routing.md`](docs/04-ai-provider-layer/03-stage-routing.md) → [`10-persistence-and-state/02b-agent-run-tables.md`](docs/10-persistence-and-state/02b-agent-run-tables.md) |
...
| --- | --- | --- | --- |
| [01-system-context](docs/01-system-context/index.md) | What the system is, the five actors, every external system it can talk to, eleven use-case scenarios, and the quality attributes the code optimises for | First. Nothing else parses without it | 7 |
| [02-container-view](docs/02-container-view/index.md) | Which processes and stores exist at run time, and how the source tree layers onto them — including which source roots compile into *two* runtime units | Before you touch any file whose runtime you are unsure of | 7 |
| [03-app-and-api](docs/03-app-and-api/index.md) | Where a request goes: route tree, provider stack, RSC boundary, the one middleware gate, process-boot hook, and the API layer's conventions and deviations | Debugging a 401/404, adding a handler, or a background job that never starts | 8 |
| [04-ai-provider-layer](docs/04-ai-provider-layer/index.md) | The single seam to every text LLM: registry, capabilities, five transports, credential arbitration, per-stage routing, boot validation, usage accounting | "Why did this call go to that vendor / not send `reasoning_effort` / ignore my key?" | 10 |
| [05-agent-runtime](docs/05-agent-runtime/index.md) | Two independent agent runtimes (durable PostgreSQL-leased authoring vs request-scoped in-class chat), the shared harness, the tool catalogue, the external driver skill | Adding a tool, debugging a stuck session, reasoning about crash survival | 10 |
| [06-generation-pipeline](docs/06-generation-pipeline/index.md) | Requirement + materials → persisted course document: ingestion, outline, stage planning, scene generation, prompts, concurrency, progress, quiz grading | Adding a scene type, swapping an extractor, retuning a prompt | 14 |
| [07-dsl-renderer-editor](docs/07-dsl-renderer-editor/index.md) | The slide document end to end: the contract, the renderer, both mutation paths (human and AI), and both PowerPoint bridges | Changing the serialized format, or a `.pptx` fidelity bug | 11 |
| [08-classroom-runtime](docs/08-classroom-runtime/index.md) | Playback state machine, the pure choreography spec shared with the video exporter, the pacing buffer, roundtable cast, PBL v2, the interactive sandbox | Changing playback without breaking video export | 11 |
| [09-media-and-export](docs/09-media-and-export/index.md) | Everything done with media bytes: TTS/ASR, the duration contract, whiteboard, images/video, web search, the nine-pass video compiler, the render container | Touching synthesis, timing, or the MP4 path | 12 |
| [10-persistence-and-state](docs/10-persistence-and-state/index.md) | Five storage primitives and their backends, real column names, the three big client stores, settings pull, chat cutover, access codes, i18n, retention | "Where does this physically live in mode X?" | 10 |
| [11-data-flows](docs/11-data-flows/index.md) | Thirteen traced flows, hop by hop with `file:line` per step, plus concurrency/backpressure and every in-flight trust crossing | Behaviour questions structure cannot answer | 14 |
| [12-api-reference](docs/12-api-reference/index.md) | All 69 `app/api/**/route.ts` files and all 86 handlers: methods, runtime, gating, shapes, side effects, limits — the routes *are* the spec | Calling, changing, or auditing an endpoint | 12 |
| [13-dependencies](docs/13-dependencies/index.md) | External services, 132 runtime + 32 dev npm packages, two vendored forks, six published packages, host prerequisites, licences, supply chain | Provider outage, `pnpm install` surprise, redistribution question | 9 |
| [14-code-quality](docs/14-code-quality/index.md) | Measured size, typing, lint walls, test strategy, eval harnesses, error handling; then the eleven complexity hotspots, the eight lint-enforced boundary rows and their six surviving violations, the duplication and dead-code passes, the strengths worth preserving, and a 22-item ranked backlog — every number with its command, no grades | First week; and before estimating a refactor | 13 |
| [15-cross-cutting](docs/15-cross-cutting/index.md) | Trust boundaries, named threats and the controls that exist (and the ones that do not), auth, configuration, secrets, observability, performance, resilience, i18n/a11y | Before exposing a deployment, or when no single component owns your question | 13 |
| [16-development-view](docs/16-development-view/index.md) | Monorepo layout, workspace dependency graph, the nine-step `postinstall`, every script, local dev, tests and evals, eight gates, five workflows | Running it, releasing a package, predicting which CI job rejects you | 12 |
| [17-deployment-view](docs/17-deployment-view/index.md) | Two build artefacts, four topologies, storage backends per topology, the render service as its own deployable, scaling costs, operations runbook | Standing it up somewhere real | 9 |
| [18-decisions](docs/18-decisions/index.md) | Six irreversible choices as decision records — two agent runtimes, no schema layer at the HTTP edge, the DSL as the serialized contract, the render service as a separate deployable, client-first persistence, one LLM entry point — each with context, alternatives rejected, and consequences | Before proposing a change that crosses two subsystems, or when a shape looks wrong and you need to know whether it was chosen | 7 |
| [glossary.md](docs/glossary.md) | One canonical definition per term. The four senses of `stage`, why `course` has no type, what `classroom` adds over it, and the three things `utterance` can mean | First, and again whenever a word stops making sense | 1 |
...
| 4+1 Logical | What are the domain abstractions and how are they layered? | [`02-container-view/04-logical-layering.md`](docs/02-container-view/04-logical-layering.md), [`02-container-view/06-bounded-contexts.md`](docs/02-container-view/06-bounded-contexts.md), plus the type vocabularies in 03–10 |
| 4+1 Process | What happens over time — who calls whom, in what order, on which thread? | `11-data-flows/` (13 traces), with [`06-generation-pipeline/07-concurrency-and-retry.md`](docs/06-generation-pipeline/07-concurrency-and-retry.md) and [`11-data-flows/11-concurrency-and-backpressure.md`](docs/11-data-flows/11-concurrency-and-backpressure.md) |
...
| 4+1 Use-Case (the "+1") | Which scenarios tie the other four together? | [`01-system-context/05-use-case-scenarios.md`](docs/01-system-context/05-use-case-scenarios.md) (11 scenarios), each forward-linked into `11-data-flows/` |
...
The rules for *adding* to this set — how to write a citation that stays navigable
and rot-detectable, when to link a fact instead of restating it, and which of
these conventions the gate actually enforces — are in
[`CONTRIBUTING-DOCS.md`](docs/CONTRIBUTING-DOCS.md). What follows is what the set
already does.
...
- **`path:line` citations.** Every non-obvious factual claim names a
  ...
  ([`lib/ai/providers.ts:2033`](lib/ai/providers.ts#L2033)). Line numbers drift with every refactor; the
...
| --- | --- | --- |
| [12-api-reference](docs/12-api-reference/index.md) | Enumeration *is* the contract — there is no OpenAPI document and no generated client, so a new route file silently makes the reference incomplete | `git ls-files 'app/api/**/route.ts' \| wc -l` against the documented 69, then `git diff --name-status c2c9553a..HEAD -- app/api` for the deltas. Reconcile against the completeness audit table in [`12-api-reference/08-ops.md`](docs/12-api-reference/08-ops.md), which exists to be the proof |
| [13-dependencies](docs/13-dependencies/index.md) | Two manifests and a lockfile move independently of any prose, and there is no Dependabot/Renovate/SBOM to announce a change | `git diff c2c9553a..HEAD -- package.json pnpm-lock.yaml packages/*/package.json render-service/package.json`. Only the counts and the licence table in [`07-licences.md`](docs/13-dependencies/07-licences.md) need re-deriving; the failure-mode prose is stable |
| [14-code-quality](docs/14-code-quality/index.md) | Every figure is a static measurement of source text at one commit; line counts, `as any` counts and suppression counts change with ordinary work | Re-run the commands printed in [`14-code-quality/01-method.md`](docs/14-code-quality/01-method.md) — they are recorded precisely so this is a rerun, not a re-investigation. Sections 02–07 are then a numbers swap; 08 is judgement and needs a human |
...
**The entry point for all ten is [`appendix/research/index.md`](docs/appendix/research/index.md)**,
...
contradiction in [`12-api-reference/index.md`](docs/12-api-reference/index.md), the nineteen non-parsing Mermaid blocks, and
...
- **Nothing verifies a `path:line` citation.** The checker resolves links, registration and
  Mermaid syntax. It cannot tell you that [`lib/ai/providers.ts:2033`](lib/ai/providers.ts#L2033) still contains
...
- **Whether these docs are meant to be published.** `docs-build.yml` exists and is
  path-filtered (per
  [`16-development-view/08-ci-workflows.md`](docs/16-development-view/08-ci-workflows.md)),
  and there is a `packages/docs` Fumadocs workspace with its own lockfile (per
  [`13-dependencies/08-supply-chain.md`](docs/13-dependencies/08-supply-chain.md)) whose
...
- **`appendix/research/ai-provider-layer/` is now complete.** It was the one pack of the ten
  ...
  [`appendix/research/index.md`](docs/appendix/research/index.md) no longer records a
...
- **Nothing enforces the 120–350-line ceiling** that §Conventions states. Fourteen files
  ...
  largest is
  [`06-generation-pipeline/06-prompt-architecture.md`](docs/06-generation-pipeline/06-prompt-architecture.md)
  at 388 lines. One file is now under the 120 floor —
  [`04-ai-provider-layer/02-model-registry-and-capabilities.md`](docs/04-ai-provider-layer/02-model-registry-and-capabilities.md)
