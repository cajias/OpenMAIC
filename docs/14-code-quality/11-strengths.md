...
ESLint rather than described in a CONTRIBUTING file ([09-architectural-consistency.md](./09-architectural-consistency.md)
...
- **The rules are complete against evasion, and say so.** The `@/…` bans match the string
  *prefix* wherever it appears, precisely so that static, `import type`, `export … from`,
  ...
  one rule with one report (`eslint.config.mjs:82-91`). What is *out of scope* is written
...
- **The allowlists are positive, not negative.** `lib/choreography`,
  ...
  (`eslint.config.mjs:275-292`, `:363-380`, `:162-179`). A new bad dependency fails by
  default.
...
(`eslint.config.mjs:336-347`). That is a level of care most repositories do not spend on
lint config.
...
| --- | --- |
| `eslint.config.mjs:5-12` | why the dynamic-import ban needs `no-restricted-syntax` rather than `no-restricted-imports` (an `ImportExpression` is invisible to the latter), and why it is spread per-block rather than declared repo-wide (flat config *replaces* rule options, so one repo-wide block would silently drop each block's own module boundary) |
| `eslint.config.mjs:603-607` | an earlier revision guarded only `.ts`/`.tsx`, which left `app/api/route.js` and `scripts/*.mjs` free to import the SDK — "review caught it", and `tests/lint-llm-entry-guard.test.ts` now pins the whole extension matrix |
| `eslint.config.mjs:640-648` | why three package trees are ignored by the dynamic-import block *and* nonetheless covered, ending with the observation that the earlier reasoning "was not good enough" because `void import('ai')` under the renderer path passed lint |
| `eslint.config.mjs:575-582` | the provenance of the single-LLM-entry wall: five direct `streamText` calls in the PBL v2 runtime meant zero usage records for the busiest traffic in the product |
| `lib/store/kv-persist.ts:508-510` | why automatic legacy-key migration was *removed*: "repeated review rounds found its legacy-adoption state space too large to keep correct" |
...
(`lib/server/config-validation.ts:198-213`). Meanwhile the checks it runs are the
...
(`lib/server/agent-runtime/agent-driver-model.ts:16,27,34,40`) and only the *boot-time*
...
- The video relay never parses the request body. `capBodyStream(req.body, 300 MiB)` caps a
  ...
  ([`../11-data-flows/08-export-video.md`](../11-data-flows/08-export-video.md)).
- `PreviewGate.acquire` returns a release function that is idempotent by construction, "so
  every route exit can safely call it once" (`render-service/src/preview-gate.ts:26-29`) —
...
| `eslint-disable` | 128, of which three rules account for 122 ([04-lint-and-format.md](./04-lint-and-format.md)) |
...
([09](./09-architectural-consistency.md) has the commands): `lib/**` → `app/**`,
...
the workflow comments (`publish-openmaic-skill.yml:8-9`,
`publish-packages.yml:29-36,298-300`), `lib/brand/brand-config.ts:1-9` ("This workspace
has no vendor shell … the config is static and the desktop flag is always off") and
`lib/brand/brand-context.tsx:8-11` ("accepts the values as props (for future wiring)") all
state what is deliberately *not* implemented, in the file where a reader would otherwise
assume it was. That is why [10](./10-duplication-and-dead-code.md) can classify
...
- Not "the tests are good". 666 files under `tests/` and 6 385 statically counted
  ...
  *volume*, and
  [05-test-strategy.md](./05-test-strategy.md) is explicit that no coverage provider is
  installed and no suite was run here.
- Not "the walls work". Six of the eight are unpinned, and verifying they still fire
  requires running ESLint ([09](./09-architectural-consistency.md), open questions).
- Not "the architecture is right". These are properties of the *engineering*, which is a
  different claim.

---

Next: [12-remediation-backlog.md](./12-remediation-backlog.md) — everything in sections
