...
**Sources:** `lib/persistence/server-provider.ts:25-34,70-90`,
`lib/persistence/asset-collector-schedule.ts:11-17,75-78,94-104,120,147-174`,
`lib/server/agent-runtime/event-notify-bus.ts:1-17,58-69`,
`lib/server/agent-runtime/runner.ts:1863,1892`,
`lib/server/agent-runtime/config.ts:7-19`,
`lib/server/material-extraction/runner.ts:31,78`,
`lib/server/provider-config.ts:417-423`, `lib/server/materials/bytes.ts:57-66`,
`lib/server/classroom-storage.ts:6-29`, `lib/server/usage-storage.ts:10,131-132`,
`render-service/src/job-store.ts`, `render-service/src/artifact-store.ts:27-42`,
`render-service/src/config.ts:79-94`,
`app/api/export-video/render/route.ts:33-38`, `instrumentation.ts`,
...
| --- | --- | --- | --- | --- |
| 1 | pg `Pool` + `PgRuntimeStore`/`PgDocumentStore`/`PgAssetStore` | `globalThis[Symbol.for('openmaic.persistence.provider')]` (`server-provider.ts:30-34`) | one per process, keyed on the connection string (`:74-77`) | N pools; PostgreSQL `max_connections` is the binding constraint |
| 2 | Asset-collector timer + a `max: 2` pool | `globalThis[Symbol.for('openmaic.asset-collector.schedule')]` (`asset-collector-schedule.ts:75-78,120`) | one per process | **Safe.** `:13-17` states several instances may run it at once; each blob is re-checked and locked `FOR UPDATE` in its own transaction, so collectors serialize on the row. The comment ends "please do not add" a distributed lock |
| 3 | `LISTEN` client + in-process fanout `Map<string, Set<Subscriber>>` | `event-notify-bus.ts:58-69` | one dedicated `pg.Client` per instance | **Safe by construction.** `:12-17`: NOTIFY is deliberately a *lossy* wakeup; every SSE route and the runner keep fallback polls and converge anyway. Cost is +1 connection per replica |
| 4 | Agent session runner scan timer + claimed sessions | `runner.ts:1892`, installed from `instrumentation.ts:49` | `maxConcurrent` = 2 per instance (`agent-runtime/config.ts:17`) | **Safe.** PostgreSQL is the authority for claims, leases, ordering, cancellation and recovery. Total concurrency becomes 2N |
| 5 | Material-extraction runner scan + heartbeat timers | `material-extraction/runner.ts:31,78` | per instance | same claim/lease pattern as #4 |
| 6 | `server-providers.yml` parse cache | `_configs: Map` (`provider-config.ts:423`) | per process, read from `process.cwd()` (`:219`) | consistent only if every replica mounts the same file; a change needs a restart everywhere |
| 7 | `LocalMaterialByteStore` singleton | `materials/bytes.ts:57-61`, root `process.cwd()/data` | per process, per filesystem | **Broken.** Bytes written on replica A 404 on replica B while the PostgreSQL metadata row says the material exists |
| 8 | Headless classroom documents and job rows | `data/classrooms/`, `data/classroom-jobs/` (`classroom-storage.ts:6-7`) | per filesystem | **Broken.** `POST /api/generate-classroom` returns a `pollUrl` that only works if the poll lands on the same replica |
| 9 | Usage metering JSONL | `data/usage/YYYY-MM.jsonl` (`usage-storage.ts:10`) | per filesystem | **Fragmented.** N partial logs; analysis must concatenate them |
| 10 | Render client identity | `'direct'` literal unless `TRUST_PROXY_HEADERS === 'true'` (`export-video/render/route.ts:34`) | per request | one shared bucket, which is why `RENDER_MAX_JOBS_PER_USER=0` in Compose |
...
`server-provider.ts:72` constructs `new Pool({ connectionString })` with no
`max`, so the `pg` default applies; `asset-collector-schedule.ts:120` pins
...
(see [`05-render-service-deployment.md`](./05-render-service-deployment.md#scaling)).

| State | Implementation | Consequence at N replicas |
| --- | --- | --- |
| Job records | `InMemoryJobStore` — a `Map` plus a 60 s TTL sweeper (`job-store.ts:25-40,72-80`) | a job submitted to A is `404 Job not found` on B (`main.ts:421`) |
| MP4 artefacts | `LocalDiskArtifactStore` — a `Map` of id → path under `/tmp/openmaic-renders` (`artifact-store.ts:27-42`) | download only works on the rendering replica |
...
| Per-identity cap (`RENDER_MAX_JOBS_PER_USER`) | counted from the local `Map` (`job-store.ts:64-70`) | a user can hold N × the limit by hitting different replicas |
| Extraction permit | `Semaphore(maxConcurrentExtractions)` fixed at 1 (`main.ts:512`, `resource-profile.ts:29`) | RAM ceiling is per replica, which is the point |
| Preview gates (`RENDER_PREVIEW_MAX_IN_FLIGHT` 8, `..._PER_USER` 2) | `PreviewGate` counters (`main.ts:236-237`) | same per-replica multiplication |
...
`app/api/agent/sessions/[id]/events/route.ts:44-45` records the operative fact
...
(`lib/server/agent-runtime/config.ts:50-55`).

## Cross-links

- [`04-serverless-vercel.md`](./04-serverless-vercel.md) — the same inventory as
  a platform-compatibility problem.
- [`08-operations-runbook.md`](./08-operations-runbook.md) — which of these
  states to back up.
- [`../05-agent-runtime/index.md`](../05-agent-runtime/index.md) — the lease,
  claim, and recovery protocol that makes rows 4-5 safe.
- [`../15-cross-cutting/index.md`](../15-cross-cutting/index.md) — the missing
  rate-limiting layer.
...
- `OPENMAIC_AGENT_RUNTIME_MAX_CONCURRENT` defaults to 2 per instance
  (`agent-runtime/config.ts:17`) with no deployment-wide ceiling anywhere. Whether
