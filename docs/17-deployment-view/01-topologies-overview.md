...
**Sources:** [`package.json:15-17`](package.json#L15-L17), [`next.config.ts:4`](next.config.ts#L4), `docker-compose.yml`,
`vercel.json`, `Dockerfile`, `render-service/Dockerfile`, [`README.md:283-489`](README.md#3-run),
`instrumentation.ts`, `lib/config/feature-flags.ts`,
`lib/server/render-service.ts`, [`lib/server/classroom-storage.ts:6-7`](lib/server/classroom-storage.ts#L6-L7),
[`lib/server/materials/bytes.ts:33`](lib/server/materials/bytes.ts#L33), [`lib/server/usage-storage.ts:10`](lib/server/usage-storage.ts#L10);
evidence packs
...
| --- | --- | --- | --- | --- |
| A | Local development | `pnpm dev` ([`package.json:15`](package.json#L15)) | 1 (`next dev`) | LLM/media provider APIs over the internet |
| B | Single-container self-host | `docker compose up --build` ([`README.md:323`](README.md#docker-deployment)) | 1 (`node server.js`, [`Dockerfile:112`](Dockerfile#L112)) | provider APIs |
...
[`04-serverless-vercel.md`](docs/17-deployment-view/04-serverless-vercel.md) is dedicated to what it
cannot do.
...
| Durable agent runtime + Pro workbench | `isAgentRuntimeConfigured()` ([`lib/config/feature-flags.ts:23`](lib/config/feature-flags.ts#L23)) plus `MODEL_ROUTES` route for `maic-agent-driver` | opt-in | **not via compose build args** | **not via compose build args** | no (see below) |
| Asset byte reclamation | `startAssetCollectorSchedule()` ([`instrumentation.ts:21`](instrumentation.ts#L21)), needs `DATABASE_URL` | opt-in | opt-in | yes | no schedule survives |
| Video export as a project ZIP | browser-only compile + JSZip | yes | yes | yes | yes |
| One-click MP4 render | `RENDER_SERVICE_URL` + a healthy `/health` ([`lib/server/render-service.ts:47`](lib/server/render-service.ts#L47)) | manual standalone service | no | yes with `video-export` | needs an externally hosted render service |
| Headless `POST /api/generate-classroom` | writes `data/classroom-jobs/` ([`lib/server/classroom-storage.ts:7`](lib/server/classroom-storage.ts#L7)) and runs in `after()` ([`app/api/generate-classroom/route.ts:48`](app/api/generate-classroom/route.ts#L48)) | yes | yes | yes | no |
| `GET /api/classroom-media/...` file serving | reads `data/classrooms/<id>/` | yes | yes | yes | no |
| Usage metering JSONL | `data/usage/YYYY-MM.jsonl` ([`lib/server/usage-storage.ts:10,17`](lib/server/usage-storage.ts#L10)) | yes | yes | yes | no (writes discarded) |
| Agent material uploads | `LocalMaterialByteStore` rooted at `process.cwd()/data` ([`lib/server/materials/bytes.ts:33`](lib/server/materials/bytes.ts#L33)) | opt-in | opt-in | opt-in | no |
| Site password gate | `ACCESS_CODE` in [`middleware.ts:60`](middleware.ts#L60) | yes | yes | yes | yes |
...
1. **`NEXT_PUBLIC_PRO_WORKBENCH_ENABLED` is not a Docker build argument.**
   [`Dockerfile:51-72`](Dockerfile#L51-L72) declares eleven `ARG`s and mirrors them into `ENV`;
   the workbench flag read by `isProWorkbenchEnabled()`
   ([`lib/config/feature-flags.ts:33`](lib/config/feature-flags.ts#L33)) is not among them, and
   [`docker-compose.yml:5-21`](docker-compose.yml#L5-L21) passes the same eleven. A Compose-built image
   ...
   [`middleware.ts:56`](middleware.ts#L56) returns a 404 for `/workbench*` and
...
2. **`NEXT_PUBLIC_PERSISTENCE_TOKEN` is baked into the public bundle.**
   [`Dockerfile:53`](Dockerfile#L53) and `:64` accept and inline it. [`README.md:390-402`](README.md#server-backed-persistence-postgresql) states
...
[`next.config.ts:4`](next.config.ts#L4) is the hinge: `output: process.env.VERCEL ? undefined :
'standalone'`. Every non-Vercel build produces `.next/standalone/server.js`,
which is what [`Dockerfile:105`](Dockerfile#L105) copies and [`Dockerfile:112`](Dockerfile#L112) runs. A Vercel build
skips standalone output and lets the platform adapter split `app/api/**` into
functions per [`vercel.json:6-10`](vercel.json#L6-L10).
...
| A (e2e) | `next dev` under Playwright | 3002 | [`playwright.config.ts:35`](playwright.config.ts#L35) |
| B, C | `openmaic` container | 3000 → host 3000 | [`Dockerfile:90`](Dockerfile#L90) `ENV PORT=3000`, [`docker-compose.yml:22-23`](docker-compose.yml#L22-L23) |
| C | `postgres` | 5432, **not published** | [`docker-compose.yml:46-64`](docker-compose.yml#L46-L64) declares no `ports` |
| C | `render-service` | 9000, `expose` only | [`docker-compose.yml:83-84`](docker-compose.yml#L83-L84) |
| standalone render | `tsx src/main.ts` | 9000 | [`render-service/src/config.ts:47`](render-service/src/config.ts#L47) |
...
([`docker-compose.yml:142-143`](docker-compose.yml#L142-L143)), which has no route to the host or the internet.

## Where to go next

- [`02-local-dev.md`](docs/17-deployment-view/02-local-dev.md) — topology A in detail.
- [`03-docker-compose.md`](docs/17-deployment-view/03-docker-compose.md) — topologies B and C.
- [`04-serverless-vercel.md`](docs/17-deployment-view/04-serverless-vercel.md) — topology D and its
  hard limits.
- [`05-render-service-deployment.md`](docs/17-deployment-view/05-render-service-deployment.md) — the
  render service, in Compose or standalone.
- [`07-scaling-and-state.md`](docs/17-deployment-view/07-scaling-and-state.md) — why none of these
...
- No Kubernetes manifests, Helm chart, or Terraform exists in the repository, so
  ...
  in-process state inventory in [`07-scaling-and-state.md`](docs/17-deployment-view/07-scaling-and-state.md)
  is the closest thing to a specification.
- Nothing in the repository pins a reverse proxy or TLS terminator.
  `TRUST_PROXY_HEADERS` ([`.env.example:491`](.env.example#L491),
  [`app/api/export-video/render/route.ts:34`](app/api/export-video/render/route.ts#L34)) is the only acknowledgement that
