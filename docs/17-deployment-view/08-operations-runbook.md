...
**Sources:** `app/api/health/route.ts`, [`middleware.ts:60-68`](middleware.ts#L60-L68),
`app/api/agent/runtime/route.ts`, `app/api/export-video/capability/route.ts`,
[`render-service/src/main.ts:241-250`](render-service/src/main.ts#L241-L250), [`docker-compose.yml:56-64`](docker-compose.yml#L56-L64),
`lib/logger.ts`, [`lib/server/config-validation.ts:1-46`](lib/server/config-validation.ts#L1-L46),
[`lib/store/persist-health.ts:13-24`](lib/store/persist-health.ts#L13-L24), [`lib/persistence/server-provider.ts:36-67`](lib/persistence/server-provider.ts#L36-L67),
[`lib/persistence/asset-collector-schedule.ts:147-174`](lib/persistence/asset-collector-schedule.ts#L147-L174), [`instrumentation.ts:57-95`](instrumentation.ts#L57-L95),
[`lib/server/usage-storage.ts:10`](lib/server/usage-storage.ts#L10), [`lib/server/classroom-storage.ts:6-7`](lib/server/classroom-storage.ts#L6-L7),
[`README.md:404-427`](README.md#server-backed-persistence-postgresql), [`.env.example:517-520`](.env.example#L517-L520).

## Health and capability endpoints

| Endpoint | Answers | Auth |
| --- | --- | --- |
| `GET /api/health` | `{ success: true, status: 'ok', version, capabilities: { webSearch, imageGeneration, videoGeneration, tts } }` ([`health/route.ts:12-23`](app/api/health/route.ts#L12-L23)) | **allowlisted** past the `ACCESS_CODE` gate ([`middleware.ts:66`](middleware.ts#L66)) |
| `GET /api/agent/runtime` | `{ enabled, runtimeEnabled }` — usability versus intent ([`runtime/route.ts:21-24`](app/api/agent/runtime/route.ts#L21-L24)) | behind the gate |
| `GET /api/export-video/capability` | `{ success: true, enabled }` after a live 3 s probe of the render service ([`capability/route.ts:14-15`](app/api/export-video/capability/route.ts#L14-L15)) | behind the gate |
| `GET /api/server-providers` | the provider configuration the browser mirrors one-way into settings | behind the gate |
| `GET /health` (render-service, port 9000) | `{ ok, accepting, resourceProfile, versions }` ([`main.ts:241-250`](render-service/src/main.ts#L241-L250)) | none; network-isolated instead |
| `pg_isready -U openmaic -d openmaic` | PostgreSQL liveness, 5 s interval, 10 retries | Compose-internal ([`docker-compose.yml:56-61`](docker-compose.yml#L56-L61)) |
...
provider is enabled and not force-disabled ([`health/route.ts:17-21`](app/api/health/route.ts#L17-L21)).
...
([`runtime/route.ts:21`](app/api/agent/runtime/route.ts#L21)), so its body really is the two-field object shown above.
...
`info`) and `LOG_FORMAT=json` ([`.env.example:451-452`](.env.example#L451-L452)) are the two knobs;
`formatLine` (`:13-26`) emits `{ timestamp, level, tag, message }` as JSON or
`[timestamp] [LEVEL] [tag] message` as text.

| Signal | Emitted by | Means |
| --- | --- | --- |
| `[config] ...` warnings at boot | `validateServerConfig` via [`lib/server/config-validation.ts:44-46`](lib/server/config-validation.ts#L44-L46) | model routing misconfiguration: unparseable `MODEL_ROUTES`, unroutable stage key, unregistered provider prefix, provider with no key, bare model id, `<PREFIX>_MODELS` on an unconfigured provider, runtime flag without `DATABASE_URL`, runtime flag without a valid `maic-agent-driver` route, workbench build flag without the server flag. **Warnings only — the app starts regardless** ([`:23-24`](lib/server/config-validation.ts#L23-L24)) |
| `[instrumentation] Agent runtime startup failed` | [`instrumentation.ts:54`](instrumentation.ts#L54) | the runner/notify-bus/extraction-runner block threw; the app still serves, the runtime does not |
| `Asset collection pass failed; retrying on the next interval` | [`asset-collector-schedule.ts:163`](lib/persistence/asset-collector-schedule.ts#L163) | transient: unreachable database, revoked bucket credential, lock timeout. Only alarming if it repeats every interval |
| `Asset collector reclaimed N unreferenced blob(s)` | [`asset-collector-schedule.ts:157`](lib/persistence/asset-collector-schedule.ts#L157) | reclamation is working; silence means nothing was eligible |
| `[instrumentation] ... drain failed` (four variants) | [`instrumentation.ts:65,70,75,80`](instrumentation.ts#L65) | a shutdown step failed; the remaining steps still ran |
| `[render-service] egress lockdown active` | [`docker-entrypoint.sh:64`](render-service/docker-entrypoint.sh#L64) | expected on every render-service start |
| `[render-service] WARNING: egress lockdown DISABLED` | [`docker-entrypoint.sh:66`](render-service/docker-entrypoint.sh#L66) | somebody set `RENDER_EGRESS_LOCKDOWN=false`; Chromium can reach the Docker network |
| `[render-service] FATAL: ...` | [`docker-entrypoint.sh:53,57,61`](render-service/docker-entrypoint.sh#L53) | the container exited rather than run unisolated |
| `[ServerProviderConfig] Failed to load server-providers.yml` | [`provider-config.ts:226`](lib/server/provider-config.ts#L226) | malformed YAML; the loader returns `{}` and falls back to env vars |
| `Render service health check failed` | [`lib/server/render-service.ts:57`](lib/server/render-service.ts#L57) | logged at `info`, so invisible at default level when `LOG_LEVEL=warn` |

### Browser-side signals

Two are visible to users and worth asking about in a bug report:

- The **persistence-unavailable toast** on `/`. [`README.md:385-388`](README.md#server-backed-persistence-postgresql) describes the
...
- The **`persist-health` channel** ([`lib/store/persist-health.ts:13-19`](lib/store/persist-health.ts#L13-L19)) with
...
| `.env.local` | out of band; contains every provider secret | excluded from images by [`.dockerignore:20-21`](.dockerignore#L20-L21) |
| `server-providers.yml` | out of band | excluded from images by [`.dockerignore:22`](.dockerignore#L22); bind-mounted read-only |
...
`ASSET_COLLECTION_GRACE_MS` deliberately — [`README.md:420-423`](README.md#server-backed-persistence-postgresql) calls it "the
...
2. **Rebuild, do not restart, to change a `NEXT_PUBLIC_*` flag or
   ...
   ([`Dockerfile:52-72`](Dockerfile#L52-L72), [`next.config.ts:38-56`](next.config.ts#L38-L56)).
3. **Restart to change `server-providers.yml`.** The parse is cached in a
   module-level `Map` ([`provider-config.ts:423`](lib/server/provider-config.ts#L423)).
4. **Rotating the PostgreSQL password does not work through
   `PERSISTENCE_POSTGRES_PASSWORD`** once the volume exists
   ([`README.md:404-410`](README.md#server-backed-persistence-postgresql)): either `docker compose --profile server-persistence
...
- [`07-scaling-and-state.md`](docs/17-deployment-view/07-scaling-and-state.md) — which of these states
  is per-replica.
- [`03-docker-compose.md`](docs/17-deployment-view/03-docker-compose.md) — the volumes and profiles
  referenced above.
- [`../14-code-quality/index.md`](docs/14-code-quality/index.md) — the measured
  view of the gaps listed in "What does not exist".
- [`../15-cross-cutting/index.md`](docs/15-cross-cutting/index.md) — logging,
...
- An issued access token does not expire. The token carries a timestamp
  ([`access-token.ts:5-7`](lib/server/access-token.ts#L5-L7)) and there are two independent verifiers of it —
  [`middleware.ts:18-44`](middleware.ts#L18-L44) (Edge, WebCrypto) for the gated surface and
  `verifyAccessToken` ([`lib/server/access-token.ts:11-24`](lib/server/access-token.ts#L11-L24), Node `createHmac`) for
