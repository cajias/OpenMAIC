...
**Sources:** [`package.json:6-8,15`](package.json#L6-L8), `.nvmrc`, [`playwright.config.ts:23-36`](playwright.config.ts#L23-L36),
`instrumentation.ts`, [`lib/persistence/bootstrap.ts:15-17,41-75`](lib/persistence/bootstrap.ts#L15-L17),
[`lib/utils/database.ts:299-322`](lib/utils/database.ts#L299-L322), [`lib/document-store/store.ts:26`](lib/document-store/store.ts#L26),
[`lib/runtime/store.ts:26`](lib/runtime/store.ts#L26), [`lib/server/usage-storage.ts:10,100`](lib/server/usage-storage.ts#L10),
[`lib/server/classroom-storage.ts:6-7`](lib/server/classroom-storage.ts#L6-L7), [`lib/server/materials/bytes.ts:33`](lib/server/materials/bytes.ts#L33),
[`lib/persistence/asset-collector-schedule.ts:102-104`](lib/persistence/asset-collector-schedule.ts#L102-L104),
`lib/config/feature-flags.ts`, `tests/setup-env.ts`,
[`.env.example:460-525`](.env.example#L460-L525),
[`README.md:283-305`](README.md#3-run); evidence pack
...
| --- | --- | --- |
| Node | `>=22.19.0` | [`package.json:7`](package.json#L7) |
| Node (nvm hint) | `22` | `.nvmrc` |
| Package manager | `pnpm@10.28.0` (with lockfile hash) | [`package.json:209`](package.json#L209) |
| Config file | `.env.local` (Next loads it; not in git) | [`.dockerignore:20-21`](.dockerignore#L20-L21) excludes `.env*` from images |
| Minimum config to be useful | one LLM provider key + optionally `DEFAULT_MODEL` | [`.env.example:8-48`](.env.example#L8-L48), [`:409`](.env.example#L409) |

`pnpm install` triggers the nine-step `postinstall` chain
([`package.json:10`](package.json#L10)) that builds both vendored forks, all six `@openmaic/*`
...
`register()` ([`instrumentation.ts:13`](instrumentation.ts#L13)) returns early unless
...
timer ([`asset-collector-schedule.ts:172`](lib/persistence/asset-collector-schedule.ts#L172)); turn on
`OPENMAIC_AGENT_RUNTIME_ENABLED` too and you get a dedicated PostgreSQL `LISTEN`
connection plus two scan timers ([`instrumentation.ts:44-51`](instrumentation.ts#L44-L51)).

## Ports

| Port | Process | Set by |
| --- | --- | --- |
| 3000 | `next dev` | Next default; [`README.md:289`](README.md#3-run) |
| 3002 | `next dev` under Playwright | [`playwright.config.ts:29,35`](playwright.config.ts#L29) (`env: { PORT: '3002' }`) |

The e2e server is a *second* dev server on a different port with
`NEXT_PUBLIC_MAIC_EDITOR_ENABLED=true` injected
([`playwright.config.ts:35`](playwright.config.ts#L35)), and `reuseExistingServer: !process.env.CI`
...
| What | Where | Lifetime |
| --- | --- | --- |
| Courses, scenes, audio/image/media byte rows, chat sessions, folders, playback state | IndexedDB `MAIC-Database` (latest schema version 17, 16 version blocks declared — 13 is skipped, [`lib/utils/database.ts:328-562`](lib/utils/database.ts#L328-L562)) | until the origin's site data is cleared |
...
| Usage metering | `./data/usage/YYYY-MM.jsonl` ([`lib/server/usage-storage.ts:10,17`](lib/server/usage-storage.ts#L10)) | until deleted by hand |
| Headless-generated classrooms | `./data/classrooms/<id>.json` plus `media/` and `audio/` subdirectories | same |
| Headless generation job rows | `./data/classroom-jobs/` ([`lib/server/classroom-storage.ts:7`](lib/server/classroom-storage.ts#L7)) | same |
| Agent material bytes | `./data/` via `LocalMaterialByteStore` ([`lib/server/materials/bytes.ts:33`](lib/server/materials/bytes.ts#L33)) | same |

`./data/` is in [`.dockerignore:33`](.dockerignore#L33), so nothing you accumulate locally leaks into
an image. It is the same relative path the container mounts as a volume — see
[`03-docker-compose.md`](docs/17-deployment-view/03-docker-compose.md).
...
([`lib/persistence/bootstrap.ts:16`](lib/persistence/bootstrap.ts#L16)), which Next inlines. Changing it requires
...
| Durable agent runtime + `/workbench` | `isAgentRuntimeConfigured()` false ([`lib/config/feature-flags.ts:23`](lib/config/feature-flags.ts#L23)) | `/workbench*` returns a plain-text 404 from [`middleware.ts:57`](middleware.ts#L57); `/workspace` `redirect('/')` at [`app/workspace/page.tsx:35`](app/workspace/page.tsx#L35); `/api/agent/sessions*` answer 404 |
| One-click MP4 export | no `RENDER_SERVICE_URL` | `GET /api/export-video/capability` returns `{ success: true, enabled: false }` ([`app/api/export-video/capability/route.ts:14-15`](app/api/export-video/capability/route.ts#L14-L15), envelope from `apiSuccess`); the export menu offers ZIP only |
| Asset byte reclamation | no `DATABASE_URL` | nothing to reclaim; the schedule returns `undefined` |
| Site password gate | no `ACCESS_CODE` | [`middleware.ts:61-63`](middleware.ts#L61-L63) passes everything through |
| Self-hosted model endpoints (Ollama, VoxCPM, local MinerU) | SSRF guard blocks private addresses | set `ALLOW_LOCAL_NETWORKS=true` ([`.env.example:461-463`](.env.example#L461-L463)) — a **global** switch affecting 13 routes |
...
- [`tests/setup-env.ts:23`](tests/setup-env.ts#L23) does **not** load `.env.local` unless
...
- [`lib/server/usage-storage.ts:100`](lib/server/usage-storage.ts#L100) returns early when `process.env.VITEST` or
  `NODE_ENV === 'test'` is set unless an explicit `baseDir` is passed, so no
  test appends to the real `data/usage/` log.

## Cross-links

- [`../16-development-view/05-local-development.md`](docs/16-development-view/05-local-development.md)
  — the first-run command sequence and toolchain.
- [`03-docker-compose.md`](docs/17-deployment-view/03-docker-compose.md) — the same shape as a
...
- The dev server's bundler is not pinned by a flag in [`package.json:15`](package.json#L15)
  (`next dev`, no `--turbo`/`--webpack`). Two source comments —
  [`next.config.ts:16-22`](next.config.ts#L16-L22) and [`lib/server/register-shutdown-signals.ts:5-10`](lib/server/register-shutdown-signals.ts#L5-L10) —
...
- [`lib/utils/database.ts:300`](lib/utils/database.ts#L300) names the constant `_DATABASE_VERSION = 17` with a
