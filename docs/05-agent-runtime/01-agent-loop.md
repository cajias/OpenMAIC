...
(`lib/server/agent-runtime/runner.ts:886-888`: "its nested finally blocks pair
...
writes a row and returns 202 (`app/api/agent/sessions/route.ts:37`, response at
`:168-175`). A background timer picks it up later. The consequence is stated in
the runner's own header (`runner.ts:2-7`): "PostgreSQL is the authority for claims,
...
once-per-process guarantee (`instrumentation.ts:3-8`), it returns early on the
...
`WORKER_ID` is `<8 hex of a fresh uuid>:<pid>` (`runner.ts:100`), so two processes
...
`runSession(ctx, meta)` (`runner.ts:889`) captures three values off the claim that
...
`meta.attempt > config.maxAttempts` (`runner.ts:305-307`), and when it is true the
...
and returns without ever constructing a model (`runner.ts:1070-1088`). The comment
...
`enqueue(write, critical)` (`runner.ts:915-934`) serialises every durable write
...
`emit(type, data)` (`runner.ts:983-1044`) is the only event writer, and it does five
...
- **`planResume`** (`resume.ts:93`) returns `{kind:'start'}`,
  ...
  (`resume.ts:33-37`): tool execution is **at-least-once**, therefore every tool
  must be idempotent.
- **Truncation is durable.** If `planResume` retained fewer messages than the tree
  ...
  than degrading (`runner.ts:1153-1159`).
- **Missing tool results are a read-time view.** `repairOrphanedToolCalls`
  (`tool-call-integrity.ts:109`) rebuilds a provider-safe message list: existing
...
| --- | --- | --- |
| Driver model | `resolveAgentDriverModel()` (`agent-driver-model.ts:83`) | reads only `MODEL_ROUTES` stage `maic-agent-driver`; `DEFAULT_MODEL` is never consulted |
| Pending durable messages | `listAgentUserMessages` + filter `seq > deliveredThrough` → `toFollowUp` (`runner.ts:1204-1222`, `:860`) | each `@`-named classroom is re-resolved to its **current** name by `resolveCourseRefsForContext` (`:704`) |
| Selected slide/interactive elements | `resolveFollowUpElementContext` → `resolveElementRefsForContext` (`runner.ts:1320`, `:412`) | five-status, six-variant result — `resolved` splits by `kind` (`:376-410`); every captured field is wrapped in `<untrusted-live-element-data>` by `untrustedElementDataBlock` (`:502`) |
| Skill bodies | `buildSkillPreload` (`skill-preload.ts:224`) | synthesises an `assistant(toolCall read)` + `toolResult(SKILL.md)` pair — "a read that already happened" |

`planRunStart` (`runner.ts:772-827`) decides whether this run opens with a prompt
...
(`runner-contract.ts:14`), which is `courseSystemPrompt` with the DSL
compatibility block forced on. Every other block is present only when its
capability is (`runner.ts:1463-1472`): `availableSkills`, `curriculum`, `search`,
`fetch`, `untrustedContent`, `materials`, `roster`, `voice`.

## Five: the model call

`buildAgent` (`build-agent.ts:65`) is the only place a pi `Agent` is constructed in
...
(`runner.ts:1473`), so STUB_MODEL only applies to a caller that omits `model`.

`createCallLlmStreamFn` (`stream-fn.ts:250`) is the integration seam. pi's
...
(`stream-fn.ts:413`) so **pi's loop owns multi-step, not the AI SDK's**, and maps
...
`build-agent.ts:103-125` sees the `turn_end` frame, calls `agent.clearAllQueues()`,
...
(`PI_EVENT_TYPES`, `lib/workbench/use-workbench-session.ts:50-61`). Read as a state
...
carried thinking (`runner.ts:1009-1043`), so the chat's thinking bar stops at the
...
loop — are in [`01b-loop-dispatch-and-settle.md`](./01b-loop-dispatch-and-settle.md),
