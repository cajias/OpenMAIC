...
Evidence: [`../appendix/research/agent-runtime/02b-tool-catalogue.md`](docs/appendix/research/agent-runtime/02b-tool-catalogue.md).
...
| Tool | Gate | Site |
| --- | --- | --- |
| `web_search` | `resolveWebSearchCapability()` non-null | [`runner.ts:1286-1293`](lib/server/agent-runtime/runner.ts#L1286-L1293) |
| `register_voice` | `hasConfiguredVoiceRegistrationCapability()` | [`runner.ts:1420`](lib/server/agent-runtime/runner.ts#L1420), allowlist `:1490` |
| `generate_video` | `hasConfiguredVideoGeneration(deps)` | [`course-tools.ts:214`](lib/server/agent-runtime/course-tools.ts#L214) |
| `render_scene_preview` | render service configured — `deps.renderService ?? resolveRenderServiceUrl()`, `if ('error' in service) return []` | [`scene-preview.ts:42-43`](lib/server/agent-runtime/scene-preview.ts#L42-L43), built at [`runner.ts:1382`](lib/server/agent-runtime/runner.ts#L1382) |
| `read` (skill-scoped) | `installedSkills.length > 0` | [`runner.ts:1190-1194`](lib/server/agent-runtime/runner.ts#L1190-L1194) |
| `use_material_media` | `deps.sessionId` present | [`course-tools.ts:216`](lib/server/agent-runtime/course-tools.ts#L216) |
| `wb_*` (classroom) | four preconditions at route level | [`app/api/chat/pi/route.ts:164-187`](app/api/chat/pi/route.ts#L164-L187) |
...
`beforeToolCall` ([`build-agent.ts:82`](lib/agent/runtime/build-agent.ts#L82)); the gate is ten lines and returns
`{block: true, reason: 'Tool "<name>" is not enabled in this build.'}`. The
runner's set ([`runner.ts:1475-1491`](lib/server/agent-runtime/runner.ts#L1475-L1491)) mixes two styles:
...
`buildCourseAllowlist` ([`course-tools.ts:223`](lib/server/agent-runtime/course-tools.ts#L223)) exists as a self-consistent
...
(`getOwnerScopedDocumentStore(meta.ownerId, leaseGuardedHook)`, [`runner.ts:1303`](lib/server/agent-runtime/runner.ts#L1303)),
and `ownerId` is deliberately absent from every model-visible parameter so the
model cannot forge a target owner ([`runner.ts:1294-1302`](lib/server/agent-runtime/runner.ts#L1294-L1302), `:1432-1435`).

`withOwnerStageAuthorization` ([`course-tools.ts:159-191`](lib/server/agent-runtime/course-tools.ts#L159-L191)) wraps every tool that
...
A second, lease-free store (`mediaJobStore`, [`runner.ts:1317`](lib/server/agent-runtime/runner.ts#L1317)) exists for exactly
...
([`mutation-fence.ts:10`](lib/server/agent-runtime/mutation-fence.ts#L10), 25 lines over `AsyncLocalStorage`), and the run's store
transaction hook asserts the tool's abort signal **and** the lease, twice, around
the lease read ([`runner.ts:1305-1309`](lib/server/agent-runtime/runner.ts#L1305-L1309)). `mediaJobStore`'s hook asserts only the
mutation fence (`:1317-1319`).

### Layer 5 — sequential scheduling

`markDocumentWritersSequential` ([`course-tools.ts:137`](lib/server/agent-runtime/course-tools.ts#L137)) marks every member of
...
`DOCUMENT_WRITING_TOOLS` ([`course-tools.ts:110-112`](lib/server/agent-runtime/course-tools.ts#L110-L112)) is
`STAGE_WRITER_TOOL_NAMES` minus `rename_stage`, which is scheduled in the
curriculum toolset instead. `STAGE_WRITER_TOOL_NAMES`
([`lib/agent-runtime/stage-writer-tools.ts:20`](lib/agent-runtime/stage-writer-tools.ts#L20)) is the single isomorphic registry
...
| `import_pptx` ([`import-pptx.ts:357`](lib/server/agent-runtime/import-pptx.ts#L357), name const [`:39`](lib/server/agent-runtime/import-pptx.ts#L39)) | ✓ | appends pages to an existing stage from an uploaded `.pptx` material | always |
| `generate_image` ([`generate-image.ts:211`](lib/server/agent-runtime/generate-image.ts#L211), const [`:40`](lib/server/agent-runtime/generate-image.ts#L40)) | ✓ | persists bytes into the target stage's media and returns a renderable `src` + dimensions; the description states "This tool never edits a page itself" ([`:213`](lib/server/agent-runtime/generate-image.ts#L213)) — no `putScene` call exists in the module | always |
| `generate_video` ([`generate-video.ts:480`](lib/server/agent-runtime/generate-video.ts#L480), const [`:43`](lib/server/agent-runtime/generate-video.ts#L43)) | | returns a `gen_vid_<id>` placeholder immediately; the detached job patches the document later through `mediaJobStore` and emits `media_ready` | only with a configured video provider |
| `use_material_media` ([`material-media.ts:21`](lib/server/agent-runtime/material-media.ts#L21), const [`:79`](lib/server/agent-runtime/material-media.ts#L79)) | ✓ | copies a session material's bytes into the stage media path | only with a `sessionId` |
| `render_scene_preview` ([`scene-preview.ts:46`](lib/server/agent-runtime/scene-preview.ts#L46), const [`:101`](lib/server/agent-runtime/scene-preview.ts#L101)) | | PNG of a persisted page | only with a render service; carries its **own** owner probe, not the generic wrapper |

### Materials and the web

`MATERIAL_TOOL_NAMES` ([`material-tools.ts:604`](lib/server/agent-runtime/material-tools.ts#L604)) is six names and **includes
...
| `fetch_url` ([`fetch-url.ts:616`](lib/server/agent-runtime/fetch-url.ts#L616)) | ✓ | creates a web material | **always registered**; the security property is the URL trust gate, not registration — only origins already observed in a user message or this session's `web_search` results |
| `web_search` ([`web-search.ts:48`](lib/server/agent-runtime/web-search.ts#L48)) | ✓ | registers every result URL in the session trust gate before returning ([`runner.ts:1289-1291`](lib/server/agent-runtime/runner.ts#L1289-L1291)) | only with a configured backend |
...
| `set_roster` ([`roster-tools.ts:234`](lib/server/agent-runtime/roster-tools.ts#L234)) | ✓ | the stage's agent roster; member of `STAGE_WRITER_TOOL_NAMES`; owner-gated by an explicit `withOwnerStageAuthorization` wrap ([`runner.ts:1402-1410`](lib/server/agent-runtime/runner.ts#L1402-L1410)) | always |
| `clip_audio` ([`voice-clone-tools.ts:272`](lib/server/agent-runtime/voice-clone-tools.ts#L272)) | ✓ | a clip artifact from an audio/video material | always |
| `register_voice` ([`voice-clone-tools.ts:330`](lib/server/agent-runtime/voice-clone-tools.ts#L330)) | ✓ | registers a cloned voice with the provider; appended to a **run-scoped** in-memory array ([`runner.ts:1398`](lib/server/agent-runtime/runner.ts#L1398)) — deliberately not persisted | only when a keyed provider reports `supportsRegistration` |

### Skills

| Tool | M | Mutates | Registration |
| --- | --- | --- | --- |
| `read` ([`skills.ts:607`](lib/server/agent-runtime/skills.ts#L607)) | | — | only when skills are installed; `realpath`-contained to installed skill directories |
| `create_skill` ([`create-skill.ts:27`](lib/server/agent-runtime/create-skill.ts#L27)) | ✓ | one `usk_` user-skill row | always; create-only, duplicates refused; a successful call is treated as run-terminal on resume ([`resume.ts:126-130`](lib/server/agent-runtime/resume.ts#L126-L130)) |
| `read_skill` ([`skill-edit-tools.ts:265`](lib/server/agent-runtime/skill-edit-tools.ts#L265)) | | — | always; `detail: 'source'` is the only view of the stored bytes and therefore the precondition for `patch_skill` |
| `patch_skill` ([`skill-edit-tools.ts:305`](lib/server/agent-runtime/skill-edit-tools.ts#L305)) | ✓ | `/content`, `/title`, `/description` of one owner skill (`/name` is not editable) | always |
...
([`runner.ts:1436-1441`](lib/server/agent-runtime/runner.ts#L1436-L1441)).

### Control and read-only history

| Tool | M | Mutates |
| --- | --- | --- |
| `ask_user` ([`ask-user.ts:47`](lib/server/agent-runtime/ask-user.ts#L47)) | | emits the durable `user_question` event and latches the run terminal |
| `search_classrooms` ([`personal-history-tools.ts:406`](lib/server/agent-runtime/personal-history-tools.ts#L406)) | | — |
| `read_classroom` (`:440`) | | — |
| `search_chats` (`:505`) | | — |
...
([`personal-history-tools.ts:19-22`](lib/server/agent-runtime/personal-history-tools.ts#L19-L22)).
...
| `call_agent` ([`call-agent.ts:574`](lib/chat/pi/tools/call-agent.ts#L574)) | director | drives a child agent | `sequential`; six skip guards |
...
| `spotlight` ([`native-spotlight.ts:78`](lib/chat/pi/tools/native-spotlight.ts#L78)) | child | emits a slide action | flag on **and** `allowedActions` includes `spotlight` **and** the authorised element-id set is non-empty; takes an `authorizedElementIds` allow-set |
...
re-read ([`native-whiteboard.ts:500`](lib/chat/pi/tools/native-whiteboard.ts#L500)).

In the **legacy** child harness the child has no tools at all
([`call-agent.ts:889-890`](lib/chat/pi/tools/call-agent.ts#L889-L890): `tools: []`, `allowedToolNames: new Set()`); actions come
...
`withAgentToolTimeout` ([`tool-timeout.ts:98`](lib/agent/runtime/tool-timeout.ts#L98)) races every call against a budget and
...
- **`import_pptx`, `generate_image` and `generate_video` render their raw wire
  ...
  i18n labels already exist. See [`08-failure-modes.md`](docs/05-agent-runtime/08-failure-modes.md).
- **The quota hook is an open stub.** [`quota.ts:8-13`](lib/agent/runtime/quota.ts#L8-L13) is 13 lines and `buildAgent`
  wires it with `remaining: () => Number.MAX_SAFE_INTEGER` ([`build-agent.ts:66`](lib/agent/runtime/build-agent.ts#L66)).
...
- **`register_voice` results do not survive the run** — the array is run-local
  ([`runner.ts:1398`](lib/server/agent-runtime/runner.ts#L1398)), the comment calls it "in-session loop by design, no
