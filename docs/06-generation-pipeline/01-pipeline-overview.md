...
evidence: [`00-overview.md`](docs/appendix/research/generation-pipeline/00-overview.md),
...
| 2 | Outline generation | requirement + bundle | `{ languageDirective, courseTitle?, outlines: SceneOutline[] }` | package ([`outline-generator.ts:120`](packages/@openmaic/generation/src/outline-generator.ts#L120)) |
...
content at [`app/api/generate/scene-content/route.ts:39`](app/api/generate/scene-content/route.ts#L39), 60 s for actions at
[`app/api/generate/scene-actions/route.ts:35`](app/api/generate/scene-actions/route.ts#L35)).
...
- The **bundle is the only ingestion output the model ever sees.** Raw provider text is
  ...
  ([`lib/document/bundle.ts:181`](lib/document/bundle.ts#L181)).
- **Stage 3 does not gate stage 4.** The stage id feeds `buildCompleteScene` as a plain
  ...
  ([`app/generation-preview/page.tsx:534`](app/generation-preview/page.tsx#L534), [`:944`](app/generation-preview/page.tsx#L944)).

## Two execution drivers over the same primitives

| | Interactive (default UI) | Headless one-shot job |
| --- | --- | --- |
| Entry | `/generation-preview` → [`app/generation-preview/page.tsx:306`](app/generation-preview/page.tsx#L306) | `POST /api/generate-classroom` → [`app/api/generate-classroom/route.ts:14`](app/api/generate-classroom/route.ts#L14) |
| Scene loop | [`lib/hooks/use-scene-generator.ts:627`](lib/hooks/use-scene-generator.ts#L627), optionally parallel | [`lib/server/classroom-generation.ts:558`](lib/server/classroom-generation.ts#L558), strictly serial |
...
| Agents | resolved from the client registry before outlines | resolved *after* outlines so personas honour `languageDirective` ([`classroom-generation.ts:505`](lib/server/classroom-generation.ts#L505)) |
| Media/TTS | fire-and-forget alongside the loop ([`use-scene-generator.ts:672`](lib/hooks/use-scene-generator.ts#L672)) | discrete post-loop phases, each best-effort |
...
([`lib/server/classroom-generation.ts:186`](lib/server/classroom-generation.ts#L186)–[`:728`](lib/server/classroom-generation.ts#L728)); the browser driver uses the step index
from [`app/generation-preview/types.ts:90`](app/generation-preview/types.ts#L90) instead of a percentage.
...
| --- | --- | --- |
| `preparing` | per-document extraction fan-out, then bundling | [`page.tsx:340`](app/generation-preview/page.tsx#L340), `:419` |
| outline stream | outlines appear one at a time as they parse | [`page.tsx:613`](app/generation-preview/page.tsx#L613) |
| `review` or `outline-ready` | either the editor opens, or a 2500 ms auto-continue timer runs | [`page.tsx:680`](app/generation-preview/page.tsx#L680), `OUTLINE_REVIEW_AUTO_CONTINUE_MS` |
| `generating-content` | first scene only: content → actions → TTS, then navigate | [`page.tsx:969`](app/generation-preview/page.tsx#L969)–`:1052` |
| classroom | remaining scenes generated behind the live classroom | [`use-scene-generator.ts:627`](lib/hooks/use-scene-generator.ts#L627) |
...
`PrepFailed` is worth naming: [`app/generation-preview/page.tsx:340`](app/generation-preview/page.tsx#L340) wraps per-document
...
[`./02b-bundling-and-egress.md`](docs/06-generation-pipeline/02b-bundling-and-egress.md#partial-failure-inside-ingestion).

## What is deliberately not in this subsystem

- Provider implementations for TTS, images, and video — see [`../09-media-and-export/index.md`](docs/09-media-and-export/index.md). Only their call sites appear here.
- The PBL *runtime* kernel (`packages/@openmaic/generation/src/pbl/operations/kernel/*`, 2368 LOC). It is exported by this package but consumed during classroom playback, not generation — see [`../08-classroom-runtime/index.md`](docs/08-classroom-runtime/index.md).
- `lib/document/transforms/**`. A complete normalise/noise-removal framework whose only caller is [`lib/rag/ingest/document.ts:138`](lib/rag/ingest/document.ts#L138); generation feeds raw provider text into the prompt.
- `lib/import/**`. Classroom-ZIP and PPTX import build scenes without the LLM at all — see [`../07-dsl-renderer-editor/index.md`](docs/07-dsl-renderer-editor/index.md).
...
- **Whether `lib/document/transforms/` was meant to run before outline generation.** Its
  ...
  ([`lib/document/transforms/types.ts:3`](lib/document/transforms/types.ts#L3)), but no generation code path calls
  `transformDocument`.
