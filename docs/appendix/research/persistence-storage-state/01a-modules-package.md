...
Version `0.28.1` (`packages/@openmaic/storage/package.json:3`). Sole runtime
dependency: `@openmaic/dsl` (workspace). Two optional peers:
`@aws-sdk/client-s3`, `@aws-sdk/s3-request-presigner`
(`package.json:110-121`). 16 subpath exports (`package.json:9-74`); the root
...
(`src/kv/types.ts:8`). `account` values are user data a server-backed deployment
may sync; `device` values are machine-local and "must never leave the device —
every backend honours that" (`src/kv/types.ts:1-7`). Default scope is `account`
(`DEFAULT_KV_SCOPE`, `src/kv/types.ts:58`).

Three type-level layers encode the device guarantee:

- `KVStore` (`kv/types.ts:15`) — `get`/`set`/`remove`/`keys`, all scope-taking.
- `DeviceSafeKVStore` (`kv/types.ts:40`) — brand `servesDeviceScopeLocally: true`.
  ...
  network transport (`kv/types.ts:22-39`).
- `LocalKVStore` (`kv/types.ts:53`) — additionally `isLocalKVStore: true`; what a
  composite demands of the backend it injects for `device`.

`assertKVScope` (`kv/types.ts:83`) fails closed on an unrecognised scope, and the
comment names both failure modes it prevents. Deliberately **no key validator**:
`kv/types.ts:95-117` argues the key domain is opaque and unbounded, and that
transport limits (URL length, `.`/`..` normalisation, unpaired surrogates) belong
to `HttpAccountKV`, not to the primitive.

`BrowserKVStore` (`kv/browser.ts:24`) stores both scopes in one `Storage` behind
key prefix `<namespace>:<scope>:` with `namespace` defaulting to `'maic'`
(`kv/browser.ts:34,37-41`). `set` treats `undefined`/function/symbol as a
...
`kvPersistStorage` (`zustand/persist.ts:54-75`) is the zustand adapter. Two
overloads and no `(KVStore, KVScope)` overload — "it would match every call the
two above reject, handing the guard straight back" (`zustand/persist.ts:56-58`).
The `device` overload requires the brand and re-checks at runtime.

## Document — `src/document/`

`MaicDocument` (`document/types.ts:87`) is `{ stage, scenes, outline?, dslVersion? }`.
`outline` is app-owned, persisted verbatim, never validated or migrated
(`document/types.ts:82-85`). `DocumentStore` (`document/types.ts:180`) is
...
(`document/types.ts:196-204`).

`DocumentFolderStore` (`document/types.ts:136`) is available "only on a bound
server store: callers choose the trusted owner with `forOwner`, never with method
parameters" (`document/types.ts:131-135`). `deleteFolder` has two modes,
`'ungroup'` (documents kept, `folder_id` cleared) and `'remove'` (returns
`removedStageIds` for the caller's own cascade).

`StageFreshnessManifestStore` (`document/types.ts:276`) reads the
trigger-maintained `document_stage_revision` / `document_scene_revision` rows —
PG backend only.

`document/pg.ts` holds `DOCUMENT_PG_SCHEMA` (`pg.ts:58-225`) and
`splitSqlStatements` (`pg.ts:234`), a real SQL tokeniser that skips quoted
...
writers comes back" (`pg.ts:129-133`). Both triggers `pg_notify` on channel
...
`runtime/pg.ts:41-53` defines the injected surface reused by every PG backend:
...
`AssetStore` (`asset/types.ts:140`) has five methods plus optional
...
- `revision` is a counter starting at 1, "never derived from the content" — a
  ...
  (`asset/types.ts:284-293`).

`EXCLUDED_RENDERABLE_TYPES` (`asset/types.ts:114`) is the hard denylist —
...
into cross-site scripting" (`asset/types.ts:104-113`).
...
(`asset/pg.ts:1-23`). A byte layer whose writes cannot join the registry
transaction must declare `writesOutsideRegistryDatabase` or the registry refuses
the configuration (`asset/pg.ts:13-18`) — otherwise it self-deadlocks on the
...
accidentally gain lease-bound write authority" (`agent-session/types.ts:1-9`):
...
- `claimNextSession` scans optimistically, then locks and rechecks; the second
  ...
  (`agent-session/types.ts:275-294`).
- `postUserMessage` locks, persists, classifies delivery (`steer` vs `queued`)
  ...
  into the runner's settle window" (`types.ts:328-337`).
- `OwnerSessionEventProjection.append` runs inside the caller's transaction
  ...
  never veto the authoritative lifecycle write" (`types.ts:455-462`).
- `AgentSessionUrlStore` is the fetch trust gate: only URLs the user typed or
  ...
  (`types.ts:569-575`). `extractObservedUrls` (`types.ts:45`) strips trailing
  prose including CJK punctuation.

`AgentSessionHooks` (`types.ts:495-567`) documents each hook's transaction
...
(`material/types.ts:1-15,98-100`). Ids are `mat_` + a Crockford-base32 encoding of
128 random bits (`createMaterialId`, `material/types.ts:21-38`). Six kinds —
`source`, `extraction`, `transcript`, `audio-track`, `image`, `web`
(`material/types.ts:44-51`) — chosen so derivatives need no schema migration.

Extraction is a leased state machine on the source row
(`material/types.ts:55-63,183-206`), with `MAX_MATERIAL_EXTRACTION_RETRIES = 2`
...
live session (`material/pg.ts:249-257`). `enqueueExtraction` only moves rows in
`('idle','failed')` (`material/pg.ts:334-342`).
...
(`material/pg.ts:86-88`). `listMaterials` is keyset-paged on a material id, default
50, capped at 200 (`material/types.ts:158-163`).

## Server handlers — `src/server/`

`createStorageHttpHandler` (`server/index.ts:728`) composes three handlers and
routes purely on pathname prefix (`server/index.ts:769-790`): `/documents*` →
...
`/kv/entries/:key` + `/kv/keys` contract (`kv/http.ts:306,351,357,364`) and
`docs/kv-http-contract.md` specifies it, but `src/server/` contains only
