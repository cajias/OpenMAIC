...
Companion to [`01a-modules-package.md`](docs/appendix/research/persistence-storage-state/01a-modules-package.md). This file covers the app's wiring of the
...
`isBrowserPersistenceEnabled()` ([`bootstrap.ts:15`](lib/persistence/bootstrap.ts#L15)) requires both
...
against `baseUrl: '/api/persistence'` ([`bootstrap.ts:45-61`](lib/persistence/bootstrap.ts#L45-L61)) and configures both
seams. The two `assert*Configurable()` preflight calls run before either
`configure*` so a partial configuration is impossible
([`bootstrap.ts:62-68`](lib/persistence/bootstrap.ts#L62-L68): "All checks are mutation-free"). A failure logs
...
([`bootstrap.ts:31-39`](lib/persistence/bootstrap.ts#L31-L39)).

## `lib/document-store/` (10 files, 1354 lines)

- `config.ts` — single-shot, client-bootstrap-only configuration.
  `assertDocumentStorageConfigurable()` ([`config.ts:69`](lib/document-store/config.ts#L69)) throws once resolution
  has started *or* once options exist. Configuration stays sealed even if
  resolution failed ([`config.ts:70-78`](lib/document-store/config.ts#L70-L78)). Factories receive
  `{ validateScene, validateStage }` so a server-backed store keeps the app's
  widened write boundary.
- `store.ts` — `getDocumentStore()` ([`store.ts:63`](lib/document-store/store.ts#L63)). Uses `defaultStore ??= (…)()`
  so a throwing factory is retried rather than cached ([`store.ts:66-71`](lib/document-store/store.ts#L66-L71)). Browser
  ...
  ([`store.ts:47-50`](lib/document-store/store.ts#L47-L50)).
...
- `migration.ts` (701 lines) — the Dexie → `DocumentStore` migration; see
  [`03-flows.md`](docs/appendix/research/persistence-storage-state/03-flows.md).
- `storage-generation.ts` — a monotonic `device`-scoped counter
  ...
  that would resurrect wiped data ([`storage-generation.ts:21-26`](lib/document-store/storage-generation.ts#L21-L26)).
- `current-scene.ts`, `canonicalize.ts`, `persistence-types.ts`.

`AppStage = Stage` and `AppDocument = MaicDocument<AppScene, AppStage>`
([`persistence-types.ts:8,56`](lib/document-store/persistence-types.ts#L8)). `AppDocumentOutline` ([`persistence-types.ts:29`](lib/document-store/persistence-types.ts#L29))
...
([`persistence-types.ts:10-25`](lib/document-store/persistence-types.ts#L10-L25)).

## `lib/persistence/` (13 files, 1783 lines)

[`server-provider.ts:36-67`](lib/persistence/server-provider.ts#L36-L67) is the one place the server touches PostgreSQL. It
...
([`server-provider.ts:30-34`](lib/persistence/server-provider.ts#L30-L34)) so dev-time module reloads reuse one pool, and a
failed init clears the cache so the next request retries
([`server-provider.ts:79-88`](lib/persistence/server-provider.ts#L79-L88)).

[`owner-bound-document-store.ts:173`](lib/persistence/owner-bound-document-store.ts#L173) builds a `WithTransaction` that, *inside every
transaction*, takes `SELECT … FROM stage_meta WHERE stage_id = $1 FOR SHARE`
(reads) or `FOR UPDATE` (writes) and enforces four refusals
([`owner-bound-document-store.ts:186-212`](lib/persistence/owner-bound-document-store.ts#L186-L212)): `foreign`, `tombstoned`,
...
`stage-meta.ts` owns the app's own table (`STAGE_META_SCHEMA`, [`stage-meta.ts:24`](lib/persistence/stage-meta.ts#L24))
and its backfill `INSERT … SELECT id, owner_id FROM document_stages … ON CONFLICT DO NOTHING`
([`stage-meta.ts:43-47`](lib/persistence/stage-meta.ts#L43-L47)). `StageAccessError extends DocumentNotFoundError`
([`stage-meta.ts:92`](lib/persistence/stage-meta.ts#L92)) so the storage layer's 404 mapping applies unchanged.

[`document-access.ts:27`](lib/persistence/document-access.ts#L27) re-parses the document path itself (`parseDocumentAction`)
...
([`server-auth.ts:1-13`](lib/persistence/server-auth.ts#L1-L13)). All assets go to one `'shared'` principal
([`server-auth.ts:26,53`](lib/persistence/server-auth.ts#L26)).

[`asset-byte-store.ts:100`](lib/persistence/asset-byte-store.ts#L100) (`lazyAssetByteStore`) defers byte-layer construction to
...
[`asset-collector-schedule.ts:88`](lib/persistence/asset-collector-schedule.ts#L88) schedules `AssetCollector.collect()` once per
...
([`kv-persist.ts:16-33`](lib/store/kv-persist.ts#L16-L33)).
...
([`kv-persist.ts:135-159`](lib/store/kv-persist.ts#L135-L159)). Two rows carry the weight: `clearing` is entered
...
([`settings.ts:81-400`](lib/store/settings.ts#L81-L400) is the `SettingsState` interface). Plus UI layout prefs
...
`version: SETTINGS_PERSIST_VERSION = 4` ([`settings.ts:48,1984-1996`](lib/store/settings.ts#L48)). A four-step
...
rehydrate through the custom `merge` ([`settings.ts:2213-2235`](lib/store/settings.ts#L2213-L2235)) "so newly added
providers/models appear without clearing cache".

`purgeLegacyPersistKey('settings-storage')` at [`settings.ts:2248`](lib/store/settings.ts#L2248) best-effort
...
as invariants ([`session-store.ts:13-22`](lib/workbench/session-store.ts#L13-L22)): the rendered UI is a pure function of
...
one-time migration source ([`chat-storage.ts:1-7`](lib/utils/chat-storage.ts#L1-L7)). Public API:
...
steps per attempt, 500 ms max backoff (`:47-49`). Details in [`03-flows.md`](docs/appendix/research/persistence-storage-state/03-flows.md).

## i18n — `lib/i18n/`

`supportedLocales` ([`locales.ts:16-29`](lib/i18n/locales.ts#L16-L29)) is 12 entries: `zh-CN`, `zh-TW`, `en-US`,
`ja-JP`, `ru-RU`, `ar-SA`, `pt-BR`, `ko-KR`, `es-MX`, `fr-FR`, `vi-VN`, `de-DE`.
`defaultLocale = 'zh-CN'` and is also `fallbackLng` ([`types.ts:5`](lib/i18n/types.ts#L5),
[`config.ts:38-39`](lib/i18n/config.ts#L38-L39)).
...
- `lib/i18n/workbench-locales/<code>.json` — 10 files, 289 lines each. English and
  Chinese workbench copy live in TypeScript ([`lib/i18n/workbench.ts:29`](lib/i18n/workbench.ts#L29)
  ...
  language from" ([`workbench.ts:1-17`](lib/i18n/workbench.ts#L1-L17)).

[`config.ts:21-35`](lib/i18n/config.ts#L21-L35) deep-merges `workbenchResourceFor(language)` under the
...
a string ([`lib/i18n/workbench.ts:723-729`](lib/i18n/workbench.ts#L723-L729)), and
...
([`lib/workbench/session-store.ts:26`](lib/workbench/session-store.ts#L26)).

### The `check:i18n-keys` contract

[`package.json:19`](package.json#L19) → `node scripts/check-i18n-keys.mjs`. The script
...
([`lib/i18n/workbench.ts:12-16`](lib/i18n/workbench.ts#L12-L16)). It also does not check that a key is *used*, nor
that a value is translated rather than copied English.

## `lib/brand/` and `lib/contexts/`

[`lib/brand/brand-config.ts:27`](lib/brand/brand-config.ts#L27) is a static `DEFAULT_BRAND`
...
([`brand-config.ts:1-9`](lib/brand/brand-config.ts#L1-L9)). [`brand-context.tsx:22`](lib/brand/brand-context.tsx#L22) defaults the context so hooks work
