...
| --- | --- | --- |
| `StreamBuffer` tick | 30 ms, 1 char/tick (≈33 chars/s) | `stream-buffer.ts:208` |
| `EFFECT_AUTO_CLEAR_MS` | 5 000 | `choreography/timing.ts:18` |
| `DISCUSSION_TRIGGER_DELAY_MS` | 3 000 | `timing.ts:21` |
| `DISCUSSION_AUTO_SKIP_MS` | 5 000 | `timing.ts:31` |
| `MAX_VIDEO_WAIT_MS` | 300 000 | `timing.ts:34` |
| whiteboard dwells | open 2 000, draw 800, edit 600, delete 300, close 700, widget 300 | `timing.ts:43-58` |
| `wbDrawCodeMs` | `min(800 + 50·lines, 3000)` | `timing.ts:64` |
| `wbClearMs` | `min(380 + 55·elements, 1400)` | `timing.ts:72` |
| speech estimate | CJK >30 % → 150 ms/char, else 240 ms/word, floor 2 000 ms, ÷ speed | `timing.ts:113` |
| cursor-save debounce | 1 000 ms | `PlaybackChromeRoot.tsx:335` |
| auto-play scene gap | 1 500 ms | `PlaybackChromeRoot.tsx:920` |
| presentation controls idle hide | 3 000 ms | `PlaybackChromeRoot.tsx:571` |
| end flash | 1 800 ms (three separate literals) | `PlaybackChromeRoot.tsx:463`, `:852`; `roundtable/index.tsx:344` |
| local user bubble | 3 000 ms | `roundtable/index.tsx:303` |
| media hydration | 4 records per idle slice, 1 000 ms idle timeout | `load-classroom.ts:494` |
| pane availability retries | 1 s, 2 s, 4 s, 8 s, 16 s | `progressive-load-policy.ts:2` |
| `IFRAME_POOL_CAP` | 3 | `interactive-iframe-pool.ts:21` |
| GenUI logical viewport | 1280 × 720 | `logical-viewport.ts:4` |
| `MAX_INSTRUCTOR_STEPS` / `MAX_HISTORY_MESSAGES` | 7 / 24 | `instructor.ts:61` |
| `MAX_ENGAGEMENT_EVENTS` | 500 | `@openmaic/generation/src/pbl/operations/kernel/engagement.ts:28` |
| `TASK_EVAL_PASS_SCORE` | 60 | `…/kernel/task-completion.ts:18` |
| PBL SSE heartbeat | 15 000 ms | `pbl/v2/api/sse.ts:215` |
| `PBL_DRAIN_TIMEOUT_MS` / chain cap | 10 000 / 20 000 | `runtime/drain.ts:29` |
| route `maxDuration` | 300 s (instructor, open-task, evaluate, simulator), 60 s (task/update) | the five route files |
| proficiency tiering | buckets at ±0.33, hysteresis ±0.20, confidence gate 0.4, 50-signal history | `lib/pbl/v2/types.ts:400-428` |
...
1. **Cancellation is designed, not patched.** Four independent epoch/generation
   ...
   loop (`engine.ts:198`), which is the hard case.
2. **The choreography extraction is real.** Not a "shared constants" file — a
   ...
   (`eslint.config.mjs:255-323`) that a Node-only exporter can interpret. The
   `clampFireAndForgetLifetimes` function (`timeline.ts:368`) reproduces a
...
| --- | --- | --- | --- |
| 1 | `app/classroom/[id]/page.tsx` duplicates the whole load body instead of using `ClassroomSurface`, and the copies have **already diverged**: the route lacks `notFound`, `resetCanvasState()`, the `outlineProducer === 'server-job'` guard and `shouldResumeClassroomGeneration` | `page.tsx:28-221` vs `ClassroomSurface.tsx:62-317`; `ClassroomSurface.tsx:4` claims to be the single copy | high |
| 2 | Animation descriptors are hand-transcribed from the React overlays with nothing enforcing agreement; a tweak to `SpotlightOverlay.tsx` silently desyncs every exported video | `descriptors/spotlight.ts:8`; only consumers are `video-export/passes/timeline.ts:67` and a schema test | medium |
| 3 | `PlaybackChromeRoot.tsx` is a 1 848-line component with **28** `useState`, **22** `useRef`, 11 `useEffect` and 31 `useCallback` occurrences (`grep -c` each), holding the engine, the audio player, the TTS hook, the chat ref and every keyboard handler. Its scene-init effect is ~300 lines and carries an `exhaustive-deps` disable | `PlaybackChromeRoot.tsx:655-961`, `:960` | medium |
...
| 5 | `bubbleRole` / `activeRole` are derived twice — once purely in `computePlaybackView`, once again inline in the roundtable with `?? <fallback>` chains, then re-published as `enrichedPlaybackView` | `derived-state.ts:146-182` vs `roundtable/index.tsx:542-608` | medium |
...
| 7 | Two SSE readers with different error tolerance: `runOneStream` aborts on any `error` frame, `submission.tsx`'s inline reader tolerates `EMPTY_LLM_OUTPUT` — and the helper that decides lives in the *other* file | `use-instructor-stream.ts:337` vs `submission.tsx:492` | medium |
| 8 | Three permanent `requestAnimationFrame` polls run whenever their component is mounted: the interactive slot rect (`interactive-renderer.tsx:56`), the PBL docked frame while not expanded (`pbl-renderer.tsx:363`), and the `ProactiveCard` anchor (`proactive-card.tsx:85`). Two are dirty-checked; the interactive one calls `setRect` on every frame and relies on the store's own equality check | cited files | medium |
| 9 | `task/update` failures are swallowed: `if (!res.ok) return;` with an empty `catch` in both `runSceneAction` and `handleCompleteTask` — the learner clicks Done, nothing happens, no message | `workspace.tsx:153`, `:239`, `:262` | medium |
| 10 | Progress on a **legacy-upgraded** PBL project is rendered but never persisted, because `preparePBLScenesForDocumentPersistence` skips non-`v2` scenes | `document-persistence.ts:22` | medium |
| 11 | Unknown action types are skipped with no log and no counter, so a document produced by a newer DSL degrades invisibly | `engine.ts:743` | low |
| 12 | The `1800 ms` end-flash duration is written out three times in two files rather than shared | `PlaybackChromeRoot.tsx:463`, `:852`; `roundtable/index.tsx:344` |
| 13 | `lib/live/server-api.ts` has nothing to do with live playback (it renames courses) — a directory name that will mislead every new reader | `lib/live/server-api.ts:33` | low |
...
| 15 | `instructor.ts`'s own header says "the three teaching tools" while listing (and implementing) two — stale prose on a 1 864-line file | `instructor.ts:7` vs `:1422` | low |
| 16 | `CJK_LANG_THRESHOLD = 0.3` is declared in `engine.ts:60` and `CJK_RATIO_THRESHOLD = 0.3` in `timing.ts:86`, with different CJK character classes, for two related purposes | both files | low |
