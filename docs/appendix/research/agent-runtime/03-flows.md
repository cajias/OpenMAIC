...
| # | Hop | Where |
| --- | --- | --- |
| 1 | `POST /api/agent/sessions` handler | `app/api/agent/sessions/route.ts:37` |
| 2 | `isAgentRuntimeConfigured()` — 404 if off | `lib/config/feature-flags.ts:23` |
| 3 | validate `existingCourse`/`stageId`/`prompt` length/`materialIds`/`courseRefs` | `route.ts:49-90` |
| 4 | `withRequestOwnerId(req, handler)` → `resolveRequestOwnerId` mints `anon:<uuid>` | `with-owner.ts:12`, `owner.ts:52` |
| 5 | explicit `skill`: `findSkill(ref, ownerId)`; else `inferSkillIdFromPrompt(prompt, ownerId)` | `skills.ts:291`, `:369` |
...
| 10 | Browser attaches `EventSource /api/agent/sessions/:id/events` | `lib/workbench/use-workbench-session.ts:153` |
...
| 16 | `loadEntryHistory()` → `AgentSessionEntryStorage.open` → `loadSessionEntryHistory` | `runner.ts:943`, `entry-tree-storage.ts:169`, `:43` |
| 17 | `planResume(historyMessages)` → `{kind:'start'}` for a fresh session | `resume.ts:93` |
| 18 | `repairOrphanedToolCalls(plannedMessages)` (no-op here) | `tool-call-integrity.ts:109` |
| 19 | `listSkills(meta.ownerId)`; `findSkill(meta.skillId, ownerId)`; `skillReadFromTranscript` | `skills.ts:211`, `:291`, `:529` |
| 20 | `createNativeSkillReadTool(installedSkills, onActivate)` when skills exist | `skills.ts:592` |
...
| 23 | `resolveAgentDriverModel()` → `createCallLlmStreamFn({languageModel, wireMaxOutputTokens, thinkingConfig, abortSignal})` | `agent-driver-model.ts:83`, `stream-fn.ts:250` |
| 24 | `buildAskUserTool({onUserQuestion})`; `resolveWebSearchCapability()` | `ask-user.ts:45`, `web-search.ts:24` |
...
| 29 | `assembleRunnerTools(...)` (a flat concat) | `runner-contract.ts:7` |
| 30 | `buildAgent({streamFn, systemPrompt: buildRunnerCoursePrompt(blocks), model: driver.piModel, tools, allowedToolNames, history?, afterToolCall})` | `build-agent.ts:65`, `runner-contract.ts:14` |
...
| 33 | `buildSkillPreload({text, skills, transcript, forced?, model, onSkipped})` → `adoptPreload(preload, true)` | `skill-preload.ts:224`, `runner.ts:1196` |
...
| 2 | pi calls `beforeToolCall` = `makeAllowlistGate(allowedToolNames)` | `allowlist.ts:5` |
| 3 | `trackToolCallMessage(inFlightToolCalls, message)` at the assistant `message_end` — the call becomes "pending durable receipt" | `runner.ts:1515`, `tool-call-integrity.ts:203` |
| 4 | wrapper `withAgentToolTimeout` starts the race (10 min default) | `tool-timeout.ts:98` |
| 5 | `withOwnerStageAuthorization` wrapper reads `params.stageId`, calls `deps.stageAccess(stageId)` = `probeStageAccess(ownerId, stageId)` | `course-tools.ts:163-186`, `curriculum-tools.ts:54` |
| 6 | non-`owned` → `isError` result `"The stage was not found, or does not belong to this session user…"`, `details.refused = true`. Flow stops. | `course-tools.ts:174-185` |
| 7 | `patch_stage.execute`: reject blank `intent`; `loadCourse(deps, stageId)` (`dsl-tools.ts:141`); `resolveCoursePath(doc, target)` (`:168`); require `kind === 'scene'` | `dsl-tools.ts:777-790` |
| 8 | `structuredClone(resolved.scene)` then `applyPatchOp` (`dsl-tools.ts:571`) per op; first failure aborts the whole batch with `failedOp` | `dsl-tools.ts:791-810` |
| 9 | `containsReadSceneMediaPlaceholder(JSON.stringify(next))` — final-state guard against a placeholder assembled across ops | `dsl-tools.ts:819-826` |
| 10 | `validationError(next)` (`dsl-tools.ts:431`, wrapping `validateAppScene`) | `dsl-tools.ts:827` |
| 11 | `runStageMutation(signal, () => putSceneBringingCurrent(store, stageId, next))` | `mutation-fence.ts:10`, `document-writes.ts:32` |
...
| 13 | `deps.onCheckpoint({tool:'patch_stage', stageId, sceneId, order, title, sceneType, detail})` | `dsl-tools.ts:846` |
...
| 16 | commit → PG emits `NOTIFY openmaic_agent_event_wakeup` | `event-notify-bus.ts:31` |
...
| 19 | browser `onAny` listener folds the frame into the session store | `lib/workbench/use-workbench-session.ts:243` |
| 20 | `presentTool(...)` builds the collapsed row from the tool's `details` (never its prose) | `components/workbench/chat/tool-presentation.ts:271` |
...
| 5 | `loadEntryHistory()` → `loadSessionEntryHistory` validates the tree and the context↔entry mapping | `entry-tree-storage.ts:43` |
| 6 | `planResume(historyMessages)` pops the incomplete assistant suffix, then returns `{kind:'continue', messages, repairedToolCalls:[danglingIds]}` | `resume.ts:93-157` |
...
| 8 | `repairOrphanedToolCalls(plannedMessages)` materializes a provider-safe **read-time** view: existing results moved next to their assistant, unwind frames dropped, missing results synthesized as `interruptedToolResult` | `tool-call-integrity.ts:109`, `:64` |
...
| 11 | `buildSkillPreload({text: resumedTurnText, skills, transcript: modelMessages, model, onSkipped})` — the transcript dedupe (`readProvesCoverage`) is the idempotence judge | `skill-preload.ts:224`, `skills.ts:515` |
...
The consequence, stated in `resume.ts:33-37`: tool execution is **at-least-once**,
...
| # | Hop | Where |
| --- | --- | --- |
| 1 | `POST /api/agent/sessions/:id/cancel` | `cancel/route.ts:16` |
| 2 | owner check; 409 `SESSION_ALREADY_TERMINAL` if already succeeded/failed/cancelled | `cancel/route.ts:25-37` |
| 3 | `store.requestCancel(id)` — durable only; the route writes no event | `cancel/route.ts:39` |
...
| 7 | `abortAgent` listener → `agent.abort()`; in-flight tool receives the derived signal and rejects with `AgentToolAbortedError` | `runner.ts:1557-1558`, `tool-timeout.ts:158-164` |
