...
- **[`director-loop.ts:29`](lib/chat/pi/director-loop.ts#L29) `runPiDirectorLoop(opts)`** builds a four-tool
  ...
  - `maxAgentTurns` and `maxActionsPerAgent` both default **and cap** at 6 and 8
    respectively ([`lib/chat/pi/config.ts:5-8`](lib/chat/pi/config.ts#L5-L8)) — the request cannot raise them;
  ...
  (`:260-282`).
- **[`director-compaction.ts:106`](lib/chat/pi/director-compaction.ts#L106) `createDirectorCompactionRuntime(opts)`** —
  ...
  anchor.
- **[`tools/call-agent.ts:531`](lib/chat/pi/tools/call-agent.ts#L531) `buildCallAgentTool(opts)`** — 1004-line module,
...
- **[`tools/native-whiteboard.ts:662`](lib/chat/pi/tools/native-whiteboard.ts#L662) `buildNativeWhiteboardTools(opts)`** —
...
- **[`director-graph.ts:484`](lib/orchestration/director-graph.ts#L484) `createOrchestrationGraph()`** — a three-node
  ...
  parsed by `parseDirectorDecision` ([`director-prompt.ts:216`](lib/orchestration/director-prompt.ts#L216)).
  ...
  request.
- **[`stateless-generate.ts:136`](lib/orchestration/stateless-generate.ts#L136) `parseStructuredChunk(chunk, state)`** — the
  ...
  bubble.
- **[`registry/store.ts:207`](lib/orchestration/registry/store.ts#L207) `useAgentRegistry`** — a zustand store with
...
- **[`registry/types.ts:83`](lib/orchestration/registry/types.ts#L83) `ROLE_ACTIONS`** — the role→action map: `teacher` gets
...
- **[`registry/agent-selection.ts:27`](lib/orchestration/registry/agent-selection.ts#L27) `restoreAgentSelection(params)`** — decides
...
- **[`tool-schemas.ts:16`](lib/orchestration/tool-schemas.ts#L16) `getEffectiveActions(allowedActions, sceneType)`** —
...
| Route | File | Notes |
| --- | --- | --- |
| `POST /api/agent/sessions` | [`app/api/agent/sessions/route.ts:37`](app/api/agent/sessions/route.ts#L37) | Validates prompt ≤ `MAX_SESSION_TEXT_LENGTH` (100 000, [`limits.ts:9`](lib/server/agent-runtime/limits.ts#L9)), ≤ 20 `materialIds`, rejects an unknown explicit `skill` with the installed list ([`:100-116`](app/api/agent/sessions/route.ts#L100-L116)), else infers `skillId` from a leading `/handle`. Creates the row as `succeeded` when it has opening context so the runner cannot claim before `postUserMessage` requeues it ([`:148`](app/api/agent/sessions/route.ts#L148)). Returns 202. |
...
| `POST /api/agent/sessions/:id/cancel` | [`cancel/route.ts:16`](app/api/agent/sessions/[id]/cancel/route.ts#L16) | makes the request durable only; the lease holder writes the terminal event ([`:3-6`](app/api/agent/sessions/[id]/cancel/route.ts#L3-L6)). 409 if already terminal. |
| `GET /api/agent/sessions/status` | `status/route.ts:11` | sparse `{id: status}` map |
| `GET /api/agent/owner-events` | [`owner-events/route.ts:39`](app/api/agent/owner-events/route.ts#L39) | owner-wide sparse SSE tail; 30 s poll, 25 s heartbeat |
| `GET /api/agent/runtime` | [`runtime/route.ts:18`](app/api/agent/runtime/route.ts#L18) | `{enabled, runtimeEnabled}` — `enabled` requires the flag **and** `DATABASE_URL` |
| `GET/POST /api/agent/skills` | [`skills/route.ts:26`](app/api/agent/skills/route.ts#L26), [`:47`](app/api/agent/skills/route.ts#L47) | picker list; upload a Skill as exporter zip or bare SKILL.md (≤ 1 MiB compressed) |
...
| `POST /api/chat/pi` | [`app/api/chat/pi/route.ts:42`](app/api/chat/pi/route.ts#L42) | Pi director SSE; 15 s heartbeat, `maxDuration = 300` |
| `POST /api/chat` | [`app/api/chat/route.ts:44`](app/api/chat/route.ts#L44) | LangGraph SSE; 15 s heartbeat, `maxDuration = 60` |
| `POST /api/chat/pi/whiteboard-visibility` | [`whiteboard-visibility/route.ts:31`](app/api/chat/pi/whiteboard-visibility/route.ts#L31) | learner-authenticated reply that settles a pending visibility query |
...
[`components/workbench/chat/tool-presentation.ts:271`](components/workbench/chat/tool-presentation.ts#L271) `presentTool(...)` is a
...
the build (`:37-39`). [`chat-timeline.tsx:59`](components/workbench/chat/chat-timeline.tsx#L59) `groupChat` and [`:136`](components/workbench/chat/chat-timeline.tsx#L136)
