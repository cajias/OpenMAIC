...
1. **Registration.** A tool the deployment cannot serve is *not built*. The
   ...
   `hasConfiguredVideoGeneration(deps)` (`course-tools.ts:214`),
   `render_scene_preview` only when a render service is configured
   (`course-tools.ts:198-202`), the skill-scoped `read` only when skills exist
   (`runner.ts:1190-1194`), and `use_material_media` only with a `sessionId`
   (`course-tools.ts:216`). The stated reason is consistent throughout: "the
...
3. **Owner scoping.** There is exactly **one** owner-bound document store per
   ...
   (`course-tools.ts:159`) wraps every tool that takes a `stageId`, probes it
...
4. **Mutation fencing.** Writers go through `runStageMutation(signal, ...)`
   (`dsl-tools.ts:836`), and the run's store transaction hook asserts both the
...
5. **Sequential scheduling.** `markDocumentWritersSequential`
   (`course-tools.ts:137`) declares every document writer
...
containment is `realpath`-based (`skills.ts:597-605`), with database skills
served from memory and any path starting `/__openmaic_user_skills__` that is not
an exact match hard-rejected (`skills.ts:637-639`).
...
| `import_pptx` (`import-pptx.ts:357`) | ✓ | appends pages to an existing stage from an uploaded `.pptx` material | always |
| `generate_image` (`generate-image.ts:211`) | | returns a `src`; **never edits the page** | always |
| `generate_video` (`generate-video.ts:480`) | | returns a `gen_vid_<id>` placeholder immediately; the job patches the document later through `mediaJobStore` and emits `media_ready` | only when a video provider is configured |
| `use_material_media` (`material-media.ts:21`) | ✓ | copies a session material's bytes into the stage media path | only with a `sessionId` |
| `render_scene_preview` (`scene-preview.ts:46`) | | PNG of a persisted page | only when the render service is configured; has its **own** owner probe, not the generic wrapper (`runner.ts:1377-1386`) |
...
| `fetch_url` (`fetch-url.ts:616`) | ✓ | creates a web material | **always registered**; the security property is the URL trust gate, not registration — only origins already seen in a user message or this session's `web_search` results (`runner.ts:1446-1449`) |
...
| `set_roster` (`roster-tools.ts:234`) | ✓ | the stage's agent roster (exactly one teacher, ≥ 2 agents); member of `STAGE_WRITER_TOOL_NAMES`; owner-gated by an explicit `withOwnerStageAuthorization` wrap (`runner.ts:1402-1410`) | always |
| `clip_audio` (`voice-clone-tools.ts:272`) | ✓ | produces a clip artifact from an audio/video material | always |
| `register_voice` (`voice-clone-tools.ts:330`) | ✓ | registers a cloned voice with the provider; appended to a **run-scoped** in-memory `sessionRegisteredVoices` array (`runner.ts:1398`) — deliberately not persisted, so a cloned voice is bindable only inside the session that registered it | only when a keyed provider reports `supportsRegistration` |

### Skills — `skills.ts`, `create-skill.ts`, `skill-edit-tools.ts`

| Tool | M | Mutates | Registration |
| --- | --- | --- | --- |
| `read` (`skills.ts:607`) | | — | only when skills are installed; path-restricted to installed skill dirs |
| `create_skill` (`create-skill.ts:27`) | ✓ | inserts one `usk_` user skill row | always; create-only, duplicates refused; a successful call is treated as run-terminal on resume (`resume.ts:126-128`) |
| `read_skill` (`skill-edit-tools.ts:265`) | | — | always; `detail: 'source'` is the **only** view of the stored bytes and therefore the precondition for `patch_skill` (`skill-edit-tools.ts:9-15`) |
| `patch_skill` (`skill-edit-tools.ts:305`) | ✓ | `/content`, `/title`, `/description` of one owner skill (`/name` not editable) | always. Every result carries `SCOPE_NOTE`: "Skill text already loaded in the current run does not refresh" (`skill-edit-tools.ts:47`) |
...
| Tool | M | Mutates |
| --- | --- | --- |
| `ask_user` (`ask-user.ts:47`) | | emits the durable `user_question` event and terminates the run |
| `search_classrooms` / `read_classroom` / `search_chats` / `read_chat` (`personal-history-tools.ts:406`, `:440`, `:505`, `:556`) | | — read-only owner history; system prompts, raw tool payloads and material bodies are excluded from `read_chat`; page limit 10 default / 20 max, scan cap 500 (`personal-history-tools.ts:19-21`) |
...
| `call_agent` (`call-agent.ts:574`) | director | drives a child agent | `sequential`; six skip guards (see `01b`) |
...
| `spotlight` (`native-spotlight.ts:78`) | child | emits a slide action | registered only when the feature flag is on **and** `agent.allowedActions` includes `spotlight` **and** the authorised element-id set is non-empty (`call-agent.ts:736-739`); the tool takes an `authorizedElementIds` allow-set |
...
authenticated learner key) — `app/api/chat/pi/route.ts:164-187`.

In the **legacy** child harness the child has no tools at all
(`call-agent.ts:889-890`: `tools: []`, `allowedToolNames: new Set()`); actions
...
`courseSystemPrompt(blocks)` (`course-tools.ts:306`) assembles the runner's
