...
Every claim here rests on a measurement in [`06-quality-and-metrics.md`](docs/appendix/research/quality-testing-ci-deps/06-quality-and-metrics.md) or a
...
3. **The release pipeline's security boundary is a job boundary, not a policy.**
   ...
   ([`publish-packages.yml:78-81`](.github/workflows/publish-packages.yml#L78-L81)); the token-bearing job downloads an immutable
   artefact, re-verifies SHA-256 against a digest anchored in a parent shell
   scalar ([`:372`](.github/workflows/publish-packages.yml#L372)), requires the commit to be on `main`'s first-parent history
   ([`:284-296`](.github/workflows/publish-packages.yml#L284-L296)), polls the Actions API for a green `ci.yml` on the same SHA
   ([`:301-331`](.github/workflows/publish-packages.yml#L301-L331)), and publishes with `--ignore-scripts`. `mark` is the only job
...
4. **Type safety is exceptional at this scale.** 31 `as any`, 23 explicit
   ...
   *negative* type assertions — and [`ci.yml:181-184`](.github/workflows/ci.yml#L181-L184) adds a second `tsc` run over
...
5. **Error handling is documented rather than swallowed.** 128 of 141 code-free
   ...
   path, and [`lib/utils/iframe.ts:3-13`](lib/utils/iframe.ts#L3-L13) writes out why.
6. **Every CI oddity carries its incident.** Sequential test steps
   ([`ci.yml:133-135`](.github/workflows/ci.yml#L133-L135)), skipping `playwright install-deps` ([`:251-254`](.github/workflows/ci.yml#L251-L254)),
   `grep -cx` over `grep -qx` ([`publish-packages.yml:288-291`](.github/workflows/publish-packages.yml#L288-L291)),
   `--no-replace-objects` ([`ci.yml:101-106`](.github/workflows/ci.yml#L101-L106)), conditional
   `cancel-in-progress` ([`:26`](.github/workflows/ci.yml#L26)) — each names the failure it prevents rather than
...
9. **Contract suites for infrastructure the app cannot fake.** Eleven
   ...
   ([`ci.yml:151-178`](.github/workflows/ci.yml#L151-L178)) and a tarball smoke test that installs the exact bytes that
   would be published.
10. **The eval harnesses are honest about their own measurement.**
    [`eval/orchestration/judge.ts:1-8`](eval/orchestration/judge.ts#L1-L8) argues *against* using an LLM judge where
    the verdict is derivable from production parsing code, and [`:30-34`](eval/orchestration/judge.ts#L30-L34) excludes
...
2. **Five contract suites never execute in CI.** Four app-level `*.pg.test.ts`
   ...
   [`park-attempt-budget.pg.test.ts:30`](tests/agent-runtime/park-attempt-budget.pg.test.ts#L30), [`session-events-live.pg.test.ts:51`](tests/agent-runtime/session-events-live.pg.test.ts#L51),
   [`tests/persistence/owner-materials.pg.test.ts:29`](tests/persistence/owner-materials.pg.test.ts#L29)) and one
   ...
   them silently skip.
3. **[`middleware.ts`](middleware.ts) — an authentication boundary — has no test.** HMAC
   verification ([`:18-44`](middleware.ts#L18-L44)) and the `/workbench` 404 gate ([`:56-58`](middleware.ts#L56-L58)) are
   unverified, and the signed timestamp is never compared to now
   ([`:22-43`](middleware.ts#L22-L43)), so a leaked `openmaic_access` cookie never expires. Two of the
   three `access-code` route handlers also have no test.
4. **Three of six storage PG suites are outside `REQUIRED_SUITES`**
   ([`scripts/assert-pg-contract-suites.mjs:61-65`](scripts/assert-pg-contract-suites.mjs#L61-L65)). The audit whose purpose is to
...
5. **The eval harnesses gate nothing.** Five harnesses, 103 scenarios, four with
   ...
   at all.
6. **[`CONTRIBUTING.md`](CONTRIBUTING.md) is factually wrong about the release contract.** Line 139
   says "Four packages … are published to npm: `dsl`, `storage`, `renderer`, and
   `importer`". The real list is six ([`scripts/openmaic-packages.mjs:34`](scripts/openmaic-packages.mjs#L34)),
   ...
   CI failure the same paragraph documents. Line 178 also describes `packages/`
   as containing only the two vendored forks.
7. **[`CONTRIBUTING.md`](CONTRIBUTING.md)'s pre-PR checklist omits `pnpm test`.** Lines 84-91 list
...
8. **`e2e/` is neither type-checked nor linted.** Excluded from
   [`tsconfig.json:39`](tsconfig.json#L39) and [`eslint.config.mjs:53`](eslint.config.mjs#L53), with no `e2e/tsconfig.json` and
...
12. **`NEXT_PUBLIC_PRO_WORKBENCH_ENABLED` is missing from the Docker build args.**
    Ten of eleven public flags are plumbed through [`Dockerfile:54-72`](Dockerfile#L54-L72) and
    [`docker-compose.yml:14-21`](docker-compose.yml#L14-L21); this one is not. Because it is inlined by
...
20. **[`components/ai-elements/web-preview.tsx:159`](components/ai-elements/web-preview.tsx#L159)** carries
...
| Fix | Effort | Removes |
| --- | --- | --- |
| Correct [`CONTRIBUTING.md:139`](CONTRIBUTING.md#changing-a-published-package) to six packages and add `pnpm test` to the pre-PR list | minutes | The most likely first-contributor CI failure, on a gate the document itself explains |
