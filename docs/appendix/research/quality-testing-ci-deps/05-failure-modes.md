...
- `scripts/check-package-version-bumps.mjs:506-514` — any non-definitive registry
...
- `scripts/check-package-version-bumps.mjs:303-306` — if the dsl serialized-format
...
- `ci.yml:69-79` — a push whose `github.event.before` is unusable (branch
  ...
  green".
- `tests/lint-llm-entry-guard.test.ts:67` — an ESLint-ignored path returns the
...
| `eval/pbl-v2-planner/runner.ts` | every case `ok && gate && completability.pass` | any case failing | — | 2 on unhandled rejection (`runner.ts:924`) |
...
| --- | --- | --- |
| Four app-level `*.pg.test.ts` never see a database | `grep -rn 'STORAGE_PG_CONTRACT_REQUIRED' tests/` returns one file; `storage-pg-contract.yml:67` invokes one file explicitly | `LISTEN/NOTIFY` delivery, attempt-budget takeover fencing, live SSE tails and owner-quota reservation locking are unverified on every merge |
...
| Three storage PG suites run but are not in `REQUIRED_SUITES` | `scripts/assert-pg-contract-suites.mjs:61-65` | Excluding them from storage's `vitest.config.ts` — an ignored publishable input, so no version bump needed — would leave the job green |
...
| `middleware.ts` has no test | `grep -rln 'middleware\|access-code\|openmaic_access\|ACCESS_CODE' tests e2e` → 3 unrelated files | The HMAC verification (`middleware.ts:18-44`) and the `/workbench` 404 gate (`:56-58`) are unverified. The signed timestamp is never checked for age, so a leaked cookie is valid forever — and nothing would fail if the verification were weakened |
...
1. **Publish pack/test filesystem race** — `publish-packages.yml:227-233`. The
...
2. **Caret dedup within one 0.x line only** —
   `scripts/check-internal-dependency-ranges.mjs:36-42` and
   `scripts/smoke-test-package-tarballs.mjs:52-57`. A consumer mixing a dependent
...
3. **"Publishable input" == "file under the package directory"** —
   `scripts/check-package-version-bumps.mjs:16-30`. An under-approximation for
...
`lib/video-export/emit-hyperframes/index.ts:395` (1) and
`render-service/src/preview-renderer.ts:128` (1). The reason is written out at
`lib/utils/iframe.ts:3-13`: the interactive iframe is sandboxed `allow-scripts`
...
`components/edit/surfaces/slide/ImagePicker.tsx:31`,
`lib/pbl/v2/runtime/drain.ts:441`,
`lib/server/material-extraction/runner.ts:74`, and
`packages/@openmaic/importer/src/import-pipeline/transformParsedToSlides.ts:749,857`.
`lib/server/material-extraction/runner.ts:74` is the one on a server path and
...
**Tier 1** (`lib/server/agent-runtime/fetch-url.ts:159-174`) is
...
is re-normalised (`:479`). The file header (lines 6-12) states the invariant
explicitly.

**Tier 2** (`lib/server/ssrf-guard.ts:253-302`) is validate-then-fetch: it
...
`app/api/provider/probe-models/route.ts:34`.

`ALLOW_LOCAL_NETWORKS=true|1` short-circuits tier 2 entirely
(`lib/server/ssrf-guard.ts:266-269`) with the reason in the message text at
...
(`lib/server/ssrf-guard.ts:178-244`): IPv4-mapped IPv6, 6to4 (`2002::/16`)
...
| --- | --- | --- | --- |
| Playwright specs | `retries: 2` in CI, `0` locally | `playwright.config.ts:8` | — |
| Playwright artefacts | `trace: 'on-first-retry'`, `screenshot: 'only-on-failure'` | `:19-20` | — |
| Chromium download | 3 attempts, each `timeout -k 10 240`, then `::error::` | `ci.yml:255-266` | "A cold download … has sat for 20+ minutes on this runner fleet" |
| `playwright install-deps` | **never run** | `ci.yml:251-254` | The Ubuntu apt mirror "has sat through the whole 15-minute job budget"; `ubuntu-latest` already ships the libraries |
| Root Vitest vs package Vitest | forced sequential | `ci.yml:133-135` | Running them together made 5 s-timeout tests flake — "CPU contention, not a product regression" |
| Playwright workers | 2 in CI | `playwright.config.ts:12` | "Two workers fit a 4-core runner" |
| `check` job concurrency | `cancel-in-progress` only on PRs | `ci.yml:26` | A cancelled `main` run takes its push range with it |
| CI polling in publish | 30 s interval, 1 800 s deadline | `publish-packages.yml:311-331` | `ci.yml` "blocks nothing" on a push to main |
| Node smoke server readiness | 50 polls × 100 ms, then dump the log | `ci.yml:163-172` | — |

Two flakes are named and explained rather than shrugged off (`ci.yml:133-135`,
`ci.yml:251-254`), which matches the repository's stated standard. No suite is
...
`eval/whiteboard-layout/types.ts:53-54`), and a `--rescore <run-dir>` mode
(`runner.ts:294-337`) re-scores existing PNGs without re-running the chat, keeping
the old score on a second failure (`:317-321`).
...
`decision: 'ERROR'` (`eval/orchestration/runner.ts:100`) and `endRate` excludes
them from the denominator (`eval/orchestration/judge.ts:31-34`), because
...
(`eval/whiteboard-layout/scorer.ts:104-113`) and validates all five dimensions
plus `overall` before returning (`:116-131`), so a partially hallucinated score
...
- `render-service` **boots without `NET_ADMIN`** but "logs a warning and does NOT
  block Chromium egress" (`docker-compose.yml:85-86`). This is fail-open on a
  security control, deliberately, so the service is usable where the capability
  cannot be granted.
- `render-service/Dockerfile:70-76` normalises CRLF out of
...
- `docker-compose.yml:27-32` sets `RENDER_SERVICE_URL` unconditionally, which
