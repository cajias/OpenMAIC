...
| --- | --- | --- | --- |
| Missing `text`/`audioId`/`ttsProviderId`/`ttsVoice` | `MISSING_REQUIRED_FIELD` | 400 | `app/api/generate/tts/route.ts:56` |
| `browser-native-tts` on the server | `INVALID_REQUEST` | 400 | route `:65`; the library also throws (`tts-providers.ts:246`) |
| Operator force-disabled the provider | `PROVIDER_DISABLED` | 403 | route `:71` |
| VoxCPM auto-voice with no prompt and no registered voice | `VOXCPM_AUTO_VOICE_REQUIRES_CONTEXT` | 400 | route `:81`; library mirror at `tts-providers.ts:384` |
| Client base URL fails SSRF | `INVALID_URL` | 403 | route `:97` |
| Keyed provider, no resolvable key | `MISSING_API_KEY` | 400 | route `:113` |
| Client model outside a managed pin list | `TTSModelNotAllowedError` (`code = 'INVALID_REQUEST'`) | 400 | `lib/server/provider-config.ts:789`, mapped at route `:176` |
| Upstream HTTP 429 | `TTSRateLimitError` | 429 `RATE_LIMITED` | `tts-providers.ts:193`, route `:167` |
| Doubao concurrency codes `45000000` / `45000292` | `TTSRateLimitError` | 429 | `tts-providers.ts:1078` |
| Provider did not respond within `TTS_REQUEST_TIMEOUT_MS` | `TTSRequestTimeoutError` | 500 `GENERATION_FAILED` | `tts-providers.ts:261` |
...
| Ordinary Qwen TTS failure | `QwenTTSError` (`code = 'QWEN_TTS_ERROR'`) | provider status | `tts-providers.ts:134`, route `:173` |
| Malformed Doubao `appId:accessKey` (empty half) | plain `Error` with a specific message | 500 | `tts-providers.ts:1021` |
| Caller aborted | the original abort error, re-thrown verbatim | — | `tts-providers.ts:260` |
...
- Rate limiting and timeout are **distinguished** from generic failures, and the
  ...
  `tts-providers.ts:182`). That distinction is what lets a session cancel abort
...
Client-side voice fallback (`lib/hooks/use-scene-generator.ts:393-453`):
...
`MAX_NARRATOR_VOICE_FALLBACK_HOPS = 1` (`use-scene-generator.ts:260`) caps this
...
Anchors: `TRANSIENT_4XX_STATUSES` `lib/media/proxy-media-cache.ts:118`;
...
`generateSingleMedia` (`lib/media/media-orchestrator.ts:229-265`):
...
Only `VideoTimelineCompileError` (`lib/video-export/ir.ts:424`) is thrown, and
only for "no scenes" (`passes/normalize.ts:94`). Everything else degrades:
...
(`lib/choreography/timeline.ts:161`): choreography's default policy for an
...
it to `'cap'` (`passes/probe.ts:41`) because it "prefers to degrade with a
...
real clip length (`passes/timeline.ts:237-244`).
...
`try/catch` and pushed to `missing` (`lib/video-export-app/collect.ts:386-388`).
`missing.length` surfaces to the user only as a warning toast
(`use-export-video.ts:60-64`, `lib/store/video-render.ts:240`).
...
| `decodeFirstFramePosterUrl` | `FIRST_FRAME_TIMEOUT_MS = 8000` | resolves `null`; also catches CORS-tainted canvas (`collect.ts:107`) |
...
(`package-zip.ts:37`, `:43`), failing the whole export. That is correct — a ZIP
...
- **Every failure path releases the reservation.** `main.ts:322-330` calls
...
Preview failures map to distinct statuses (`main.ts:399-412`): 413 too large,
...
1. **Egress lockdown.** `RENDER_EGRESS_LOCKDOWN=true` (the default) with no root,
   ...
   (`docker-entrypoint.sh:16-21`, `:51-63`).
2. **Resource-profile validation.** `validateResourceProfileStartup`
   (`resource-profile.ts:138`) throws below the memory floor, and throws when a
...
3. **Manifest validation.** `emitManifest` (`passes/emit.ts:28`) re-parses the IR
...
it returns `false` on any error (`browser-projection.ts:42-44`), so a projection
