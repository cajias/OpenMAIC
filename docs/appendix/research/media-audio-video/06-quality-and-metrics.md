...
| `TODO`/`FIXME`/`HACK`/`XXX` in scope | 1 ([`lib/audio/tts-providers.ts:120`](lib/audio/tts-providers.ts#L120)) | `rtk grep -c "TODO\|FIXME\|HACK\|XXX" <scope>` |
...
| IR diagnostic codes | 13 | [`lib/video-export/ir.ts:80-94`](lib/video-export/ir.ts#L80-L94) enum members |
| Compiler passes | 8 named + 1 inline (`markUnsupported`) | [`lib/video-export/compile.ts:152-202`](lib/video-export/compile.ts#L152-L202) |
| TTS providers / ASR providers | 10 / 6 built-in | [`lib/audio/types.ts:82-92`](lib/audio/types.ts#L82-L92), [`:179-185`](lib/audio/types.ts#L179-L185) |
| Image / video / web-search providers | 8 / 6 / 9 | [`lib/media/types.ts:73`](lib/media/types.ts#L73), [`:194`](lib/media/types.ts#L194); [`lib/web-search/types.ts:8`](lib/web-search/types.ts#L8) |
...
[`eslint.config.mjs:348-492`](eslint.config.mjs#L348-L492) blocks `@/…` string literals *and* template literals,
...
([`tests/lint-llm-entry-guard.test.ts:53`](tests/lint-llm-entry-guard.test.ts#L53)). This is the difference between a
...
DSL's `FIRE_AND_FORGET_ACTIONS` rather than hardcoding it ([`timeline.ts:51`](lib/choreography/timeline.ts#L51)).

**`clampFireAndForgetLifetimes` is unusually careful.**
[`lib/choreography/timeline.ts:368`](lib/choreography/timeline.ts#L368) models three real engine behaviours that a
...
([`lib/video-export/ir.ts:61-94`](lib/video-export/ir.ts#L61-L94)). `AssetPlanEntry.present: false` keeps a
...
`present: false`) without parsing message strings ([`passes/assets.ts:204-208`](lib/video-export/passes/assets.ts#L204-L208)).
...
enforce stage-wide action-id uniqueness ([`compile.ts:126-135`](lib/video-export/compile.ts#L126-L135)).
`mediaRefBySceneElement` is keyed by scene because element ids are only
slide-unique ([`timeline-deps.ts:335-341`](lib/video-export-app/timeline-deps.ts#L335-L341)). `AssetPlanner.owners` is keyed by
`(kind, assetId)` because one ref may legitimately be both narration audio and
video media ([`passes/assets.ts:59`](lib/video-export/passes/assets.ts#L59)). Each of these is a bug class that was found
...
unwrapping ([`lib/server/ssrf-guard.ts:217-241`](lib/server/ssrf-guard.ts#L217-L241)) plus IPv4-mapped IPv6 and cloud
...
`..` ([`comfyui-image-adapter.ts:171-178`](lib/media/adapters/comfyui-image-adapter.ts#L171-L178)). And the workflow-name predicate is
shared by the API route and the adapter so "a filename the UI offers is always a
filename the adapter will accept" ([`comfyui-workflows.ts:5-9`](lib/media/comfyui-workflows.ts#L5-L9)).

**The render container fails closed.** [`docker-entrypoint.sh:16-21`](render-service/docker-entrypoint.sh#L16-L21) names the
...
app. Combined with [`resource-profile.ts:75`](render-service/src/resource-profile.ts#L75) refusing conflicting overrides rather
than honouring them, the operational posture is "refuse to run wrong" rather than
"warn and continue".

**Admission before buffering.** [`main.ts:216-228`](render-service/src/main.ts#L216-L228) states the ordering as the
...
[`components/whiteboard/index.tsx:81`](components/whiteboard/index.tsx#L81) computes `Math.min(380 + elementCount * 55,
...
([`types.ts:29-49`](lib/audio/types.ts#L29-L49)), which is honest but is still four opportunities to get out of
...
`GENERATION_FAILED` 500 ([`app/api/generate/tts/route.ts:179`](app/api/generate/tts/route.ts#L179)) alongside every
...
every error and returning `false` ([`browser-projection.ts:42`](lib/whiteboard/runtime/browser-projection.ts#L42)) means a genuine
storage fault is indistinguishable from "nothing to project", and
[`quiz-layout.ts:212`](lib/video-export-app/quiz-layout.ts#L212) / [`timeline-deps.ts:491`](lib/video-export-app/timeline-deps.ts#L491) silently degrade measurement with no
diagnostic reaching the manifest — unlike every *compiler* degradation, which
does.

**`Math.random()` in the ComfyUI seed patch.** [`comfyui-image-adapter.ts:391`](lib/media/adapters/comfyui-image-adapter.ts#L391)
...
**`decodeFirstFramePosterUrl` returns a data URL.** [`collect.ts:106`](lib/video-export-app/collect.ts#L106) uses
...
[`lib/store/video-render.ts:39-46`](lib/store/video-render.ts#L39-L46) documents that the render is not uniform (prep →
...
**`MAX_POLL_ATTEMPTS` allows a one-hour client poll** ([`video-render.ts:36`](lib/store/video-render.ts#L36):
...
[`chunk-executor.ts:621-640`](render-service/src/chunk-executor.ts#L621-L640) and [`:816-871`](render-service/src/chunk-executor.ts#L816-L871) save, overwrite and restore
...
- `tests/video-export/eslint-boundary.test.ts` runs ESLint programmatically over
  ...
  at `../../quiz/math-text` (`:17-27`, matching [`eslint.config.mjs:493-499`](eslint.config.mjs#L493-L499)).
...
- [`tests/security/iframe-sandbox.test.ts:52`](tests/security/iframe-sandbox.test.ts#L52) extracts sandbox attribute values
