...
| --- | --- | --- | --- |
| 1 | `acquireExport()` | `lib/video-export-app/export-in-flight.ts:24` | Module-singleton mutex shared with the subtitles-only download. A per-hook ref would reset when the dialog unmounts, so this is deliberately module-level. |
| 2 | `await import('./build-export-zip')` | `use-export-video.ts:49` | Dynamic import keeps the compiler, emitter and JSZip out of the main bundle; asserted by `tests/video-export/export-loading-boundary.test.ts`. |
| 3 | `getVideoExportCoverLabels(locale)` + `configuredVideoExportCta()` | `build-export-zip.ts:147-148` | Chrome is pinned **before the first `await`**, so a user switching UI language mid-compile cannot produce a mixed-locale export. |
| 4 | `compileStageIr({ resolution, locale, labels })` | `build-export-zip.ts:74` | Reads `useStageStore.getState()`; zero scenes → `NoScenesError`. Resolves the display name from `accessDocument(stage.id)`. |
| 5 | `createVideoTimelineDeps({ stage, scenes })` ∥ `createQuizLayoutProbe({...})` | `timeline-deps.ts:205`, `quiz-layout.ts` | Run concurrently via `Promise.all` (`build-export-zip.ts:95`). |
...
| 5b | `enumerateAssetManifest({ stage, scenes })` | `@openmaic/dsl` via `timeline-deps.ts:233` | The manifest gates which refs are loaded at all — an orphan Dexie row is not even read. |
| 5c | Audio load | `timeline-deps.ts:258-302` | Per referenced `audioId`: `db.audioFiles.get(id)` for metadata + `resolveAudioBlob(id)` for bytes (pool-first). Unconverted documents' legacy `audioUrl`s are fetched only when the id produced no bytes. |
...
| 6 | `compileVideoTimeline({stage, scenes}, {timing, assets, geometry, interactive, quizLayout})` | `lib/video-export/compile.ts:152` | Nine pure passes → `VideoTimeline`. |
| 7 | `emitHyperframes(ir, { width, height, burnInSubtitles, labels, locale, cta })` | `emit-hyperframes/index.ts:1229` | Text files + `vendorAssets` descriptors. |
| 8 | `collectVideoAssets(ir, scenes, deps.records, { frameWidth: width })` | `collect.ts:326` | Iterates only owning plan entries (`present && !dedupOf`, `:341`). |
...
| 9 | `packageVideoZip(project, blobs)` | `package-zip.ts:53` | Text files at the root; blobs at `assets/<planPath>` (via `assetUrl`); vendor fonts at their declared `path`; GSAP fetched from `/vendor/gsap.min.js` and written at `project.gsapVendorPath`. `DEFLATE`. |
| 10 | `saveAs(zipBlob, \`${sanitizeFilename(stageName)}-video.zip\`)` | `use-export-video.ts:57` | Plus a warning toast when `missingCount > 0 \|\| errorCount > 0`. |

Resolution table (`export-options.ts:4`): `720p` 1280×720, `1080p` 1920×1080,
...
(`build-export-zip.ts:11-13`).

| # | Hop | Location | What happens |
| --- | --- | --- | --- |
| 1 | `GET /api/export-video/capability` | `app/api/export-video/capability/route.ts:13` | `checkRenderServiceHealth()` — configured **and** `/health` responds within 3 s. A configured-but-absent service reports `enabled: false` so the menu only offers "Download ZIP". Never leaks the service URL. |
| 2 | `useVideoRenderStore.startRender(t, locale)` | `lib/store/video-render.ts:121` | Guards against a duplicate submit via `inFlight(status)` — the reason the lifecycle lives in a store rather than the menu component. |
| 3 | `buildExportZip(...)` | as Flow 4 | Status `compiling`. `NoScenesError` → dedicated toast. |
| 4 | `FormData`: `project` (zip), `fps`, `quality`, `format=mp4` | `video-render.ts:169-173` | Status flips to `rendering`. |
| 5 | `runPolledTask({ submit, poll, intervalMs: 3000, maxAttempts: ceil(3_600_000/3000) })` | `:177` | One hour of polling at 3 s. |
| 6 | `POST /api/export-video/render` | `app/api/export-video/render/route.ts:46` | `maxDuration = 300`. `MAX_UPLOAD_BYTES = 300 MiB`. Declared `content-length` gives a courtesy 413; the real bound is `capBodyStream` on the forwarded stream (`:70`). The body is **not** parsed — identity comes from the `x-openmaic-client` header. |
...
| 7 | Service `POST /render` | `render-service/src/main.ts:252` | `coordinator.reserve(identity)` → `extractionGate.run(...)` → `capBodyStream` + `formData()` + `parseOptions` + `unzipProject` → `coordinator.submit(...)` → `202 { jobId }`. Every failure path releases the reservation and cleans the project dir (`:322-330`). |
| 8 | `RenderCoordinator.run(job)` | `render-coordinator.ts:261` | Under `executionGate` (size = `maxConcurrency`, fixed at 1 by both resource profiles): marks `running`/`preparing`, calls `executor.execute({ projectDir, outputPath: projectDir + '/output.mp4', options, signal, deadlineMs, onProgress })`. |
| 9 | `InProcessExecutor.execute(request)` | `render-executor.ts:188` | Non-chunked path: `createRenderJob(buildProducerJobConfig(options, workers))` then `executeRenderJob(job, projectDir, outputPath, onProgress, abort.signal)`. Progress is normalized from producer percent to 0..1 (`:314`). |
...
| 10 | `artifacts.put(id, outputPath)` then `jobs.update(id, { status:'succeeded', progress:1, currentStage:'complete' })` | `render-coordinator.ts:302-310` | Followed by `cleanupProject` on any non-success (`finishNonSuccess`, `:242`). |
| 11 | Client polls `GET /api/export-video/render/:jobId` | `app/api/export-video/render/[jobId]/route.ts:12` | Relays the service body plus `pollIntervalMs: 3000`; upstream 404 preserved, everything else 502. |
| 11a | ETA | `video-render.ts:201-223` | Percent from `progress*100`; instantaneous speed (percent/ms) EMA-smoothed with `SPEED_SMOOTHING = 0.3`; only forward progress updates the speed; ETA shown from `ETA_MIN_PERCENT = 3`. |
| 12 | On `succeeded`: `GET /api/export-video/render/:jobId/download` | `.../download/route.ts` → service `main.ts:441` | Service streams the MP4 with `Content-Disposition: attachment; filename="<jobId>.mp4"`, or 302s to a presigned URL when the artifact store is URL-backed. |
| 13 | `saveAs(mp4, filename)` | `video-render.ts:237` | Plus a warning toast for missing assets / non-info diagnostics. |

Degradation on failure (`video-render.ts:243-269`) is deliberately narrow: the
...
| # | Hop | Location | What happens |
| --- | --- | --- | --- |
| 1 | Caller builds a `WhiteboardRuntimePayloadV1` | `lib/whiteboard/runtime/types.ts:62` | `{ payloadVersion: 1, operationId, operation }`. `operationId` doubles as the record id. |
| 2 | `service.append({ stageId, expectedLastSeq, payload })` | `runtime/store.ts:188` | Validates `stageId`, `expectedLastSeq` (safe non-negative integer or `null`), clones + asserts the payload. |
| 3 | `withRuntimeStorageSharedLock(...)` | `store.ts:174` | Whole append runs under the storage lock. |
...
| 5 | `foldSession` → `foldWhiteboardRuntimeRecords(session.id, records)` | `runtime/fold.ts:170` | Full replay from seq 0. Envelope validation, anchor rejection, `seq === index`, `record.id === payload.operationId`, digest-based idempotency. |
| 6 | `findExactReplay(before, payload)` | `store.ts:158` | Same `operationId` + same digest → return `{ replayed: true }`. Same id, different digest → `WHITEBOARD_RUNTIME_OPERATION_CONFLICT`. |
...
| 9 | **Dry-run** `applyWhiteboardRuntimeOperation(session.id, before.whiteboard, operation)` | `fold.ts:51` | The transition is applied *before* persisting, so a `WhiteboardRuntimeNoChangeError` (empty clear) or a validation failure never writes a record. |
...
| 12 | `refreshWhiteboardRuntimeProjection(stageId, minimumLastSeq?)` | `runtime/browser-projection.ts:8` | Reads folded state, then three staleness guards: stage changed, generation superseded, or an existing projection with a strictly higher `lastSeq`. Sets `useCanvasStore.setRuntimeWhiteboardProjection`. |
| 13 | `Whiteboard` component | `components/whiteboard/index.tsx:39-44` | `runtimeAuthoritative` = projection present, matching stage id, non-null `lastSeq`. When true it renders the projected board and hides clear/history (`:151`). Otherwise it falls back to `stage.whiteboard[0]`. |
...
| --- | --- | --- |
| `lib/choreography/**`, `lib/video-export/**` | Pure Node **and** browser | eslint `no-restricted-syntax` / `no-restricted-imports` (`eslint.config.mjs:348-492`): no `@/…`, no React/DOM/GSAP/motion, no `import()`/`require()`, depth-specific relative allowlist |
...
| `lib/audio/constants.ts`, `lib/audio/types.ts` | Both | Header states the split explicitly: kept free of Node libs so client components can import it (`constants.ts:5-8`) |
| `lib/audio/tts-providers.ts` | Server (uses `Buffer`, `process.env`) | `browser-native-tts` throws with a client-side directive (`:246`) |
| `lib/media/comfyui-workflows.ts`, `comfyui-image-adapter.ts` | Module import-safe in both; `fs` paths server-only | `typeof window === 'undefined'` wrapping a **dynamic** `import('fs')` so the bundler can dead-code-eliminate it (`comfyui-workflows.ts:57-71`) |
