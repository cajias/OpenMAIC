...
disabled (`lib/config/feature-flags.ts:10`). `.env.example:304` states the consequence explicitly:
...
| --- | --- | --- | --- |
| `NEXT_PUBLIC_MAIC_EDITOR_ENABLED` | no | gates the Pro-mode editor toggle in the header; implied by the workbench flag | `lib/config/feature-flags.ts:47`, `.env.example:315` |
| `NEXT_PUBLIC_PRO_WORKBENCH_ENABLED` | no | implies the editor gate above | `lib/config/feature-flags.ts:32`, `.env.example:310` |
| `NEXT_PUBLIC_MAIC_EDITOR_RENDERER_ENABLED` | no | selects `@openmaic/editor` inside Pro mode instead of the legacy in-app canvas; **does not** enable Pro mode by itself | `lib/config/feature-flags.ts:64`, `.env.example:318` |
| `NEXT_PUBLIC_MAIC_PLAYBACK_RENDERER_ENABLED` | no | selects `@openmaic/renderer` for the classroom playback canvas | `lib/config/feature-flags.ts:55`, `.env.example:321` |
| `NEXT_PUBLIC_ENABLE_PPTX_IMPORT` | no | shows the PPTX import entry point (the pipeline itself is unaffected) | `lib/config/feature-flags.ts:126`, `.env.example:334` |
| `NEXT_PUBLIC_ENABLE_VIDEO_EXPORT` | no | shows the video-export entry point; the renderer's `snapshot/measure.ts` geometry probe feeds that compiler | `lib/config/feature-flags.ts:121`, `.env.example:333` |

No env var configures the DSL, the importer, or the pptx exporter. Their knobs are function
parameters: `ParseOptions.mediaMode` (`packages/@openmaic/importer/src/index.ts:12`),
`ImportPptxOptions.upload` (`import-pipeline/index.ts:38`), `SlideToPngOptions`
(`packages/@openmaic/renderer/src/snapshot/index.ts:42`), `MeasureOptions`
(`snapshot/measure.ts:42`), and `NormalizeSlideOptions`
...
| --- | --- | --- |
| `useViewportSize` defaults | `viewportSize 1000`, `viewportRatio 0.5625`, `canvasPercentage 100` | `renderer/src/hooks/useViewportSize.ts:38` |
| snapshot / measure defaults | `DEFAULT_VIEWPORT_RATIO 0.5625`, `DEFAULT_TIMEOUT_MS 5000` | `renderer/src/snapshot/index.ts:80`, `measure.ts:54` |
| `slideToPng` pixel ratio | `window.devicePixelRatio ?? 1` | `renderer/src/snapshot/index.ts:102` |
| shape SVG overflow pad cap | `CAP = 4000` px | `renderer/src/elements/shape/BaseShapeElement.tsx:185` |
| line draw animation | `DRAW_ANIMATION_MS = 600` | `renderer/src/elements/line/BaseLineElement.tsx:14` |
| drag commit threshold | `DRAG_THRESHOLD_PX = 2` | `editor/src/react/useEditGesture.ts:17` |
| undo depth | `MAX_EDITOR_HISTORY = 50` (package), `MAX_HISTORY = 50` (app) | `editor/src/core/index.ts:3`, `lib/edit/slide-ops.ts:10` |
| import concurrency | `createConcurrencyLimiter(6)` | `importer/src/import-pipeline/transformParsedToSlides.ts:393` |
| import fallback viewport | `FALLBACK_VIEWPORT_SIZE = 1280` | `importer/src/import-pipeline/index.ts:34` |
| import context ratio | `ratio = 96 / 72`, `viewportWidth = 960` | `importer/src/import-pipeline/mockContext.ts` |
| server import caps | `MAX_IMPORT_SLIDES 80`, `MAX_IMPORT_BYTES 8 MiB`, `PARSE_PPTX_TIMEOUT_MS 90 000` | `lib/server/agent-runtime/import-pptx.ts:41` |
| pptx export text defaults | `DEFAULT_FONT_SIZE = 16`, `DEFAULT_FONT_FAMILY = 'Microsoft YaHei'` | `lib/export/use-export-pptx.ts:44` |
...
standalone sub-app (`pnpm-workspace.yaml:4`).

### Per-package build commands

| Package | Build | Output |
| --- | --- | --- |
| `@openmaic/dsl` | `tsc -p tsconfig.json && node scripts/gen-schema.mjs` (`package.json:24`) | `dist/*.js`, `dist/*.d.ts`, `dist/schema/{stage,scene,action}.schema.json` |
| `@openmaic/renderer` | `generate-fonts-css.mjs && generate-katex-fonts.mjs && rollup -c && tsc --emitDeclarationOnly` (`package.json:41`) | `dist/`, plus a generated `fonts.css` shipped as an export |
| `@openmaic/editor` | `rollup -c && tsc --emitDeclarationOnly` (`package.json:37`) | `dist/{core,react,ui}` |
| `@openmaic/importer` | `rollup -c && tsc --emitDeclarationOnly` (`package.json:24`) | `dist/index.js`, `dist/index.cjs`, `dist/index.d.ts` |
...
(`packages/@openmaic/dsl/src/storage.ts:55`).
...
| --- | --- | --- | --- |
| `react` / `react-dom` | `>=18` peer | everything | `renderer/package.json:64`, `editor/package.json:61` |
| `motion` | `^12.27.5` (root); `>=11` peer | `AnimatePresence` around the laser overlay | `renderer/src/SlideCanvas.tsx:4` |
| `katex` | `^0.16.33` | LaTeX element rendering, and the persisted `html` snapshot | `renderer/package.json:85`, `lib/edit/slide-edit-elements.ts:154` |
| `echarts` | `^6.0.0`; optional peer | chart element | `renderer/package.json:65`, `:73` |
| `shiki` | `^3.21.0`; optional peer | code element syntax highlighting | `renderer/package.json:69`, `:76` |
| `tinycolor2` | `^1.6.0` | table sub-theme colours, pptx colour formatting | `renderer/src/utils/element.ts:1`, `lib/export/use-export-pptx.ts:4` |
| `clsx` + `tailwind-merge` | `^2.1.1` / `^3.4.0` | the renderer's `cn` helper | `renderer/package.json:82`, `:87` |
| `lucide-react` | `^0.562.0` | audio/video element chrome, editor toolbars | `renderer/package.json:86` |
| `html-to-image` | `^1.11.13` (renderer dep) | primary snapshot rasterizer (foreignObject SVG) | `renderer/package.json:83`, `snapshot/index.ts:35` |
| `html2canvas-pro` | `^2.0.4` (renderer dep) | snapshot fallback rasterizer | `renderer/package.json:84`, `snapshot/index.ts:34` |
| `prosemirror-*` (11 packages) | `^1.x` | rich-text editing, in both the packaged editor and `lib/prosemirror` | `editor/package.json:48`–`:58`; root `package.json` |
| `immer` | `^11.1.3` | app-side op kernel (`lib/edit/slide-ops.ts:1`), and an editor dependency | root + `editor/package.json:45` |
| `react-colorful` | `^5.7.0` | the packaged editor's colour picker | `editor/package.json:59` |
| `temml` | `^0.13.1` | LaTeX → MathML for the pptx formula path | `lib/export/latex-to-omml.ts:1` |
| `jszip` | `^3.10.1` | classroom ZIP, resource pack, and inside pptxgenjs | `lib/export/use-export-classroom.ts:81` |
| `file-saver` | `^2.0.5` | download the produced Blob | `lib/export/use-export-pptx.ts:5` |
| `nanoid` | `5.1.16` | element/slide id generation in the importer and agent tools | `importer/package.json:55` |
...
| --- | --- | --- | --- |
| `pptxgenjs` | `workspace:*` → 4.0.1 | fork adds `addFormula`/OMML support | root `package.json:129`; `packages/pptxgenjs/src/slide.ts:253` |
| `mathml2omml` | `workspace:*` → 0.5.0 | fork fixes `.includes[...]` → `.includes(...)` | root `package.json:114`; commit `a3f88d53` |
| `@openmaic/importer` | `workspace:*` → 0.1.3 | fork of `pptxtojson` carrying the entire DSL transform | root `package.json:77` |
...
rejected outright by Turbopack (`scripts/sync-maic-importer.mjs:6`). `pptxtojson` is listed even though
the package *is* a fork of it — it is imported purely for the parsed-JSON types
(`src/import-pipeline/transformParsedToSlides.ts:1`, and `ParsedPptxJson` at `:34` is
...
| Package | Version | Used for | Evidence |
| --- | --- | --- | --- |
| `typebox` | `^1.1.39` | the closed agent-facing element/canvas schemas | `lib/server/agent-runtime/course-edit/element-schema.ts:17` |
| `linkedom` | `^0.18.13` | a DOM for the pptx parse worker (`linkedom/worker`) | `lib/server/agent-runtime/import-pptx-worker.mjs:7` |
| `@earendil-works/pi-agent-core` | `0.78.0` | the `AgentTool` type the DSL tools implement | `lib/server/agent-runtime/import-pptx.ts:19` |

### Build-only

| Package | Where | Purpose |
| --- | --- | --- |
| `ts-json-schema-generator ~2.4.0` | `packages/@openmaic/dsl/package.json:51` | emits the three JSON Schema artifacts; explicitly build-only so the package keeps zero runtime deps (`scripts/gen-schema.mjs:5`) |
...
`.github/workflows/ci.yml:61` and `:79`, and into the release workflow at
`.github/workflows/publish-packages.yml:115` and `:375`.
...
The rule the whole gate exists for is stated at `packages/@openmaic/dsl/src/version.ts:25`: changing
...
is spelled out at `scripts/check-package-version-bumps.mjs:290`: one published `@openmaic/storage`
...
`scripts/openmaic-packages.mjs:17` also carries an explicit threat model: the gates are configuration
