...
| `SceneType` members | 4 | [`stage.ts:22`](packages/@openmaic/dsl/src/stage.ts#L22) |
| `WidgetType` members | 6 | [`interactive.ts:4`](packages/@openmaic/dsl/src/interactive.ts#L4) |
| `EditorOperation` variants | 13 | read from [`editor/src/core/index.ts:88`](packages/@openmaic/editor/src/core/index.ts#L88) |
| `EditIntent` variants | 10 | read from [`editor/src/core/index.ts:119`](packages/@openmaic/editor/src/core/index.ts#L119) |
...
| --- | --- | --- |
| **Two-sided exhaustiveness on eight enumerated unions.** `satisfies readonly T[]` proves each tuple entry is valid; a companion conditional type proves the tuple covers the union. Adding a variant without extending the tuple is a build failure, so a validator can never silently reject a new valid variant. Eight `as const satisfies readonly T[]` sites carry it: scene types, action types, element types, widget types, runtime session statuses, core runtime kinds, chat runtime roles and quiz attempt phases. Whether this covers *every* enumerated union in the DSL was not proven — that is a negative over all string-literal unions, and only these eight were enumerated. | [`stage.ts:30`](packages/@openmaic/dsl/src/stage.ts#L30), [`action.ts:312`](packages/@openmaic/dsl/src/action.ts#L312), [`guards.ts:36`](packages/@openmaic/dsl/src/guards.ts#L36), [`interactive.ts:20`](packages/@openmaic/dsl/src/interactive.ts#L20), [`runtime.ts:65`](packages/@openmaic/dsl/src/runtime.ts#L65)/`:166`/`:289`/`:339` | strength |
| **The version design refuses to guess.** Disjoint stamp fields are recognised as *necessary but not sufficient*, and the cross-line guard closes the residual hole by throwing on an undecidable envelope. Both silent alternatives are named and rejected in a comment, with the specific corruption each would cause. | [`version.ts:99`](packages/@openmaic/dsl/src/version.ts#L99), [`:465`](packages/@openmaic/dsl/src/version.ts#L465), [`:589`](packages/@openmaic/dsl/src/version.ts#L589) | strength |
| **Migration endpoints are pinned literals, never the moving constant.** So appending a step cannot retroactively re-target an existing one. | [`version.ts:78`](packages/@openmaic/dsl/src/version.ts#L78), [`:235`](packages/@openmaic/dsl/src/version.ts#L235) | strength |
| **One source of truth for element defaults, mirrored into the schema and pinned by a test.** `ELEMENT_DEFAULTS` and the `@default` JSDoc tags cannot drift. | [`normalize.ts:69`](packages/@openmaic/dsl/src/normalize.ts#L69), [`gen-schema.mjs:36`](packages/@openmaic/dsl/scripts/gen-schema.mjs#L36) | strength |
| **Atomicity is structural, not conventional.** Every mutating path clones first and throws before returning, so a partially applied transaction is not expressible. | [`editor/src/core/index.ts:397`](packages/@openmaic/editor/src/core/index.ts#L397); [`course-edit/apply.ts:242`](lib/server/agent-runtime/course-edit/apply.ts#L242), [`:290`](lib/server/agent-runtime/course-edit/apply.ts#L290), [`:397`](lib/server/agent-runtime/course-edit/apply.ts#L397) | strength |
| **Comments explain the *rejected* alternative, not the chosen one.** Repeatedly: why `normalizeSlide` is unary ([`normalize.ts:614`](packages/@openmaic/dsl/src/normalize.ts#L614)), why `Date.parse` cannot validate ISO ([`runtime.ts:114`](packages/@openmaic/dsl/src/runtime.ts#L114)), why the parser path beats the preset formula ([`transformParsedToSlides.ts:958`](packages/@openmaic/importer/src/import-pipeline/transformParsedToSlides.ts#L958)), why the SVG viewport is grown ([`BaseShapeElement.tsx:174`](packages/@openmaic/renderer/src/elements/shape/BaseShapeElement.tsx#L174)), why `fonts.ready` alone is racy ([`measure.ts:110`](packages/@openmaic/renderer/src/snapshot/measure.ts#L110)). This is unusually high-value documentation. | many | strength |
| **The gates carry their own threat model and limitations.** The version-bump script states what it under-approximates and why closing it is a different decision; the package list states that config-in-repo cannot stop deliberate subversion. | [`check-package-version-bumps.mjs:14`](scripts/check-package-version-bumps.mjs#L14); [`openmaic-packages.mjs:17`](scripts/openmaic-packages.mjs#L17) | strength |
| **Degrade-vs-fail is a policy, not an accident.** `NormalizeSlideOptions.onInvalid` makes the choice explicit at the call site, and the importer's reasoning for choosing `'drop'` is written down. | [`normalize.ts:589`](packages/@openmaic/dsl/src/normalize.ts#L589); [`import-pipeline/index.ts:85`](packages/@openmaic/importer/src/import-pipeline/index.ts#L85) | strength |
| **The editor's controlled-component contract is honest.** Id-based selection survives edits, `EMPTY_SELECTION` is frozen, the canvas is inert without callbacks, and one gesture is documented to equal one undo entry. | [`editor/src/react/types.ts:36`](packages/@openmaic/editor/src/react/types.ts#L36), [`:47`](packages/@openmaic/editor/src/react/types.ts#L47), [`:72`](packages/@openmaic/editor/src/react/types.ts#L72) | strength |
| **The asset seam is designed against an information leak.** `put` must allocate a fresh ref every call, with the existence-oracle reasoning spelled out, and the residual channels (quota, metering) are named as out of scope rather than ignored. | [`storage.ts:77`](packages/@openmaic/dsl/src/storage.ts#L77) | strength |

## 3. Fragilities

| Observation | Evidence | Severity |
| --- | --- | --- |
| **The agent-facing TypeBox schemas are a hand-maintained mirror of `slides.ts`.** [`element-schema.ts:15`](lib/server/agent-runtime/course-edit/element-schema.ts#L15) says the field sets "mirror `@openmaic/dsl` `slides.ts` exactly" — 694 lines of hand-written duplication of a 995-line type file, with no generated cross-check in that file. The DSL already emits `scene.schema.json` from the same types; nothing forces the TypeBox copy to track it. Adding a DSL field means editing two files, and forgetting the second makes every `patch_stage` touching that field fail with "unknown property". | [`lib/server/agent-runtime/course-edit/element-schema.ts:15`](lib/server/agent-runtime/course-edit/element-schema.ts#L15) vs `packages/@openmaic/dsl/src/slides.ts` and `dist/schema/scene.schema.json` | high |
| **Two near-duplicate op kernels with different strictness.** `@openmaic/editor/src/core/index.ts` (779 LOC, strict: throws on a missing element) and `lib/edit/slide-ops.ts` (359 LOC, immer, lenient: silent no-op). Both are live — the packaged one behind a flag, the app one on the default path. The divergence is documented as deliberate for the *server* surface ([`course-edit/apply.ts:122`](lib/server/agent-runtime/course-edit/apply.ts#L122)) but the browser-vs-browser duplication is not. A behaviour fix has to land twice. | [`editor/src/core/index.ts:661`](packages/@openmaic/editor/src/core/index.ts#L661) vs [`lib/edit/slide-ops.ts:184`](lib/edit/slide-ops.ts#L184) | high |
| **Two near-duplicate ProseMirror stacks.** `lib/prosemirror/` (1 152 LOC) and `packages/@openmaic/editor/src/react/text/prosemirror/` — same schema shape, same command names, same plugin set, different `createDocument`/`createTextDocument` implementations. The packaged one has the markup-detection two-branch parse ([`document.ts:12`](packages/@openmaic/editor/src/react/text/prosemirror/document.ts#L12)); the app one does not ([`lib/prosemirror/index.ts:12`](lib/prosemirror/index.ts#L12) always wraps in `<div>`). So a plain-text `content` with newlines round-trips differently depending on which editor is enabled. | [`lib/prosemirror/index.ts:12`](lib/prosemirror/index.ts#L12) vs [`editor/src/react/text/prosemirror/document.ts:12`](packages/@openmaic/editor/src/react/text/prosemirror/document.ts#L12) | high |
| **`presets.ts` is 6 574 lines with 154 registry entries and one shared fallback.** Nothing in the file structure prevents a wrong path formula from silently shipping; the only signal for an *unregistered* shape is a `console.warn` that never reaches the UI, and there is no signal at all for a registered-but-wrong one. The importer has 932 test LOC against 22 133 source LOC. | [`presets.ts:6572`](packages/@openmaic/importer/src/shapes/presets.ts#L6572); test ratio measured above | high |
| **Line elements overload `width` as stroke width.** The DSL documents `width` on `PPTBaseElement` as the element width and `PPTLineElement` inherits it, but the renderer uses it as `strokeWidth` and as the dash-array base. Nothing in the type or the validators says so; the only evidence is the renderer. A caller that resizes a line by setting `width` changes its thickness. | [`slides.ts:480`](packages/@openmaic/dsl/src/slides.ts#L480) vs [`renderer/src/elements/line/BaseLineElement.tsx:34`](packages/@openmaic/renderer/src/elements/line/BaseLineElement.tsx#L34), [`:119`](packages/@openmaic/renderer/src/elements/line/BaseLineElement.tsx#L119) | medium |
| **The renderer silently paints nothing for an unknown element type.** No default branch, no warning. | [`renderer/src/SlideElement.tsx:99`](packages/@openmaic/renderer/src/SlideElement.tsx#L99) | medium |
| **`resizeIntent` emits box props only, by design.** A shape's `path`/`viewBox`, a table's `cellMinHeight`, and an image's clip mode are explicitly not recomputed and left to the host. The legacy canvas *does* recompute them (`components/slide-renderer/Editor/Canvas/hooks/useScaleElement.ts`, 695 LOC). So resizing the same shape produces different documents depending on the flag. | [`editor/src/react/core/intent.ts:19`](packages/@openmaic/editor/src/react/core/intent.ts#L19) | medium |
| **Three version lines, one of them undocumented in the package.** `dslVersion`, `runtimeDslVersion` and the app-only `SlideContent.schemaVersion` (still `1`, with no migration bodies). The DSL's [`stage.ts:184`](packages/@openmaic/dsl/src/stage.ts#L184) calls it "Optional for backward compatibility" and points at the app's migrator; nothing connects it to the ladder mechanics in `version.ts`. | [`lib/edit/slide-schema.ts:24`](lib/edit/slide-schema.ts#L24); [`stage.ts:184`](packages/@openmaic/dsl/src/stage.ts#L184) | medium |
| **The importer's `detectUnit` heuristic has no provenance fallback.** `|value| > 20000 → EMU`. Right on real decks, silently wrong on a pathological one, with nothing to cross-check. | [`packages/@openmaic/importer/src/parser/units.ts:47`](packages/@openmaic/importer/src/parser/units.ts#L47) | medium |
| **LaTeX size in the pptx export is estimated, not measured.** `fontSize = round(boxHeightPt / (lines * 3))` where `lines` counts `\\` occurrences. Any formula whose visual height is not proportional to its line count exports at the wrong size. | [`lib/export/use-export-pptx.ts:1040`](lib/export/use-export-pptx.ts#L1040)–[`:1043`](lib/export/use-export-pptx.ts#L1043) | medium |
| **`postProcessOmml` edits XML with regexes.** Stripping `xmlns:w`/`xmlns:m` and injecting `<a:rPr>` via `String.replace` on OMML output. Works for the shapes `mathml2omml` emits today; a change upstream in whitespace or nesting silently breaks font application rather than failing. | [`lib/export/latex-to-omml.ts:41`](lib/export/latex-to-omml.ts#L41)–[`:56`](lib/export/latex-to-omml.ts#L56) | medium |
| **`stripUnsupportedMathML` strips exactly one tag.** `['mpadded']`, by deleting open and close tags. Any other element `mathml2omml` cannot handle makes the whole conversion throw and fall back to a raster image. | [`lib/export/latex-to-omml.ts:12`](lib/export/latex-to-omml.ts#L12) | low |
| **The vendored `pptxtojson` fork still depends on the npm `pptxtojson`.** For types only (`ParsedPptxJson = Awaited<ReturnType<typeof parsePptxDefault>>`), but it is a *value* import, so the two versions must stay structurally compatible with nothing enforcing it. The comment at [`import-pipeline/index.ts:68`](packages/@openmaic/importer/src/import-pipeline/index.ts#L68) calls the bridge a cast. | [`transformParsedToSlides.ts:1`](packages/@openmaic/importer/src/import-pipeline/transformParsedToSlides.ts#L1), [`:34`](packages/@openmaic/importer/src/import-pipeline/transformParsedToSlides.ts#L34); [`importer/package.json:58`](packages/@openmaic/importer/package.json#L58) | medium |
| **`sameContent` / `applyToContent` compare documents with `JSON.stringify`.** Correct only because key insertion order is stable through `structuredClone` + `Object.assign`; an op that rebuilt an object with different key order would register as a change and push a spurious undo step. | [`editor/src/core/index.ts:336`](packages/@openmaic/editor/src/core/index.ts#L336), [`:401`](packages/@openmaic/editor/src/core/index.ts#L401) | low |
| **Snapshot determinism depends on a 5 s timeout.** `measureSlideElementGeometry` and `slideToPng` both race font settling against `DEFAULT_TIMEOUT_MS = 5000`. On a cold cache or a slow link the measurement silently proceeds with fallback faces, producing geometry that differs from the live canvas — exactly the drift the module exists to prevent. | [`snapshot/measure.ts:55`](packages/@openmaic/renderer/src/snapshot/measure.ts#L55), [`:127`](packages/@openmaic/renderer/src/snapshot/measure.ts#L127); [`snapshot/index.ts:81`](packages/@openmaic/renderer/src/snapshot/index.ts#L81) | medium |
...
- **The round-trip suite that exists is edit → export, not import → export.**
  ...
  ([`tests/edit/round-trip/geometry.test.ts:52`](tests/edit/round-trip/geometry.test.ts#L52)). `buildPptxBlob` is exported for exactly this harness
  ([`lib/export/use-export-pptx.ts:494`](lib/export/use-export-pptx.ts#L494)). There is no `.pptx` → DSL → `.pptx` fidelity suite:
