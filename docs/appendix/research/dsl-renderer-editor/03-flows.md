...
(`NEXT_PUBLIC_MAIC_EDITOR_RENDERER_ENABLED`, `lib/config/feature-flags.ts:65`).

| # | Hop | Location |
| --- | --- | --- |
| 1 | `RendererEditorCanvas` resolves media slots, then renders the packaged surface | `components/edit/surfaces/slide/RendererEditorCanvas.tsx:24` (`useResolvedSlide`), `:82` |
| 2 | `EditableSlideCanvasWithUI` builds `content = {type:'slide', canvas: documentSlide}` and a `dispatch` | `packages/@openmaic/editor/src/ui/EditableSlideCanvasWithUI.tsx:55` → `ui/runtime/useEditorDispatcher.ts:25` |
| 3 | `EditableSlideCanvas` mounts `ElementInteractionLayer` + gesture hooks | `packages/@openmaic/editor/src/react/EditableSlideCanvas.tsx:72`, `:23`–`:31` |
| 4 | `useEditGesture` captures pointer-down, builds a live working copy for 60 fps feedback | `react/useEditGesture.ts:73` |
| 5 | `prepareSnapping` / `computeDragMove` (or `computeMultiDragMove`) run per pointer-move | `react/core/drag.ts:57`, `:79`, `:170`; guides from `react/core/snapping.ts:47` `buildAlignLines`, `:125` `snapRange` |
| 6 | pointer-up: movement > `DRAG_THRESHOLD_PX = 2` decides commit vs select-only | `react/useEditGesture.ts:17`, `:289` |
| 7 | one intent is emitted: `moveIntent` for 1 element, `moveManyIntent` for N | `react/core/intent.ts:8`, `:38`; call site `react/useEditGesture.ts:297` |
| 8 | `dispatch(intents)` → `createEditorTransactionFromIntents({content, intents})` | `ui/runtime/useEditorDispatcher.ts:31` → `core/index.ts:266` |
| 9 | `compileEditorEditIntents` walks the intents against a private advancing snapshot → `EditorOperation[]` | `core/index.ts:162` |
| 10 | `createEditorTransaction` freezes `{origin:'canvas', history:'record', operations}` | `core/index.ts:143` |
| 11 | host callback `onTransaction` → `applyTransactionForScene(sceneId, tx)` (drops the tx if the scene changed) | `RendererEditorCanvas.tsx:57` → `slide-edit-session.ts:138` |
| 12 | `applyEditorTransaction(history, tx)` → `applyToContent` clones, applies each op, returns the original ref if unchanged | `core/index.ts:293`, `:393` |
| 13 | history mode `'record'` pushes `present` onto `past` (capped at 50) and clears `future` | `core/index.ts:326`, `:773` |
| 14 | `replace(history)` → `writeThrough(history.present)` → `useStageStore.updateScene(sceneId, {content})` | `slide-edit-session.ts:85`, `:79` |
| 15 | store subscribers re-render; `SlideCanvas` repaints from the new `Slide` | `packages/@openmaic/renderer/src/SlideCanvas.tsx:216` |
...
Undo path: `undo()` → `undoEditorTransaction(history)` (`core/index.ts:375`) → `replace` →
...
`past`, but does clear `future` (`slide-edit-session.ts:147`, reasoning at `:50`).
...
| # | Hop | Location |
| --- | --- | --- |
| 1 | agent reads the scene: `read_stage` returns `inventoryScene` / `textSlide` projections | `lib/server/agent-runtime/course-edit/apply.ts:582`, `:74` |
| 2 | agent calls `patch_stage` with `{stageId, sceneId, intent, ops[]}` | tool declared at `lib/server/agent-runtime/dsl-tools.ts:772` |
| 3 | per op, `applyPatchOp` dispatches by `op.op` | `dsl-tools.ts:571` |
| 4a | `set`/`remove` on a slide scene: path must start `/content/canvas/`, prefix stripped, → `applySlideEdit({op:'patch'})` | `dsl-tools.ts:556`, `:562`, `:626` |
| 4b | `add_element` / `delete_element`: slide scenes only | `dsl-tools.ts:572`, `:577` |
| 4c | `str_replace`: path must start `/content/` or `/actions/`; → `applyStrReplace` | `dsl-tools.ts:587`, `:597` |
| 4d | non-slide content (`quiz`/`interactive`/`pbl`): generic `applyJsonPointerEdit` on the whole scene | `dsl-tools.ts:631` |
| 5 | `applyJsonPointerPatch` decodes the pointer (`~0`/`~1` escapes checked), rejects out-of-bounds array indices and non-canonical indices | `course-edit/apply.ts:377`, `:146`, `:161` |
| 6 | identity guard: canvas id, element id set, and per-element `type` must be unchanged | `course-edit/apply.ts:361`, `:405` |
| 7 | **whole-canvas** schema validation against `SlideCanvasSchema` (`additionalProperties: false` at every level) | `course-edit/apply.ts:407` → `course-edit/element-schema.ts:689` |
| 8 | LaTeX side-effect: any element whose `latex` changed gets its `html` KaTeX snapshot re-rendered or deleted | `course-edit/apply.ts:414` → `lib/edit/slide-edit-elements.ts:154` |
| 9 | `add_element` extras: `id` must be absent, `validateElementInput` must pass, `afterId` XOR `index`, server assigns `el-<nanoid(8)>` | `course-edit/apply.ts:431`–`:463` |
| 10 | read-time media placeholders are rejected in values, anchors and replacements | `course-edit/apply.ts:218`, `:237`, `:327`, `:332` |
| 11 | after the last op, the serialized scene is re-scanned for placeholders assembled across ops | `dsl-tools.ts:820` |
| 12 | `validationError(next)` → `validateAppScene` (which delegates to the DSL `validateScene` for slide/quiz) | `dsl-tools.ts:827`, `:431`; `lib/document-store/validators.ts:27` |
| 13 | `runStageMutation(signal, () => putSceneBringingCurrent(store, stageId, next))` | `dsl-tools.ts:836` |
| 14 | `putScene`; on `DocumentVersionError{kind:'not-current'}` reload → splice → `saveDocument` | `lib/server/agent-runtime/document-writes.ts:32`, `:41` |
| 15 | `deps.onCheckpoint({tool:'patch_stage', …})` records the edit for the session timeline | `dsl-tools.ts:846` |
...
`patch` cannot change identity at all: `elementIdentityIssue` (`course-edit/apply.ts:361`) rejects a
...
| # | Hop | Location |
| --- | --- | --- |
| 1 | user picks a file; `useImportPptx.handleFileChange` | `lib/import/use-import-pptx.ts:48` |
...
| 5 | `toArrayBuffer` → `parseZip(buffer)` | `import-pipeline/index.ts:127`, `parser/ZipParser.ts` |
...
| 8 | `parsedToSlides(json, options)`: `createMockImportContext` + deck-width override | `import-pipeline/index.ts:56`, `:60`, `:65` |
| 9 | `transformParsedToSlides(json, ctx)` — per-slide background, per-element branch, px conversion via `ratio = 96/72` | `import-pipeline/transformParsedToSlides.ts:367`, `:459` |
| 10 | media uploads queued through a 6-way concurrency limiter | `:393` (`createConcurrencyLimiter(6)`), `:42` |
| 11 | `await Promise.allSettled(uploadTasks)` — an inner missing `.catch` cannot fail the import | `import-pipeline/index.ts:76`, rationale at `:13` |
| 12 | `normalizeImportedSlides(slides)` = `normalizeSlideWith({onInvalid:'drop', onDropped: console.warn})` | `import-pipeline/index.ts:101`, `:112` |
| 13 | `onImported(slides)` → app writes scenes | `lib/import/use-import-pptx.ts:89` |
...
(`lib/server/agent-runtime/import-pptx-worker.mjs:1`). `installHost()` (`:93`) installs a `linkedom`
...
(`lib/server/agent-runtime/import-pptx.ts:41`) and refuses to run if the worker file is absent
(`:268`).
...
`FALLBACK_VIEWPORT_SIZE = 1280` when `size.width <= 0` (`import-pipeline/index.ts:34`, `:64`), so a
...
| # | Hop | Location |
| --- | --- | --- |
| 1 | `useExportPPTX()` collects stage + scenes + viewport ratios | `lib/export/use-export-pptx.ts:1300` |
...
| 4 | `derivePptxMediaReferenceSet(slides)` builds synthetic scenes and runs `enumerateAssetManifest` | `:437` → `@openmaic/dsl/asset-manifest.ts:119` |
...
| 7 | text: `formatHTML(html, ratioPx2Pt)` → `toAST` (local himalaya port) → `pptxgen.TextProps[]` | `:64`, `lib/export/html-parser/index.ts:9` |
...
| 9 | latex: `latexToOmml(latex, fontSize)` → `pptxSlide.addFormula({omml, …})` (fork-only API) | `:1044`, `:1047`; `packages/pptxgenjs/src/slide.ts:253` |
...
| --- | --- |
| LaTeX becomes a **static image** when `temml`/`mathml2omml` throws | `use-export-pptx.ts:1056`; `latex-to-omml.ts:77` returns `null` and warns |
| MathML `<mpadded>` spacing is discarded before OMML conversion | `latex-to-omml.ts:11` |
| LaTeX font size is *estimated* from `\\` count, not measured | `use-export-pptx.ts:1040`–`:1043` |
| `special` shapes (paths using commands beyond L/Q/C/A) become images | DSL `slides.ts:422`; importer sets the flag at `transformParsedToSlides.ts:991` |
| An unresolvable media ref makes the element **skipped entirely** (`continue`) | `use-export-pptx.ts:1114`, `:1082` |
| `blob:`/remote URLs cannot survive; everything is re-fetched to base64, so an offline or CORS-blocked asset is lost | `use-export-pptx.ts:1116`–`:1127` |
| Extension guessing falls back to `mp4`/`mp3` when neither the URL nor `el.ext` says otherwise | `use-export-pptx.ts:1142` |
...
| Dropped | Evidence |
| --- | --- |
| `Stage.whiteboard` asset bytes are deliberately excluded from the archive | `lib/export/use-export-classroom.ts:110` |
| Assets that fail to inline are reported, not fatal: `InlineReport.failed` rides out with the ZIP | `use-export-classroom.ts:56` (`inlineFailures`), aggregate at `:96` |
| Reference conversion is best-effort: on failure it rolls back fresh allocations and falls back to the accessed document snapshot | `use-export-classroom.ts:71` |
