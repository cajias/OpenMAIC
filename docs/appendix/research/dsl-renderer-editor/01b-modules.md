...
Three export paths (`packages/@openmaic/editor/package.json:8`): `./core` (also the default `.`),
`./react`, `./ui`. The split is a hard layering rule stated in
`packages/@openmaic/editor/src/react/types.ts:18`:
...
the same origin (`EditableSlideCanvas.tsx:60`); both read the **same** `fitScale` so overlay and
...
per animation frame, so it maps 1:1 onto one host undo entry (`src/react/types.ts:36`). Multi-element
moves collapse into a single `element.updateMany` (`src/react/core/intent.ts:38`); single-element moves
stay on `element.update` for host backward compatibility.

`resizeIntent` (`src/react/core/intent.ts:19`) deliberately emits **box props only**. Kind-specific
...
`createTextDocument` (`prosemirror/document.ts:10`) has a two-branch parse keyed on
...
(`renderer/src/utils/richText.ts:8`), so a plain-text `content` round-trips the same way in the static
paint and in the editor. `serializeTextDocument` (`:29`) is the inverse via `DOMSerializer`.

`TextEditCommand` (`react/text/types.ts:3`) is a 3-arm union — no-value marks
...
(`react/text/commandExecutor.ts:82`) dispatches them onto a ProseMirror `EditorView`.
...
`TextEditorController` (`react/text/types.ts:53`) is the imperative handle the host gets:
...
Layer responsibilities are stated in `packages/@openmaic/importer/DESIGN.md:23`: `parser` does zip +
...
(`DESIGN.md:105`); the DSL transform multiplies by `ctx.ratio = 96/72` to get px
(`import-pipeline/mockContext.ts`, `transformParsedToSlides.ts:459`).

`detectUnit` (`units.ts:47`) is a heuristic: `|value| > 20000` → EMU, and `smartToPx` uses it. Right on
...
2. Otherwise look up `shapeList.find(item => item.pptxShapeType === el.shapType)` (`:880`) — the
   ...
   (`src/openmaic/configs/shapes.ts:278`, `SHAPE_LIST`).
...
| Registry | Count | File |
| --- | --- | --- |
| `presetShapes.set(...)` — OOXML preset geometry generators | 154 | `src/shapes/presets.ts:149` |
...
| `SHAPE_PATH_FORMULAS` — resize-time recompute formulas | 21 | `src/openmaic/configs/shapes.ts:30` |
| `SHAPE_LIST` entries carrying `pptxShapeType` | 26 | `src/openmaic/configs/shapes.ts:278` |
...
| Loss | Evidence |
| --- | --- |
| Unknown OOXML preset → rectangle | `presets.ts:6572` `console.warn('Unknown preset shape: ...')` then `M0,0 L…Z` |
| `custom` geometry with unsupported path commands → flagged `special`, exported later as an image | DSL `slides.ts:422` describes `special`; importer sets it at `transformParsedToSlides.ts:991` |
| An element the DSL cannot normalize is **dropped** | `import-pipeline/index.ts:101` `normalizeSlideWith({onInvalid:'drop'})`, warning at `:104` |
| Group transforms are baked, not preserved | `DESIGN.md:96` — `flip`/`rotation` folded into children, emitted group is always `rotate:0, isFlipH/V:false` |
| Curved connectors approximated to a single cubic Bézier | `transformParsedToSlides.ts:119` docstring for `parseCubicFromPath` |
...
| Media without an `upload` callback stays base64 (images) or a tab-scoped `blob:` URL (a/v) | `import-pipeline/index.ts:41` |
| Failed media upload leaves the original base64 in place, silently | `transformParsedToSlides.ts:416` `console.error('背景图片上传失败:', error)` |
| Chart types collapse to the DSL's 8 `ChartType`s; 3-D variants map to their 2-D peers | `transformParsedToSlides.ts:1269`–`:1302` (`bar3DChart`, `line3DChart`, `area3DChart`, `pie3DChart`, `bubbleChart`) |

Server-side import caps: `MAX_IMPORT_SLIDES = 80`, `MAX_IMPORT_BYTES = 8 MiB`,
`PARSE_PPTX_TIMEOUT_MS = 90_000` (`lib/server/agent-runtime/import-pptx.ts:41`).
...
`lib/export/html-parser/` is a 5-file TypeScript rewrite of himalaya (`index.ts:1`):
`lexer.ts` (274) → `parser.ts` (136) → `format.ts` (47) → `stringify.ts` (30), types in `types.ts`.
`toAST(str)` (`index.ts:9`) is the only entry the pptx exporter uses; `toHTML` is the inverse.

The "DSL → HTML" export is **not** a slide-to-HTML renderer. It is two things:
`inlineSceneContent` (`lib/export/use-export-classroom.ts:44`), which for `interactive` scenes only
...
| --- | --- | --- | --- |
| `packages/pptxgenjs` | 4.0.1 | needs an OMML formula primitive upstream does not have | `addFormulaDefinition` (`src/gen-objects.ts:669`), `Slide.addFormula` (`src/slide.ts:253`), `SLIDE_OBJECT_TYPES.formula`, `FormulaProps` in `types/index.d.ts:2664` |
| `packages/mathml2omml` | 0.5.0 | upstream bug in the text-container check | `a3f88d53` — `textContainerNames.includes[arr[level].name]` → `.includes(...)` in `src/parse-stringify/parse.js:79` |
| `packages/@openmaic/importer` | 0.1.3 | a fork of `pptxtojson` that grew the whole DSL transform | the entire `src/import-pipeline/` and `src/openmaic/` trees; still lists npm `pptxtojson@^1.11.0` as a dependency for the parsed-JSON **types** (`src/import-pipeline/transformParsedToSlides.ts:1`) |

The first two are wired as `"workspace:*"` in the root `package.json` (`:114`, `:129`) and built by the
root `postinstall` before anything else (`package.json:10`).
...
(`scripts/sync-maic-importer.mjs:6`). So the app loads it from a **static URL** instead.
...
`HEAD` probe so it can raise `PARSER_NOT_DEPLOYED` instead (`lib/import/use-import-pptx.ts:70`).
...
| `lib/prosemirror/` (16 files, 1 152 LOC) | the app's ProseMirror schema/plugins/commands | near-duplicate of the packaged one; `active-editor-registry.ts:28` is the additive bridge letting chrome run text commands without importing renderer internals |
...
`lib/server/agent-runtime/course-edit/apply.ts:122` states the server agent op union and the browser
