...
1. **What is the effective request-body limit for the routes with no cap of their
   own?** `parse-pdf` (`route.ts:61-62`) and `transcription` (`route.ts:79`) buffer
...
2. **Does `maxDuration` do anything in the target deployment?** 24 files declare
   ...
   (`app/api/agent/owner-events/route.ts:19-21`,
   `app/api/agent/sessions/[id]/events/route.ts:44-46`,
   `app/api/generate/image/route.ts:41`). Which deployment target is primary — and
...
3. **Would Next 16 / undici reject a `"` inside a `Content-Disposition` value?**
   ...
   `app/api/export-video/render/[jobId]/download/route.ts:57` assumes a quote
...
4. **What does the framework do with the `PUT`/`PATCH` methods the
   ...
   delegate to one handler (`route.ts:325-329`), and the Node-style handler decides
...
5. **`decideDocumentAccess(action, ownerId, readStageMeta, existsProbe, readStageMeta)`**
   (`app/api/persistence/[...path]/route.ts:291-300`) is the entire authorisation
...
6. **Does the render service actually enforce a per-identity limit on
   `x-openmaic-client`?** `app/api/export-video/render/route.ts:23-38` describes
...
7. **`getOwnerScopedDocumentStore(ownerId)`** — every `stages/**`, `folders/**`
   ...
   (asserted in comments at `app/api/stages/[id]/route.ts:5-9` and
   `app/api/stage-meta/[stageId]/route.ts:52-55`). I did not verify that claim in
...
8. **`bindOwnerMaterialsToSession` and `SessionMaterialBindingError`** — the
   ...
   (`sessions/route.ts:178-180`, `messages/route.ts:113-115`). Whether it fires for
...
9. **`resolveVisionImagesForPrompt(images, req.headers)`** takes the raw request
   headers (`app/api/generate/scene-content/route.ts:142`,
   `scene-outlines-stream/route.ts:358`), which implies it re-authenticates against
...
10. **`resolveServerAsset(assetId, req.headers, cap)`** in `extract-document`
    (`route.ts:533-537`) returns a status union including `'unauthenticated'`, so it
...
11. **Is `stages/[id]/publish` / `unpublish` intentionally dormant, or a
    regression?** `lib/server/agent-runtime/owner.ts:46-50` reads like a
...
12. **Is a trusted reverse proxy assumed in production?**
    ...
    setup exposes the app directly (`export-video/render/route.ts:23-31`). If a
...
13. **What is the intended lifetime of the `PERSISTENCE_DEV_TOKEN` scheme?**
    `lib/persistence/server-auth.ts:1-13` says production must replace the module.
...
15. **Is `comfyui-workflows` returning `{workflows: []}` on error deliberate?**
    (`route.ts:19-22`.) Every other silent degradation in the surface has a
...
16. **Is the `403 'Forbidden'` in `agent/sessions/[id]/messages`
    (`route.ts:116-118`) a deliberate exception to the no-existence-oracle rule?**
...
- `app/api/probe/route` does not exist. It is a synthetic path in a lint guard's
  allowlist (`tests/lint-llm-entry-guard.test.ts:47`).
