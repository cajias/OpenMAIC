...
- `middleware(request)` at `middleware.ts:46`. Matcher
  `['/((?!_next/static|_next/image|favicon.ico|logos/).*)']` (`middleware.ts:89`)
  so it runs for every API path.
- Workbench 404: `isProWorkbenchEnabled() && (!canInspectServerRuntime || isAgentRuntimeConfigured())`
  gates `/workbench*` only — not `/api/*` (`middleware.ts:53-58`).
- Access code: when `ACCESS_CODE` is unset the middleware is a pass-through
  (`middleware.ts:60-63`). When set, the allowlist is `/api/access-code/` prefix
  and the exact path `/api/health` (`middleware.ts:66`).
- Cookie verification is a Web-Crypto HMAC-SHA256 over the token's timestamp
  half (`verifyToken`, `middleware.ts:18-44`). **The timestamp is never checked
...
- API requests failing the gate get `401 {success:false, errorCode:'INVALID_REQUEST', error:'Access code required'}`
  (`middleware.ts:78-81`); page requests pass through so the client can render a
  modal (`middleware.ts:85`).
- Comparison in the edge path is length-then-XOR over char codes, explicitly
  documented as "not truly constant-time in JS" (`middleware.ts:37-43`).

## `lib/server/access-token.ts` — the access-code token

- `createAccessToken(accessCode)` → `` `${timestamp}.${hmacSha256(accessCode, timestamp)}` ``
  (`lib/server/access-token.ts:4-8`).
- `verifyAccessToken(token, accessCode)` uses `crypto.timingSafeEqual` on the hex
  buffers (`lib/server/access-token.ts:11-25`). This is the Node-side twin of the
  edge `verifyToken`; `/api/access-code/status` uses it (`app/api/access-code/status/route.ts:13`).
...
`readBoolean` accepts only `'true'` or `'1'` (`lib/config/feature-flags.ts:10-12`).
...
`401 {error:'login_required'}` (`app/api/stages/[id]/publish/route.ts:26-31`,
`unpublish/route.ts:25-30`) — are unreachable in the current build.
...
- The file header states outright that `NEXT_PUBLIC_PERSISTENCE_TOKEN` is
  ...
  confidentiality and no user isolation" (`lib/persistence/server-auth.ts:1-13`).
...
- Used by `app/api/persistence/[...path]` (runtime + asset requests only) and
  ...
  whiteboard path (`app/api/chat/pi/route.ts:173`).
...
- `apiError(code, status, error, details?)` (`lib/server/api-response.ts:51`)
...
| `normalizeUrlForStrictFetch(value)` | `:55` | sync; throws `UnsafeNetworkTargetError`; additionally rejects userinfo in the URL and any port other than 80/443, and rejects `metadata.google.internal`. Used only by `lib/server/agent-runtime/fetch-url.ts:50`, not by any route. |
...
(`app/api/proxy-media/route.ts:55-56`).
...
- Single JSON env var `MODEL_ROUTES`; example at `lib/server/model-routes.ts:14-15`.
...
| --- | --- | --- |
| `MAX_SESSION_TEXT_LENGTH` | 100 000 chars | `lib/server/agent-runtime/limits.ts:9` |
| `STAGE_NAME_MAX_LENGTH` | 120 chars | `lib/server/agent-runtime/stage-limits.ts:9` |
| `MAX_MATERIAL_LIST_LIMIT` | 200 | `app/api/materials/route.ts:77` |
| `MAX_BATCH_SCENE_IDS` | 200 | `app/api/stages/[id]/scenes/route.ts:33` |
| `MAX_PROXY_BYTES` | 25 MiB | `app/api/proxy-media/route.ts:67` |
| `MAX_UPLOAD_BYTES` (render) | 300 MiB | `app/api/export-video/render/route.ts:18` |
| `maxUploadBytes` / `maxDocumentBytes` | 50 MiB each (env-overridable) | `lib/server/agent-runtime/config.ts:46-48` |
| `maxMaterialsPerOwner` | 100 | `lib/server/agent-runtime/config.ts:50` |
| `maxMaterialBytesPerOwner` | 2 GiB | `lib/server/agent-runtime/config.ts:52-55` |
| `MAX_OUTLINE_STREAM_BYTES` | 512 KiB | `app/api/generate/scene-outlines-stream/route.ts:486` |
| skill upload cap | 1 048 576 bytes | `app/api/agent/skills/route.ts:63` |
