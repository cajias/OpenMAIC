...
**Sources:** [`app/page.tsx:604-682`](app/page.tsx#L604-L682), [`app/generation-preview/page.tsx:306`](app/generation-preview/page.tsx#L306), [`:568`](app/generation-preview/page.tsx#L568),
`:680`, `:1014-1052`, [`lib/hooks/use-scene-generator.ts:640-880`](lib/hooks/use-scene-generator.ts#L640-L880),
...
defensively ([`app/generation-preview/page.tsx:223-237`](app/generation-preview/page.tsx#L223-L237) backfills a missing
...
| # | Where | Call | Effect |
| --- | --- | --- | --- |
| 1 | [`app/page.tsx:608-615`](app/page.tsx#L608-L615) | build `UserRequirements` | `taskEngineMode: true` only when the vocational toggle is on |
| 2 | [`app/page.tsx:671-673`](app/page.tsx#L671-L673) | `sessionStorage.setItem('generationSession', …)` then `router.push` | the whole handoff payload |
| 3 | [`app/generation-preview/page.tsx:306`](app/generation-preview/page.tsx#L306) | `startGeneration(sessionOverride?)` | one `AbortController` per run, replacing the previous one ([`:311-313`](app/generation-preview/page.tsx#L311-L313)) |
| 4 | [`page.tsx:568`](app/generation-preview/page.tsx#L568) | `fetch('/api/generate/scene-outlines-stream', { headers: getApiHeaders() })` | credentials travel as `x-model` / `x-api-key` / `x-base-url` / `x-provider-type` headers |
| 5 | [`app/api/generate/scene-outlines-stream/route.ts:299`](app/api/generate/scene-outlines-stream/route.ts#L299) | `resolveModelFromRequest(req, body, 'scene-outlines-stream')` | a `MODEL_ROUTES` entry for the stage **wins over `x-model` and discards the client key** |
| 6 | [`route.ts:339-397`](app/api/generate/scene-outlines-stream/route.ts#L339-L397) | `sortDocumentImagesForVision` → slice at `MAX_VISION_IMAGES` → `resolveVisionImagesForPrompt` | unresolvable asset ids are dropped from **both** the prompt text and the attachments, so `[see attached]` can never promise a missing image |
| 7 | [`route.ts:421`](app/api/generate/scene-outlines-stream/route.ts#L421) | `buildOutlinePrompt(requirements, {...})` | package-owned; four golden snapshots pin it byte-for-byte |
| 8 | [`route.ts:461-489`](app/api/generate/scene-outlines-stream/route.ts#L461-L489) | `new ReadableStream` + 15 s `:heartbeat` | SSE opens with HTTP 200 before any model token arrives |
| 9 | [`route.ts:551-599`](app/api/generate/scene-outlines-stream/route.ts#L551-L599) per chunk | `extractLanguageDirective` / `extractCourseTitle` (8 KiB head scan, once each), then `extractNewOutlines(fullText, scanFrom)` | resumable brace matcher; each complete object is normalised, id-uniquified, `order`-assigned, emitted |
| 10 | [`route.ts:543`](app/api/generate/scene-outlines-stream/route.ts#L543) | 512 KiB buffer ceiling | stop reading, finalise with whatever parsed |
| 11 | [`route.ts:613-632`](app/api/generate/scene-outlines-stream/route.ts#L613-L632) | zero outlines and attempts left | emit `retry`, restart the whole stream (3 attempts total) |
| 12 | [`route.ts:661-672`](app/api/generate/scene-outlines-stream/route.ts#L661-L672) | `uniquifyMediaElementIds` then emit `done` | `gen_img_1` → `gen_img_<nanoid(8)>` across the whole course |
| 13 | [`page.tsx:616-625`](app/generation-preview/page.tsx#L616-L625) | a `retry` frame | clears collected outlines **and** the latched directive/title, so a succeeding attempt cannot inherit stale values |
| 14 | [`page.tsx:680-697`](app/generation-preview/page.tsx#L680-L697) | review gate | `reviewOutlineEnabled` setting, or a sticky mid-stream intent; otherwise auto-continue after `OUTLINE_REVIEW_AUTO_CONTINUE_MS` (2500 ms) |
| 15 | [`page.tsx:722-733`](app/generation-preview/page.tsx#L722-L733) | `stage.taskEngineMode`, `stage.languageDirective`, `stage.name = courseTitle` | the LLM-inferred title replaces the raw-requirement placeholder |

## Hop table — per-scene stage (browser)

| # | Where | Call | Effect |
| --- | --- | --- | --- |
| 1 | [`use-scene-generator.ts:645`](lib/hooks/use-scene-generator.ts#L645) | capture `startEpoch = state.generationEpoch` | every later step re-checks it |
...
| 8 | [`app/api/generate/scene-content/route.ts:116`](app/api/generate/scene-content/route.ts#L116) | `resolveModelFromRequest(req, body, 'scene-content:<type>')` | per-scene-type model routing, falling back to the base `scene-content` key |
| 9 | [`route.ts:179`](app/api/generate/scene-content/route.ts#L179) | `applyOutlineFallbacks(outline, !!languageModel, …)` | returns `effectiveOutline`, echoed back so the client's actions call uses the demoted type |
| 10 | [`use-scene-generator.ts:804-816`](lib/hooks/use-scene-generator.ts#L804-L816) | `POST /api/generate/scene-actions` with `content` + `previousSpeeches` | serial by necessity |
| 11 | [`app/api/generate/scene-actions/route.ts:170`](app/api/generate/scene-actions/route.ts#L170) | `buildCompleteScene(outline, content, actions, stageId)` | returns `null` on shape mismatch |
| 12 | [`use-scene-generator.ts:831-847`](lib/hooks/use-scene-generator.ts#L831-L847) | `generateTTSForScene(scene, …)` | **a TTS failure fails the whole scene** and pauses the batch |
| 13 | [`use-scene-generator.ts:850-854`](lib/hooks/use-scene-generator.ts#L850-L854) | epoch re-check after TTS | epoch moved ⇒ `removeFreshTtsAllocations(...)` and break, so a stage switch never leaks audio assets |
| 14 | [`use-scene-generator.ts:856-859`](lib/hooks/use-scene-generator.ts#L856-L859) | `useStageStore.addScene(scene)` | scene becomes visible; `previousSpeeches` advances |
...
| composer → outline call | `UserRequirements` | [`app/page.tsx:608`](app/page.tsx#L608) builds it |
...
| actions response | `{ scene: Scene, previousSpeeches: string[] }` | assembled by `buildCompleteScene`, [`scene-builder.ts:22`](packages/@openmaic/generation/src/scene-builder.ts#L22) |
| persisted unit | `Stage` + `Scene[]` + outline | `packages/@openmaic/dsl/src/stage.ts` |
| audio | `AudioFileRecord { id, stageId, blob, duration, format, text, voice }` | written at [`use-scene-generator.ts:474-483`](lib/hooks/use-scene-generator.ts#L474-L483) |
...
The asymmetry is documented in place at [`use-scene-generator.ts:698-706`](lib/hooks/use-scene-generator.ts#L698-L706): scene
...
| Store | `useStageStore` (Zustand) | `createInMemoryStore(stage)` + `createStageAPI(store)` — the store seam is faked in-process ([`classroom-generation.ts:89`](lib/server/classroom-generation.ts#L89)) |
| PBL failure | fails the scene, pauses the batch | `containPBLGenerationError` turns it into `null` — the scene is **skipped** ([`classroom-generation.ts:42-46`](lib/server/classroom-generation.ts#L42-L46)) |
...
| Progress | Zustand store state | a polled job row: 8 `ClassroomGenerationStep` values ([`classroom-generation.ts:62-70`](lib/server/classroom-generation.ts#L62-L70)), `pollIntervalMs: 5000` |
...
`GenerateClassroomInput` ([`classroom-generation.ts:48-60`](lib/server/classroom-generation.ts#L48-L60)) has exactly eleven
members. Notably absent: any `language` field — see
[`09-external-workbench.md`](docs/11-data-flows/09-external-workbench.md) for why that matters.
...
- `sessionStorage['generationParams']` is never removed. A stale entry from a
  previous run is read by `/classroom/[id]` ([`page.tsx:159-160`](app/classroom/[id]/page.tsx#L159-L160)); whether the
  stage-id mismatch is checked before use was not traced end to end here.

## Related

- [`03-document-to-classroom.md`](docs/11-data-flows/03-document-to-classroom.md) — the extraction half of step 3.
- [`11-concurrency-and-backpressure.md`](docs/11-data-flows/11-concurrency-and-backpressure.md) — `lazyBoundedMap`, the epoch guard, and where the real bottleneck sits.
- [`../06-generation-pipeline/index.md`](docs/06-generation-pipeline/index.md) — component structure of the package these hops call.
