...
loop and re-posts full state each iteration (`lib/chat/agent-loop.ts:154`); the
...
| --- | --- | --- | --- |
| 1 | `app/api/chat/pi/route.ts:43` | `isPiChatEnabled()` | 404 `Pi chat runtime is disabled` |
| 2 | `route.ts:56-66` | `messages` array, `storeState`, `config.agentIds` present | 400 `MISSING_REQUIRED_FIELD` |
| 3 | `route.ts:70` | `resolveSlideElementReference(body)` | `ElementReferenceValidationError` ⇒ 400 |
| 4 | `route.ts:79-90` | `agentIds` non-empty, all trimmed non-empty strings, no duplicates | 400 |
| 5 | `route.ts:98-107` | `resolveModel({ stage: 'chat-adapter', … })` | a routed `chat-adapter` stage's thinking config wins over the client's |
| 6 | `route.ts:109-111` | `isProviderKeyRequired(providerId) && !resolvedApiKey` | 401 `MISSING_API_KEY` |
| 7 | `route.ts:113-129` | `resolveAgentConfigs(body)` | request `agentConfigs` override the registry; unknown ids ⇒ 400 |
| 8 | `route.ts:147-148` | `getPiMaxAgentTurns` / `getPiMaxActionsPerAgent` | clamped to 6 and 8 — default **equals** max (`lib/chat/pi/config.ts:5-8`), so a client cannot raise them |
| 9 | `route.ts:164-187` | whiteboard capability probe | needs **all six**: native child mode, `piEnableWhiteboardTools`, an agent declaring a `wb_*` action, a valid stage id, `NEXT_PUBLIC_PERSISTENCE === '1'`, `DATABASE_URL` **and** `PERSISTENCE_DEV_TOKEN`, plus a trimmed `learnerKey` from `authenticatePersistenceHeaders` |
| 10 | `route.ts:188-203` | `resolveClassroomWebSearchConfig` | native child only; bad config ⇒ 400 |
| 11 | `route.ts:132-140` | `TransformStream` + `send: SendEvent` writing `data: <json>\n\n` | plus a 15 s `:heartbeat` (`:210-221`) |
| 12 | `director-loop.ts:118-129` | `createCallLlmStreamFn({ source: 'pi-chat-director' })` + `createDirectorCompactionRuntime` | native pi compaction, disposed in a `finally` (`:250-252`) |
| 13 | `director-loop.ts:131-208` | four director tools built | `read_scene`, `call_agent`, `close_session`, `cue_user` — nothing else |
| 14 | `director-loop.ts:238` | `afterToolCall` terminal barrier | `sessionClosed \|\| userCued \|\| directorToolCalls >= maxDirectorToolCalls` |
| 15 | `director-loop.ts:256-258` | post-loop backstop | content exists but neither `close_session` nor `cue_user` fired ⇒ the loop cues the user itself |
| 16 | `director-loop.ts:260-282` | terminal `done` frame | carries `directorState { turnCount, agentResponses, whiteboardLedger }` for the next iteration |
...
| --- | --- | --- |
| browser → `POST /api/chat/pi` (and `POST /api/chat`) | `StatelessChatRequest` — `messages`, `storeState { stage, scenes, outlines?, currentSceneId, mode, whiteboardOpen, whiteboardManualVisibilityRevision?, quizResults? }`, `config`, `directorState?` | `lib/types/chat.ts:320` |
| carried-forward turn state | `DirectorState { turnCount, agentResponses, whiteboardLedger }` — its own docstring: *"Client-maintained — backend is stateless"* | `lib/types/chat.ts:303` |
| child agent turn → director context | `AgentTurnSummary { agentId, agentName, contentPreview, actionCount, whiteboardActions, actionWarnings? }`, accumulated into `piAgentResponses` | `lib/orchestration/types.ts:34` |
| server → browser SSE frames (`opts.send`) | `StatelessEvent` — a **9-variant** discriminated union: `agent_start`, `agent_end`, `text_delta`, `action`, `thinking`, `whiteboard` (3 inner kinds), `cue_user`, `done`, `error` | `lib/types/chat.ts:435`; the terminal `done` payload is assembled at `director-loop.ts:260-282` |
| SSE frame → paced queue | `BufferItem` — **8** kinds: `AgentStartItem`, `AgentEndItem`, `TextItem` (growable `text` plus a `sealed` flag), `ActionItem`, `ThinkingItem`, `CueUserItem`, `DoneItem`, `ErrorItem` | `lib/buffer/stream-buffer.ts:84` |
| `ActionItem` → `ActionEngine` | `Action` | union at `packages/@openmaic/dsl/src/action.ts:235`; `execute(action, options)` at `lib/action/engine.ts:214` |
| whiteboard tool → durable record | `WhiteboardRuntimePayloadV1` | `lib/whiteboard/runtime/types.ts:62` |
...
| --- | --- | --- |
| the request itself | **none server-side** — `StatelessChatRequest`'s docstring is *"All state is sent from the client on each request"*; the browser re-posts full state per iteration | `lib/types/chat.ts:316-319`, `lib/chat/agent-loop.ts:154` |
| `directorState` / `whiteboardLedger` | browser memory only, and the ledger carries **only this turn's** mutations | `director-loop.ts:271-279` |
| transcript | React state in `setSessions`; no route writes it | `components/chat/use-chat-sessions.ts` |
| discussion TTS audio | ephemeral — the response URL is handed straight to `new Audio(...)`, never written to Dexie. Contrast flow 02, which stores an `AudioFileRecord` | `lib/hooks/use-discussion-tts.ts:271` |
| whiteboard mutations | **durable.** `createWhiteboardRuntimeService` appends a record per operation, partitioned by `stageId` + `learnerKey`, idempotent on a `sha256Canonical(payload)` digest, with `expectedLastSeq` optimistic concurrency (`WHITEBOARD_RUNTIME_OPERATION_CONFLICT` / `RuntimeAppendConflictError`) | `lib/whiteboard/runtime/store.ts:170`, `:158-168`, `:214-216` |

The whiteboard path is durable only when all six probe conditions of hop 9 hold.
Fail any one and the tools are simply absent from the turn (`route.ts:183-185`
...
[`04-scene-playback.md`](./04-scene-playback.md).

## The LangGraph path, for contrast

`POST /api/chat` → `statelessGenerate` (`lib/orchestration/stateless-generate.ts:392`)
→ `createOrchestrationGraph()` (`lib/orchestration/director-graph.ts:484`) →
...
| 2 | `PlaybackChromeRoot.tsx:1656` | during a `qa`/`discussion` session: `pauseActiveLiveBuffer()`, `discussionTTS.pause()`, `setIsDiscussionPaused(true)`; and when `engineMode` is `playing`/`live`: `engine.pause()` |
| 3 | `engine.ts:222` | `pause()` stashes `speechTimerRemaining`, or saves browser-TTS chunks and cancels, or pauses the audio element — skipped entirely if `currentTrigger` is set |
| 4 | `roundtable/index.tsx:403` | `handleSendMessage()` → `showLocalUserMessage(text)` (a 3 s local bubble) → `onMessageSend(text)` |
| 5 | `PlaybackChromeRoot.tsx:1587-1602` | `setIsDiscussionPaused(false)`, `resumeActiveLiveBuffer()`, `discussionTTS.cleanup()` — **before** `sendMessage` creates the next buffer, because `livePausedRef` is sticky and would be inherited |
| 6 | `PlaybackChromeRoot.tsx:1619` | `engineMode ∈ {playing, live, paused}` ⇒ `engine.handleUserInterrupt(msg)`. `paused` is included deliberately: step 2 already paused the engine while the learner typed |
| 7 | `engine.ts:440` | bump generation → save `savedActionIndex = max(0, actionIndex - 1)` **only if not already saved** → clear `triggerDelayTimer` → `currentTopicState = 'active'` → `setMode('live')` **before** stopping audio → `onUserInterrupt(text)` |
| 8 | `PlaybackChromeRoot.tsx:860` | `sendMessageWithElementReference(text, pendingInterruptElementReferenceRef.current)` → SSE request |
...
| # | Where | Behaviour |
| --- | --- | --- |
| 1 | `engine.ts:680-686` | already in `consumedDiscussions` ⇒ recurse immediately, zero dwell |
| 2 | `engine.ts:687-696` | `agentId` set but not selected ⇒ `markDiscussionConsumed` then recurse |
| 3 | `engine.ts:706-713` | `setTimeout(DISCUSSION_TRIGGER_DELAY_MS = 3000)`; the callback re-checks generation **and** `mode === 'playing'` |
| 4 | `engine.ts:710-711` | `currentTrigger = trigger`; `onProactiveShow(trigger)`. The engine now idles with **no timer running** |
| 5 | `PlaybackChromeRoot.tsx:822` | when `trigger.agentId` is absent, the trigger object is **mutated in place** with `pickStudentAgent()` so `confirmDiscussion` reads the same object |
| 6 | `components/chat/proactive-card.tsx:96` | countdown of `DISCUSSION_AUTO_SKIP_MS = 5000` in 50 ms steps, `onSkip()` at zero — both the countdown and the auto-skip are gated on `mode === 'playback'`; in `autonomous` mode the card waits indefinitely |
| 7a | `engine.ts:354` (`confirmDiscussion`) | bump generation → consume the id → save the cursor **as-is** (discussions are authored *after* their speech) → `currentTopicState = 'active'` → `setMode('live')` → `onDiscussionConfirmed(question, prompt, agentId)` |
| 7b | `engine.ts:385` (`skipDiscussion`) | consume the id → `onProactiveHide` → `processNext(generation)` when still `playing` |

## Auto-resume after the session ends

| # | Where | Step |
| --- | --- | --- |
| 1 | `PlaybackChromeRoot.tsx:1794` | session end → `onStopSession(payload)` → `handleSessionStop` |
| 2 | `PlaybackChromeRoot.tsx:495` | `engine.hasLectureInterruption()` is read **first**, because `handleEndDiscussion` clears the saved position |
| 3 | `PlaybackChromeRoot.tsx:451` | `doSessionCleanup()`: `manualStopRef = true` → `engine.handleEndDiscussion()` → flash → `discussionTTS.cleanup()` → `resetLiveState()` |
| 4 | `engine.ts:399` | bump generation, `clearEffects()`, `currentTopicState = 'closed'`, `setWhiteboardOpen(false)`, `restoreSavedLectureState()`, `onDiscussionEnd()`, `setMode('idle')` |
| 5 | `lib/playback/auto-resume.ts:37` | `shouldAutoResumeLecture({source, endReason, hadLectureInterruption, engineMode, isExhausted, playbackCompleted})` |
| 6 | `PlaybackChromeRoot.tsx:519-532` | eligible ⇒ `await chatAreaRef.startLecture(engine.getCurrentSceneId())`, then **re-check** `engineRef.current === engine` and `engine.getMode() === 'idle'`; on either failure `endSession(sessionId)`; otherwise `engine.continuePlayback()` |
...
| --- | --- | --- |
| Model needs a key, none resolved | 401 before any stream opens | `route.ts:109-111` |
| Persistence init throws during the whiteboard probe | **degrade** — `log.warn`, the turn runs with no whiteboard tools | `route.ts:183-185` |
| Client sends `maxAgentTurns: 50` | clamped to 6 | `lib/chat/pi/config.ts:5-6` |
| Director never calls `cue_user` or `close_session` | post-loop backstop cues the user | `director-loop.ts:256-258` |
| Request aborted mid-loop | `opts.signal.aborted` ⇒ return **before** the `done` frame; the browser sees a truncated stream | `director-loop.ts:254` |
| A child agent produces no visible text | `agentHadContent` stays false; `cue_user` is refused by `canCueUser` and the loop exits on `empty_turns` | `director-loop.ts:100-102`, `:204-205` |
| `read_scene` returns a non-`ok` status | `afterToolCall` marks the result `isError` so the model sees the failure rather than trusting stale evidence | `director-loop.ts:220-243` |
...
- [`04-scene-playback.md`](./04-scene-playback.md) — the lecture state the turn interrupts and returns to.
- [`06-edit-with-ai.md`](./06-edit-with-ai.md) — the *other* agent runtime, durable and server-owned.
- `../05-agent-runtime/index.md` — component structure of both runtimes and the shared pi harness.
