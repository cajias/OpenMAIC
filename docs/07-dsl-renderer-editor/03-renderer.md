...
evidence [../appendix/research/dsl-renderer-editor/01a-modules.md](docs/appendix/research/dsl-renderer-editor/01a-modules.md) §2.

## 1. Export surface

Four subpath exports plus a static stylesheet ([`packages/@openmaic/renderer/package.json:9`](packages/@openmaic/renderer/package.json#L9)): `.`,
`./elements`, `./types`, `./snapshot`, `./fonts.css`. `./types` is a pure re-export of the whole DSL
plus the renderer's own `effects` types ([`src/types/index.ts:4`](packages/@openmaic/renderer/src/types/index.ts#L4)), which is why a historical
...
1. **Array order is z-order.** `elementIndexById` maps element id → `index + 1` over the
   **unfiltered** element list ([`SlideCanvas.tsx:119-122`](packages/@openmaic/renderer/src/SlideCanvas.tsx#L119-L122)) and that becomes `zIndex`
   ([`SlideElement.tsx:170`](packages/@openmaic/renderer/src/SlideElement.tsx#L170)). Hiding an element therefore does not renumber its siblings.
2. **The element box lives in the leaf, not the wrapper.** `SlideElement`'s root div is
   ...
   ([`SlideElement.tsx:164-175`](packages/@openmaic/renderer/src/SlideElement.tsx#L164-L175)). Each `Base*Element` positions itself from the authored
   `left/top/width/height` — e.g. [`BaseTextElement.tsx:36-42`](packages/@openmaic/renderer/src/elements/text/BaseTextElement.tsx#L36-L42), [`BaseShapeElement.tsx:144-150`](packages/@openmaic/renderer/src/elements/shape/BaseShapeElement.tsx#L144-L150).
3. **`dragOffsets` never touches the document.** It is a compositor-only `translate3d` plus
   `willChange:'transform'` on the hit-target wrapper ([`SlideElement.tsx:181-184`](packages/@openmaic/renderer/src/SlideElement.tsx#L181-L184)), supplied by the
...
| `elementIdPrefix` (default `'slide-element-'`) | `:69` | must match `SpotlightOverlay`'s prefix, and the snapshot probe hard-codes the same default ([`snapshot/measure.ts:57`](packages/@openmaic/renderer/src/snapshot/measure.ts#L57)) |
...
`canvasPercentage = 100` ([`useViewportSize.ts:38-41`](packages/@openmaic/renderer/src/hooks/useViewportSize.ts#L38-L41)). `viewportStyles` is always the slide's own
...
([`SlideCanvas.tsx:213`](packages/@openmaic/renderer/src/SlideCanvas.tsx#L213)). The `Object.is` de-dupe at `:52` is what keeps a stable container from
...
| --- | --- | --- |
| `getElementPercentageGeometry` | [`src/utils/geometry.ts:16`](packages/@openmaic/renderer/src/utils/geometry.ts#L16) | the **authored** `left/top/width/height` against `viewportSize × viewportSize*viewportRatio`, returning 0–100 space. Pure, synchronous, needs no DOM |
| `measureSlideElementGeometry` | [`src/snapshot/measure.ts:73`](packages/@openmaic/renderer/src/snapshot/measure.ts#L73) | the **rendered** `.element-content` `getBoundingClientRect()`, normalized against the SlideCanvas inner container, also 0–100 |

The divergence is real and documented at [`measure.ts:6-13`](packages/@openmaic/renderer/src/snapshot/measure.ts#L6-L13): `BaseTextElement` renders
`height: elementInfo.vertical ? '100%' : 'auto'` on `.element-content` plus `padding: '10px'`
([`BaseTextElement.tsx:62-66`](packages/@openmaic/renderer/src/elements/text/BaseTextElement.tsx#L62-L66)), so an auto-sized text box's rendered extent is not its authored box —
...
anything unmeasured ([`measure.ts:18-20`](packages/@openmaic/renderer/src/snapshot/measure.ts#L18-L20)); see [../09-media-and-export/index.md](docs/09-media-and-export/index.md).

`getElementPercentageGeometry` returns `null` for an element lacking any of the four box fields
([`geometry.ts:21-28`](packages/@openmaic/renderer/src/utils/geometry.ts#L21-L28)) — which is exactly a `line` element, since it has no `height`.

### 3.3 Line geometry is its own coordinate system

`getElementRange` ([`src/utils/element.ts:43`](packages/@openmaic/renderer/src/utils/element.ts#L43)) has three branches, and the line branch is the odd one:
...
stroke width ([`BaseLineElement.tsx:119`](packages/@openmaic/renderer/src/elements/line/BaseLineElement.tsx#L119)), as the dash-array base (`:32-39`) and as the arrow/dot
...
`getLineElementPath` ([`element.ts:72`](packages/@openmaic/renderer/src/utils/element.ts#L72)) turns the mutually-exclusive control-point fields into an SVG
...
`createTextProseStyles(selector)` ([`src/styles.ts:18`](packages/@openmaic/renderer/src/styles.ts#L18)) exists precisely so the static renderer and the
ProseMirror editor share one layout contract while keeping different DOM roots (`:11-17`); it is
injected once as an inline `<style>` at the top of `SlideCanvas` ([`SlideCanvas.tsx:170`](packages/@openmaic/renderer/src/SlideCanvas.tsx#L170)), which is why
...
| --- | --- | --- |
| auto-height text extent depends on loaded font faces | [`BaseTextElement.tsx:66`](packages/@openmaic/renderer/src/elements/text/BaseTextElement.tsx#L66) | [`measure.ts:117-129`](packages/@openmaic/renderer/src/snapshot/measure.ts#L117-L129) force-loads every `(style, weight, family)` triple actually present in the tree before awaiting `document.fonts.ready` |
| `document.fonts.ready` alone is racy | [`measure.ts:110-114`](packages/@openmaic/renderer/src/snapshot/measure.ts#L110-L114) | it can resolve *before* the off-screen render triggers a self-hosted woff2 fetch, leaving a fallback face whose advances differ |
| KaTeX re-fits after `KaTeX_Size` loads (a React state update) | [`measure.ts:136-139`](packages/@openmaic/renderer/src/snapshot/measure.ts#L136-L139) | two `requestAnimationFrame` waits bracket the read on each side ([`:115`](packages/@openmaic/renderer/src/snapshot/measure.ts#L115), [`:138`](packages/@openmaic/renderer/src/snapshot/measure.ts#L138)) |
| the settle wait is capped | [`measure.ts:55`](packages/@openmaic/renderer/src/snapshot/measure.ts#L55) `DEFAULT_TIMEOUT_MS = 5000` | on a cold cache the measurement proceeds with fallback faces — producing exactly the drift the module exists to prevent |
| shape paths can extend outside the element box | [`BaseShapeElement.tsx:174-181`](packages/@openmaic/renderer/src/elements/shape/BaseShapeElement.tsx#L174-L181) | the SVG viewport is grown to the path's coordinate bbox and offset back, capped at `CAP = 4000` px (`:185`), because html2canvas-pro clips SVG overflow and turned connector arcs into stubs |
| an unknown SVG path token | [`BaseShapeElement.tsx:82`](packages/@openmaic/renderer/src/elements/shape/BaseShapeElement.tsx#L82) | skipped rather than looped on; a non-finite bbox returns `null` (`:85-93`) and no padding is applied |
| an unrecognised element `type` | [`SlideElement.tsx:98`](packages/@openmaic/renderer/src/SlideElement.tsx#L98), [`:103`](packages/@openmaic/renderer/src/SlideElement.tsx#L103) | **none** — the switch returns `null` with no default warning, so the element silently does not paint |
...
Why native-paint-first ([`snapshot/index.ts:10-25`](packages/@openmaic/renderer/src/snapshot/index.ts#L10-L25)): `html-to-image` serializes the slide into an SVG
...
`measureSlideElementGeometry` mirrors the same mount lifecycle ([`measure.ts:22-23`](packages/@openmaic/renderer/src/snapshot/measure.ts#L22-L23)) and is
...
| --- | --- | --- |
| `BaseChartElement` → `Chart` | `echarts >=5` | optional peer ([`package.json:65`](packages/@openmaic/renderer/package.json#L65), `:73`) |
| `BaseCodeElement` | `shiki >=1.0.0` | optional peer (`:69`, `:76`) |
| `BaseLatexElement` | `katex ^0.16.33` | dependency (`:85`) |
| `BaseTableElement` → `StaticTable` | `tinycolor2` for sub-theme colours ([`utils/element.ts:1`](packages/@openmaic/renderer/src/utils/element.ts#L1)) | dependency |
| `LaserOverlay` | `motion` `AnimatePresence` ([`SlideCanvas.tsx:4`](packages/@openmaic/renderer/src/SlideCanvas.tsx#L4)) | peer `>=11` |
...
pin `echarts ^6.0.0` (root [`package.json:97`](package.json#L97)) and `shiki ^3.21.0` (`:150`), but the renderer accepts
echarts 5.x and shiki 1.x too.

## Open questions

- Is `@openmaic/renderer`'s required `tailwindcss >= 4` peer ([`package.json:70`](packages/@openmaic/renderer/package.json#L70)) still genuine now
  that `SLIDE_RENDERER_STYLES` is injected as a `<style>` element ([`src/styles.ts:53`](packages/@openmaic/renderer/src/styles.ts#L53),
  [`SlideCanvas.tsx:170`](packages/@openmaic/renderer/src/SlideCanvas.tsx#L170)) and every element uses inline styles? The `DESIGN.md` D3 decision predates
  that injection.
- What `chrome: false` costs beyond the shadow and radius. The docstring names exactly those two
  effects ([`SlideCanvas.tsx:78-85`](packages/@openmaic/renderer/src/SlideCanvas.tsx#L78-L85)); whether snapshot output is otherwise byte-identical to the
...
- Whether the renderer's `./fonts.css` export and the generated `KATEX_FONT_EMBED_CSS` are kept in
  sync with the app's own font loading. [`packages/@openmaic/renderer/FONTS.md`](packages/@openmaic/renderer/FONTS.md) was not read in full.
- Whether the silent `null` for an unknown element type ([`SlideElement.tsx:98`](packages/@openmaic/renderer/src/SlideElement.tsx#L98)) is intended as
