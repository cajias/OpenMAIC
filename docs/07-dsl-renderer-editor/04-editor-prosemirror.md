...
evidence [../appendix/research/dsl-renderer-editor/01b-modules.md](docs/appendix/research/dsl-renderer-editor/01b-modules.md) §3,
[../appendix/research/dsl-renderer-editor/03-flows.md](docs/appendix/research/dsl-renderer-editor/03-flows.md) Flow A.

## 1. Three layers, stated as a rule

[`packages/@openmaic/editor/src/react/types.ts:18-24`](packages/@openmaic/editor/src/react/types.ts#L18-L24) names the layering explicitly:
...
| **L2** | the agent tool surface, "expected to churn and lives outside this package" | `lib/server/agent-runtime/` — see [./05-ai-edit-operations.md](docs/07-dsl-renderer-editor/05-ai-edit-operations.md) |

Three subpath exports ([`packages/@openmaic/editor/package.json:8`](packages/@openmaic/editor/package.json#L8)): `./core` (also the default `.`),
`./react`, `./ui`.

## 2. The intent → operation mapping

L1 has 10 variants ([`core/index.ts:119`](packages/@openmaic/editor/src/core/index.ts#L119)), L0 has 13 ([`:88`](packages/@openmaic/editor/src/core/index.ts#L88)). L1 is coarser and **lenient**; L0 is
...
- **The compiler advances a private working snapshot.** `append` ([`core/index.ts:169`](packages/@openmaic/editor/src/core/index.ts#L169)) applies each
...
`applyOperation` ([`core/index.ts:424`](packages/@openmaic/editor/src/core/index.ts#L424)) throws on every one of these:
...
The gesture→intent contract is stated at [`src/react/types.ts:36-39`](packages/@openmaic/editor/src/react/types.ts#L36-L39): one completed gesture emits
**exactly one** intent (or one batch), never one per animation frame, so it maps 1:1 onto one host
undo entry. [`useEditGesture.ts:17`](packages/@openmaic/editor/src/react/useEditGesture.ts#L17) `DRAG_THRESHOLD_PX = 2` decides commit versus select-only
...
[`src/react/core/intent.ts:8`](packages/@openmaic/editor/src/react/core/intent.ts#L8), [`:38`](packages/@openmaic/editor/src/react/core/intent.ts#L38)).

`resizeIntent` ([`src/react/core/intent.ts:19`](packages/@openmaic/editor/src/react/core/intent.ts#L19)) deliberately emits **box props only**. Its docstring
...
`writeThrough` ([`components/edit/surfaces/slide/slide-edit-session.ts:79`](components/edit/surfaces/slide/slide-edit-session.ts#L79)) is the single point where
...
`buildPlugins` ([`plugins/index.ts:16`](packages/@openmaic/editor/src/react/text/prosemirror/plugins/index.ts#L16)) installs six plugins in a fixed order, plus a seventh only when
...
| --- | --- | --- |
| `buildInputRules(schema)` | [`plugins/inputrules.ts:49`](packages/@openmaic/editor/src/react/text/prosemirror/plugins/inputrules.ts#L49) | stock `smartQuotes`, `ellipsis`, `emDash`, then five local rules: `> ` wraps a blockquote (`:11`); `1. ` opens an ordered list whose `order` attr is the typed number, and the rule only fires when that number continues the list (`:13-19`); `-`/`+`/`*` + space opens a bullet list (`:21`); `` `x` `` replaces the backticks with `x ` and applies the `code` mark (`:23-34`); a bare URL — the scheme is optional (`:37`) — gets a `link` mark with `href` and `title` both set to the matched text (`:36-47`) |
| `keymap(buildKeymap(schema))` | [`plugins/keymap.ts:18`](packages/@openmaic/editor/src/react/text/prosemirror/plugins/keymap.ts#L18) | mark toggles `Mod-b/i/u/d/e/;/'` → strong, em, underline, strikethrough, code, superscript, subscript (`:28-34`); `Mod-z`/`Mod-y` undo/redo; `Backspace` → `undoInputRule`, so an autoformat that just fired can be typed back out (`:26`); `Escape` → `selectParentNode`; `Alt-Arrow{Up,Down}` → `joinUp`/`joinDown`; `Enter` is a `chainCommands` that tries `splitListItem` first and falls back through `newlineInCode`, `createParagraphNear`, `liftEmptyBlock`, `splitBlockKeepMarks` (`:35-44`); `Mod-[`/`Mod-]`/`Tab` lift and sink list items (`:45-47`) |
| `placeholderPlugin(placeholder)` | [`plugins/placeholder.ts:9`](packages/@openmaic/editor/src/react/text/prosemirror/plugins/placeholder.ts#L9) | a decorations-only plugin: it decorates the paragraph **holding the cursor** with `data-placeholder` whenever that paragraph is empty (`nodeSize === 2`, `:5-7`), so the hint follows the caret rather than appearing only on an empty document |

One trap in that keymap: `Tab` is bound *only* to `sinkListItem` ([`keymap.ts:47`](packages/@openmaic/editor/src/react/text/prosemirror/plugins/keymap.ts#L47)). Outside a list the
...
([`RendererTextEditor.tsx:144`](packages/@openmaic/editor/src/react/text/RendererTextEditor.tsx#L144)), which keeps a pure-scroll or pure-metadata transaction from
...
`content` with `white-space: pre-line` ([`BaseTextElement.tsx:27`](packages/@openmaic/renderer/src/elements/text/BaseTextElement.tsx#L27)); the editor's tag-free branch turns
the same literal newlines into explicit `<br>` nodes so a plain-text `content` round-trips the same
way in both.

The legacy app stack does **not** do this. [`lib/prosemirror/index.ts:12-17`](lib/prosemirror/index.ts#L12-L17) always wraps:
...
`TextEditCommand` ([`src/react/text/types.ts:3`](packages/@openmaic/editor/src/react/text/types.ts#L3)) is a three-arm union:
...
`executeTextCommand` ([`src/react/text/commandExecutor.ts:82`](packages/@openmaic/editor/src/react/text/commandExecutor.ts#L82)) dispatches them onto an `EditorView`.
...
`TextEditorController` ([`src/react/text/types.ts:53`](packages/@openmaic/editor/src/react/text/types.ts#L53)) is the imperative handle the host receives:
...
| Missing element | **throws** (`:661`) | silent no-op ([`lib/edit/slide-ops.ts:184`](lib/edit/slide-ops.ts#L184), [`:198`](lib/edit/slide-ops.ts#L198)) |
| Reached when | `NEXT_PUBLIC_MAIC_EDITOR_RENDERER_ENABLED` is on | the default path |

Both are live today, selected by feature flag. [`lib/server/agent-runtime/course-edit/apply.ts:122`](lib/server/agent-runtime/course-edit/apply.ts#L122)
...
- Which browser op kernel is the intended future? [`slide-edit-session.ts:160-165`](components/edit/surfaces/slide/slide-edit-session.ts#L160-L165) casts an
...
- Is `components/slide-renderer/Editor/Canvas/` (the legacy editable canvas: `useScaleElement.ts` 695
  ...
  ([`lib/config/feature-flags.ts:60`](lib/config/feature-flags.ts#L60)) but no deprecation marker exists on the legacy tree.
- Is `'navigate'` ever used with an operation type its matcher does not recognise? If so it silently
  degrades to "replace present, clear future" ([`core/index.ts:323`](packages/@openmaic/editor/src/core/index.ts#L323)). No caller was traced.
- `EditableSlideCanvasProps` carries props documented as no-ops "until Part A" — `grid`, `ruler`,
  `snapping` ([`src/react/types.ts:150`](packages/@openmaic/editor/src/react/types.ts#L150)) — and `onElementsChange` is documented as "Not yet emitted by
  the Stage 0 shell" (`:101-106`) while [`useEditGesture.ts:302`](packages/@openmaic/editor/src/react/useEditGesture.ts#L302) clearly calls it. `snapping` is also
  consumed ([`useEditGesture.ts:74`](packages/@openmaic/editor/src/react/useEditGesture.ts#L74)). Those comments are stale; whether anything else in that props
