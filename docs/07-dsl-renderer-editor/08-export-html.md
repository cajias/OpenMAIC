...
evidence [../appendix/research/dsl-renderer-editor/01b-modules.md](../appendix/research/dsl-renderer-editor/01b-modules.md) §5.3.
...
| --- | --- | --- |
| Entry | `exportResourcePack` (`use-export-pptx.ts:1375`) | `exportClassroomZip` (`use-export-classroom.ts:261`) |
| Builder | `buildResourcePackZip` (`use-export-pptx.ts:1243`) | `buildClassroomExportZip` (`use-export-classroom.ts:80`) |
...
| Reimportable | no | yes — `agentConfigFromManifest` (`classroom-zip-types.ts:98`) is the documented inverse |
...
(`use-export-pptx.ts:1378`, `use-export-classroom.ts:153`).
...
`AssetRefKind` (`html-asset-inventory.ts:6-25`): `link`, `script`, `img`, `srcset`, `poster`,
...
so earlier offsets stay valid (`html-asset-inventory.ts:71-88`). Any overlapping or out-of-bounds patch
...
`createProxiedFetch()` (`proxied-fetch.ts:10`) routes cross-origin `http(s)` requests through
...
[../15-cross-cutting/index.md](../15-cross-cutting/index.md) for the SSRF boundary itself.

### 3.4 Import maps keep an online fallback by default

`keepImportmapFallbacks` defaults to `true` (`inline-assets-shared.ts:10-11`): the inlined import map
...
| CSS `@import` | fetched and wrapped so `layer` / `supports` / `media` conditions survive (`inline-assets.ts:79-92`) |
...
| MIME when the server omits `content-type` | guessed from the extension (`guessMime`, `inline-assets.ts:170`) |

### 4.2 Dropped or degraded

| Dropped | Evidence |
| --- | --- |
| Any asset over `maxAssetBytes` (default **8 MiB**) | `inline-assets.ts:110`, `:132` — returns `null`, recorded in `report.failed` |
| Any asset whose fetch returns a 4xx other than 429 | `inline-assets.ts:127` — permanent, no retry |
| Any asset still failing after 3 attempts | `inline-assets.ts:121-144` |
| `Stage.whiteboard` refs and their bytes, from the classroom ZIP | `use-export-classroom.ts:109-113`: "Classroom ZIP v1 has never serialized `Stage.whiteboard` … archiving their bytes would create an unreconstructable, permanently orphaned payload on import." Scene whiteboards **are** kept |
| `Stage.interactiveMode` | `classroom-zip-types.ts:32-33`: "intentionally NOT exported — it reflects the original generation prompt branch, which imports can't faithfully reproduce" |
| Agent ids — identity becomes positional (index into `manifest.agents`) | `classroom-zip-types.ts:49-53`; `multiAgent.agentIds` become `agentIndices` (`use-export-classroom.ts:176-178`) |
| `SpeechAction.audioId` — replaced by `audioRef`, a ZIP path | `ManifestAction = Omit<Action,'audioId'> & {audioRef?, agentIndex?}` (`classroom-zip-types.ts:128`) |
| Referenced audio whose bytes resolved nowhere | kept as a `{missing: true}` `mediaIndex` row rather than silently omitted (`use-export-classroom.ts:203-214`) |
| A malformed voice binding on **import** | dropped per field; the agent survives (`classroom-zip-types.ts:67-89`) |
| Everything a slide *looks* like | slides ship as DSL JSON; nothing renders them to HTML |
| Non-interactive scenes in the resource pack | only `content.type === 'interactive' && content.html` produces a file (`use-export-pptx.ts:1266`) |

Failures are reported, never fatal: `InlineReport.failed` rides out with the ZIP
(`use-export-classroom.ts:58`, aggregated at `:159-162`) and surfaces as a
`toast.warning(export.inlinePartial)` listing the distinct failing **hosts**
(`use-export-classroom.ts:273-289`, and the same pattern at `use-export-pptx.ts:1408-1427`).
...
(`packages/@openmaic/dsl/src/interactive.ts:45-46`). The inliner does not touch behaviour:
...
| `postMessage` conversation with the OpenMAIC host | the host is `InteractiveIframeHost` in the live app ([../08-classroom-runtime/index.md](../08-classroom-runtime/index.md)); a standalone file has no peer |
...
`CLASSROOM_ZIP_FORMAT_VERSION = 1` (`classroom-zip-types.ts:11`) — a fourth version line, independent
of `dslVersion`, `runtimeDslVersion` and `SlideContent.schemaVersion`
([./02-dsl-invariants.md](./02-dsl-invariants.md) §3).
...
The critical ordering decision is documented at `use-export-classroom.ts:61-76`: the **authoritative
...
Note `MediaIndexEntry.sourceRef`'s comment (`classroom-zip-types.ts:140-141`): "Original document ref;
...
- Whether a standalone interactive page is ever verified to actually run offline. The
  ...
  off.
- Whether the 8 MiB `maxAssetBytes` default is tuned or arbitrary (`inline-assets.ts:110`). A single
...
- What an importer does with a `{missing: true}` `mediaIndex` row
  (`use-export-classroom.ts:203-214`). The producer side is explicit; the consumer side is outside this
  subsystem.
- Why `buildClassroomExportZip` wraps its body in `try { … } catch (error) { throw error }`
  (`use-export-classroom.ts:100`, `:246-248`) — the catch is a no-op rethrow, so either a `finally`
