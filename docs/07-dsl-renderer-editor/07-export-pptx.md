...
evidence [../appendix/research/dsl-renderer-editor/03-flows.md](../appendix/research/dsl-renderer-editor/03-flows.md) Flow D,
...
they go through the resource pack ([./08-export-html.md](./08-export-html.md)).
...
| `formatHTML` | `:64` | the slide's HTML `content` → `pptxgen.TextProps[]` via the local himalaya port (`lib/export/html-parser/index.ts:9` `toAST`) |
...
beyond `L Q C A`, set by the importer at `transformParsedToSlides.ts:991` — is rasterized: an inline
...
(`packages/pptxgenjs/src/slide.ts:253`, `packages/pptxgenjs/src/gen-objects.ts:669`
`addFormulaDefinition`) — see [./09-vendored-forks.md](./09-vendored-forks.md). When it is taken, the
...
`buildMathRPr` (`latex-to-omml.ts:25`) is the reason the fork is needed at all: PowerPoint requires
...
| --- | --- |
| A LaTeX element becomes a **static image** when temml or mathml2omml throws | `use-export-pptx.ts:1056`; `latex-to-omml.ts:77-80` returns `null` and warns |
| A LaTeX element with **neither** OMML nor a legacy `path` is silently omitted | `use-export-pptx.ts:1046`, `:1056` — there is no `else` branch and the export still succeeds |
| MathML `<mpadded>` spacing is discarded before conversion | `latex-to-omml.ts:12` |
| LaTeX font size is **estimated** from `\\` count, not measured | `use-export-pptx.ts:1040-1043` |
| `special` shapes become images, losing editability | `use-export-pptx.ts:691-726`; the flag's semantics at `packages/@openmaic/dsl/src/slides.ts:422` |
| A shape's gradient collapses to one mixed colour | `use-export-pptx.ts:737-743` |
| A shape whose path `toPoints` cannot parse is skipped | `use-export-pptx.ts:733` |
| An unresolvable media ref makes the element **skipped entirely** | `use-export-pptx.ts:1114` (`continue`) — no counter, no toast |
| An offline or CORS-blocked asset is lost: everything is re-fetched to base64 | `use-export-pptx.ts:1116-1127` |
| Extension guessing falls back to `mp4`/`mp3` | `use-export-pptx.ts:1142` |
| A deck whose `viewportRatio` is neither `0.625` nor `0.75` gets `LAYOUT_16x9` | `use-export-pptx.ts:524-526` |
| Interactive / quiz / PBL scenes have no `.pptx` representation | the hook filters to slide scenes (`:1313`) |
| Speaker-note asset refs and non-rendered whiteboards are outside the export's media scope | `use-export-pptx.ts:431-436` |
...
un-edited fixture does *not* already match (`tests/edit/round-trip/geometry.test.ts:52`).
`buildPptxBlob` is exported for exactly this harness (`use-export-pptx.ts:494`).
...
- `postProcessOmml` edits XML with regexes (`latex-to-omml.ts:41-58`). It works for the shapes
...
- `stripUnsupportedMathML` strips exactly one tag (`latex-to-omml.ts:12`). Any other element
