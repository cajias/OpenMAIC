...
[`packages/@openmaic/dsl/src/version.ts:1-51`](packages/@openmaic/dsl/src/version.ts#L1-L51) is a 51-line module docstring that states the
...
the package version happens to be" ([`version.ts:4-10`](packages/@openmaic/dsl/src/version.ts#L4-L10)).
...
admits minors as well, so a **MAJOR**" ([`version.ts:35-36`](packages/@openmaic/dsl/src/version.ts#L35-L36)). The rule is stated that way
...
([`scripts/check-package-version-bumps.mjs:308`](scripts/check-package-version-bumps.mjs#L308)) reads both constants at the merge base and
...
([`version.ts:250-254`](packages/@openmaic/dsl/src/version.ts#L250-L254)).
...
([`version.ts:548-551`](packages/@openmaic/dsl/src/version.ts#L548-L551)).
...
lint-walled against exactly that ([`eslint.config.mjs:97-140`](eslint.config.mjs#L97-L140)).

**One version line for both shapes.** Rejected in writing, with the corruption scenario
quoted above ([`version.ts:250-254`](packages/@openmaic/dsl/src/version.ts#L250-L254)).
...
([`version.ts:46-50`](packages/@openmaic/dsl/src/version.ts#L46-L50)).
...
chance" ([`version.ts:165-174`](packages/@openmaic/dsl/src/version.ts#L165-L174)). The 0.1.0 → 0.2.0 step is therefore a pure stamp that
...
process ([04](docs/18-decisions/04-render-service-as-a-separate-deployable.md)) because what crosses the
boundary is that document rather than live application state. Migrations are pure and
idempotent by contract ([`version.ts:148-151`](packages/@openmaic/dsl/src/version.ts#L148-L151)), so they are testable without a store.

**Bad.**

- **Every migration endpoint must be a pinned literal, never the moving constant**
  ([`version.ts:79-84`](packages/@openmaic/dsl/src/version.ts#L79-L84), [`:214-216`](packages/@openmaic/dsl/src/version.ts#L214-L216)) — an easy rule to break and the ladder's correctness
...
- **A hand-maintained mirror of the element schema exists anyway.**
  ...
  ([`../14-code-quality/08-complexity-hotspots.md`](docs/14-code-quality/08-complexity-hotspots.md) §2).
...
- Whether `element-schema.ts` was meant to be generated from the DSL and never was.
  ...
  ([`../14-code-quality/08-complexity-hotspots.md`](docs/14-code-quality/08-complexity-hotspots.md) §2).
- `RUNTIME_DSL_MIGRATIONS` ships empty ([`version.ts:314`](packages/@openmaic/dsl/src/version.ts#L314)). Whether the runtime shape has
  genuinely never changed, or changed before it was versioned, is not recorded.

---

Previous [02-no-schema-layer-at-the-http-edge.md](docs/18-decisions/02-no-schema-layer-at-the-http-edge.md)
· next [04-render-service-as-a-separate-deployable.md](docs/18-decisions/04-render-service-as-a-separate-deployable.md)
· back to [index.md](docs/18-decisions/index.md)
