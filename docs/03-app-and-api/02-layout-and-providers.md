...
[`../appendix/research/app-shell-and-routing/02b-interfaces-client-contracts.md`](../appendix/research/app-shell-and-routing/02b-interfaces-client-contracts.md).

## The stack, outer to inner

`app/layout.tsx:42-62`. `RootLayout` itself is a **server** component; every one
...
| --- | --- | --- | --- | --- |
| 1 | `ThemeProvider` | `{ theme, setTheme, resolvedTheme }` via `ThemeContext`; toggles the `dark` class on `document.documentElement` (`use-theme.tsx:33-40`); persists to `localStorage['theme']` | `useTheme()` — throws outside the provider (`use-theme.tsx:68`) | `children` |
| 2 | `I18nProvider` | `{ locale, setLocale, t }`; side-effect-imports `@/lib/i18n/config` (line 6) so i18next is initialised by mounting | `useI18n()` — throws outside the provider (`use-i18n.tsx:63`) | `children` |
...
The comment at `app/layout.tsx:54-56` is explicit: `StorageHealthNotice` can raise
...
(`use-theme.tsx:23-29`, with an explicit
...
The re-fetch inside `AccessCodeGuard.onSuccess` (`access-code-guard.tsx:53`) exists
...
after a successful unlock. See [`./04-middleware.md`](./04-middleware.md) for the
401.
...
| --- | --- | --- |
| Geist Sans / Geist Mono | `next/font` via the `geist` package; CSS variables land on `<body className>` (`app/layout.tsx:45`) | standard next/font path |
| Inter (the UI font) | `@fontsource-variable/inter` **stylesheet** import (`app/layout.tsx:29`) | only the stylesheet carries per-subset `unicode-range` declarations |

The 13-line comment at `app/layout.tsx:16-28` records both failed alternatives:
...
Four global CSS imports at `app/layout.tsx:4-7`: `./globals.css`,
...
declaration** at `lib/brand/brand-context.tsx:27`.
...
`markSrc: '/openmaic-mark.png'`, `themeColor: '#722ed1'`, `lib/brand/brand-config.ts:27`),
...
`brand-config.ts:1-9`: "The reference (live deployment) resolves the brand per vendor from
...
off." `brand-context.tsx:8-11` adds that the provider "accepts the values as props (for
...
`tests/workbench/workspace-rail-session-rename.test.ts:22-23` mocks the context module
...
exact). See [`../10-persistence-and-state/index.md`](../10-persistence-and-state/index.md).

`app/classroom/[id]/page.tsx:224` mounts a **second** `ThemeProvider` nested
inside the root layout's. Harmless (the inner context wins and behaves
identically) but redundant.

## Open questions

- `components/ui/sonner.tsx:3` imports `useTheme` from **`next-themes`**, not from
  `lib/hooks/use-theme`. `next-themes` is a declared dependency
  (`package.json:120`) and this is its only importer in the repo; no
...
- `app/editor-fonts.ts` claims in its docstring to be "imported once from the root
  layout". It is not — the only importer is `lib/edit/preload-editor.ts:35`, a lazy
  `import()`.
