...
**Sources:** `package.json:6`-`:8`, `.nvmrc`, `README.md:106`-`:109`,
...
[persistence-storage-state/04](../appendix/research/persistence-storage-state/04-dependencies-and-config.md).
...
`engines.node` is `>=22.19.0` (`package.json:6`-`:8`). Where that constrains
anything:

| Point | Value | Effect |
| --- | --- | --- |
| `package.json:7` | `>=22.19.0` | The declaration. Advisory at install time — there is **no `.npmrc`** in the repository, so `engine-strict` is off and `pnpm install` will not refuse an older Node. |
| `scripts/check-node-engine-contract.mjs` (`pnpm check:node-engine`, `ci.yml:93`) | — | Does **not** check the developer's Node. It reads every installed direct production dependency's own `engines.node` and fails if the root minimum (`semver.minVersion`) is *below* any of them (`:57`-`:63`). It is a consistency proof about the declaration, not an enforcement of it. Requires `pnpm install` to have run first (`:40`-`:44`). |
| `.nvmrc` | `22` | Resolves to the newest available 22.x, which satisfies `>=22.19.0` today. Nothing pins the patch floor. |
| `ci.yml:56`, `:203`, `:236` | `node-version: 22` | Same shape: `setup-node` resolves the latest 22.x. |
| `Dockerfile:4`, `:82` | `node:22-alpine` (floating tag) | Latest 22.x at build time. Not digest-pinned. |
| `render-service/Dockerfile:8` | `node:22.22.2-bookworm-slim@sha256:f3a68cf4…` | The only **digest-pinned** Node in the repository, and the only exact version. |
| `render-service/package.json` | `engines.node: ">=22"` | Looser than the root. |
| `README.md:108` | `Node.js >= 22.19` | The operator-facing statement. |
...
(`package.json:209`) carries an integrity hash, and `Dockerfile:21`-`:22` activates
exactly that version through corepack. `README.md:109` asks for `>= 10`.
...
(`lib/document/extractors/local-media.ts:180`-`:199`): every `PATH` entry, then
...
(`local-media.ts:818`-`:820`), which is what lets
...
rather than a skip (`ci.yml:274`, `:279`).

## PostgreSQL

| Where | Version | Notes |
| --- | --- | --- |
| `docker-compose.yml:47` | `postgres:16` | Behind the `server-persistence` profile, so it is opt-in. Default password `openmaic-dev`, labelled development-only. Volume `openmaic-postgres`. |
| `.github/workflows/storage-pg-contract.yml:20` | `postgres:16` service | The storage contract database. |
| `.github/workflows/publish-packages.yml:87` | `postgres:16` service | Release `validate` runs the PG contract suites with `STORAGE_PG_CONTRACT_REQUIRED=1`, because those suites skip themselves without a database and publishing `storage` without ever running them would ship the one backend only a real PostgreSQL can confirm (`:82`-`:84`). |

No minimum version is declared in code. Twenty tables are created by five
idempotent `ensure*Schema` calls at first use
(`lib/persistence/server-provider.ts:36`-`:67`). Inferred: the floor is whatever
...
1. **The entrypoint fails closed.** Three separate `FATAL exit 1` paths rather
   than a degraded start (`render-service/docker-entrypoint.sh:51`-`:63`). The
...
3. **Every apt package is pinned to an exact Debian version from one dated,
   ...
   (`render-service/Dockerfile:18`), covering `chromium-common` and
...
| --- | --- | --- |
| **IndexedDB** | every browser storage backend: `maic-documents`, `maic-runtime`, `maic-asset-pool`, the Dexie `MAIC-Database` | `lib/document-store/store.ts:47`-`:50` probes the *capability*, not the environment, so an injected fake works. No fallback store. |
| **localStorage** | `BrowserKVStore` under `maic:account:*` and `maic:device:*`; settings and profile persistence; the raw workbench panel preference | `ambientLocalStorage()` catches the throw some privacy modes produce (`lib/store/kv-persist.ts:462`-`:468`); the persist state machine moves that key to `unavailable` and raises a `persist-health` event. |
| **Web Locks (`navigator.locks`)** | chat-storage partitions, learner-key minting, document migration, quiz-attempt serialisation | Five files feature-detect it at six sites (`lib/document-store/migration.ts:227`, `lib/quiz/runtime.ts:219`, `lib/runtime/learner-key.ts:52`, `lib/utils/chat-storage-lock.ts:19`, `lib/utils/chat-storage.ts:220` and `:1435`). `lib/utils/chat-storage-lock.ts` provides an in-process reader/writer queue fallback, but `withPartitionLocks` **throws `ChatStorageLockUnavailableError`** when a cross-realm lock is genuinely required (`lib/utils/chat-storage.ts:230`-`:234`). `lib/runtime/learner-key.ts:61`-`:66` documents its accepted residual race: a tab whose re-read lands before another tab's write keeps an orphaned key, splitting one anonymous learner's local history. |
| **`indexedDB.databases()`** | probed before deleting the runtime DB so a device that never wrote runtime data is not made to create it (`lib/runtime/store.ts:63`-`:69`) | Assumed present; older Firefox lacks it. |
| **EventSource** | the owner-level session stream and per-session event stream that feed the workbench fold | Abstracted behind `OwnerEventSource` for tests (`lib/workbench/owner-session-client.ts:40`-`:46`). |
| **`crypto.subtle`** | Edge-compatible HMAC verification of the access-code cookie in `middleware.ts:26`-`:35`; `node:crypto` `createHmac`/`timingSafeEqual` on the Node side | Required wherever `ACCESS_CODE` is set. |
| **`speechSynthesis`** | narration fallback with sentence-level chunking, and a cancel+re-speak pause because Firefox's `speechSynthesis.pause` is broken (`lib/playback/engine.ts:246`) | Without it and without server TTS there is no narration. |
| **Fullscreen + Keyboard Lock** | presentation mode: `stageElement.requestFullscreen()` then `navigator.keyboard?.lock(['Escape'])` so Escape does not auto-exit (`components/edit/PlaybackChromeRoot.tsx:589`-`:593`) | Optional-chained and `.catch(() => {})`; presentation still works, Escape just exits fullscreen. |

### Chrome-family engine for export

The slide-to-PNG path is **native-paint-first** by design
(`packages/@openmaic/renderer/src/snapshot/index.ts:10`-`:24`): `html-to-image`
...
| --- | --- | --- |
| `public/vendor/video-export/fonts/` | 25 files, 2.0 MiB | `pnpm gen:video-export-katex` (20 KaTeX WOFF2 faces, count asserted at `scripts/generate-video-export-katex.mjs:36`), `gen:video-export-noto-cjk`, `gen:video-export-noto-script-fonts` |
