...
**Sources:** `package.json:169`-`:202`, the six `packages/@openmaic/*/package.json`
...
| --- | --- | --- | --- |
| `typescript` | `^5` | `npx tsc --noEmit` | `strict: true` (`tsconfig.json:7`). Six `tsc` invocations across CI: the root `--noEmit` in the parallel linter fan (`ci.yml:130`), `@openmaic/generation run typecheck` and `@openmaic/storage run typecheck` — **each of which is two invocations**, `tsconfig.json` then `tsconfig.test.json` — and `npm run typecheck` in the render-service job (`ci.yml:216`). `next build` uses `tsconfig.build.json` instead of `tsconfig.json` when `NODE_ENV=production` (`next.config.ts`). |
...
| `semver` | `7.8.5` **exact** | `pnpm check:node-engine`, `.github/scripts/check-clawhub-version.mjs` | `scripts/check-node-engine-contract.mjs:4` uses `semver.minVersion` to prove the root `engines.node` floor is not below any direct production dependency's. Also installed globally in the skill-publish workflow. |
...
(`.github/workflows/ci.yml:125`-`:131`, with the reason in the step comment).
...
| `fake-indexeddb` | `^6.2.5` | IndexedDB shim in the storage test setup. Works because `lib/document-store/store.ts:47`-`:50` probes the *capability* rather than sniffing the environment, so an injected fake is indistinguishable from a real browser. |
| `pg` | `^8.16.3` (a runtime dep) | `scripts/assert-pg-contract-suites.mjs:138` reaches it via `createRequire` from the storage package to read `pg_stat` `n_tup_ins` deltas **from outside** the Vitest process — the only way to prove a contract suite actually inserted rows, because everything under `test/` is an ignored publishable input. |
| `hyperframes` | `0.7.60` **exact** | CLI lint over seven materialised video-export samples in the e2e job. Exact because the check string-matches its output: version 0.7.60 reports warning-only lint with exit status **zero**, so `ci.yml:307` additionally greps for the literal `0 errors, 0 warnings`. |
...
| `@rollup/plugin-node-resolve`, `@rollup/plugin-commonjs` | `^16`, `^28` | Declared at root **for the vendored forks**, whose own configs import them (`packages/pptxgenjs/rollup.config.mjs:2`-`:3`, `packages/mathml2omml/rollup.config.js:1`). The `@openmaic` packages declare their own copies. |
| `rollup-plugin-typescript2` | `^0.36.0` | Only `packages/pptxgenjs/rollup.config.mjs:4`. Kept at root because the fork's config is upstream's. |
...
| `ts-json-schema-generator` | `~2.4.0` (dsl devDep) | `packages/@openmaic/dsl/scripts/gen-schema.mjs:5` emits `dist/schema/{stage,scene,action}.schema.json` at build time. Build-only **so the DSL keeps zero runtime dependencies** — that is the stated reason. |
| `tsx` | `^4.21.0` | Runs all six eval harness entry points (`package.json:28`-`:33`: four `runner.ts` plus the two extra orchestration runners, `answering-runner.ts` and `answer-content-runner.ts`) and, in the render container, the service itself: `render-service` has no build step and `tsx` is a production dependency there. |
| `tailwindcss` + `@tailwindcss/postcss` | `^4` | The whole CSS pipeline; `postcss.config.mjs` declares exactly one plugin. |
| `fontkit` | `2.0.4` **exact** | `scripts/generate-video-export-noto-script-fonts.mjs:4` (`openSync`) — reads font tables to subset the Noto script fonts. Pinned because its output bytes land in a committed asset. |
...
**The renderer's build writes tracked files.** That is why both `ci.yml:107`-`:121`
and `publish-packages.yml:143`-`:157` diff the working tree against `$GITHUB_SHA`
...
- **`@openmaic/renderer` runs `vitest run` but does not declare `vitest`.** Its
  ...
  (`package.json:200`). Every other `@openmaic` package declares its own.
- **Root `gsap` `^3.15.0` has no first-party consumer.** No file under `scripts`,
  ...
  (`tests/video-export/e2e-materialize.test.ts:404` reads that path). Together
...
See [16-development-view](../16-development-view/index.md) for the CI job graph
these tools sit inside, and [14-code-quality](../14-code-quality/index.md) for
