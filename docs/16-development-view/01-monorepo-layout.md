...
**Sources:** `pnpm-workspace.yaml`, `package.json`, [`tsconfig.json:33-41`](tsconfig.json#L33-L41),
[`tsconfig.build.json:4-15`](tsconfig.build.json#L4-L15), `.prettierignore`, `eslint.config.mjs`,
`.gitignore`, [`next.config.ts:5-15`](next.config.ts#L5-L15), `render-service/package.json`,
...
| --- | --- | --- |
| `app/` | 86 | Next.js App Router. Six user-facing routes plus `app/api/**` (69 `route.ts` files). `app/page.tsx` alone is 79 KB. See [`../03-app-and-api/index.md`](docs/03-app-and-api/index.md). |
...
| `eval/` | 39 | Six manual LLM eval harnesses (`tsx` entry points) across four directories, plus `eval/shared/` reporting helpers. Five of the six results directories are gitignored ([`.gitignore:76-80`](.gitignore#L76-L80)); `eval/pbl-v2-planner/results/` is not. |
...
| `public/` | — | Static assets. `public/vendor/maic-importer` is **gitignored** ([`.gitignore:37`](.gitignore#L37)) and produced by `postinstall`. |
...
| `docs/` | — | This documentation set. **Tracked** — the `/docs` entry was removed from `.gitignore` so that CI can see the set and `scripts/check-docs-links.mjs` can gate it ([`07-quality-gates.md`](docs/16-development-view/07-quality-gates.md) §Gate 6). Before that change it was untracked, which is how eighteen dead internal links survived unnoticed. |
...
CI job ([`.github/workflows/ci.yml:189-223`](.github/workflows/ci.yml#L189-L223)).
...
the two. See [`08-ci-workflows.md`](docs/16-development-view/08-ci-workflows.md) for what CI does and
...
| --- | --- | --- | --- |
| root `tsc --noEmit` | [`tsconfig.json:25-41`](tsconfig.json#L25-L41) | `**/*.ts`, `**/*.tsx`, `**/*.mts` | `node_modules`, `dist`, `packages/*/src`, `packages/docs`, `openclaw`, `e2e`, `render-service` |
| production typecheck | [`tsconfig.build.json:4-15`](tsconfig.build.json#L4-L15) | inherits `include` | the above **plus** `tests`, `eval`, `packages/@openmaic/*/test` |
...
1. **`packages/@openmaic/*/src` is inside the root `tsc` run.** The exclude entry
   ...
   production build config ([`next.config.ts:13`](next.config.ts#L13) selects `tsconfig.build.json`
...
2. **Workflow YAML is neither format-checked nor lint-checked.** `.prettierignore`
   ...
   [`06-testing-and-evals.md`](docs/16-development-view/06-testing-and-evals.md).
3. **`e2e/` is checked by nothing.** Excluded from [`tsconfig.json:39`](tsconfig.json#L39) and listed in
   ESLint's `globalIgnores` ([`eslint.config.mjs:53`](eslint.config.mjs#L53), "Playwright e2e tests (not
...
documented in [`03-build-pipeline.md`](docs/16-development-view/03-build-pipeline.md):
...
| [`next.config.ts:15`](next.config.ts#L15) `transpilePackages` | `mathml2omml`, `pptxgenjs`, `@openmaic/importer` | the Next build graph | the forks ship untranspiled or CJS-flavoured output |
| [`next.config.ts:5-11`](next.config.ts#L5-L11) `outputFileTracingIncludes` | `lib/server/agent-runtime/import-pptx-worker.mjs`, `skills/openmaic/**`, `skills/agent-runtime/**` | the standalone bundle | files reached only through dynamic paths that tracing cannot see |

## Open questions

- `openclaw` appears in both [`tsconfig.json:38`](tsconfig.json#L38) and [`.gitignore:9-10`](.gitignore#L9-L10) but no such
...
- [`CONTRIBUTING.md:171-181`](CONTRIBUTING.md#project-structure) describes `packages/` as holding only
