...
**Sources:** `package.json:9-33`, all 17 files under `scripts/`, every
...
| --- | --- | --- |
| `postinstall` | the nine-step build chain — see [`03-build-pipeline.md`](./03-build-pipeline.md) | never by hand; pnpm runs it |
...
| `lint` | `eslint` | before every commit (`CONTRIBUTING.md:80-82` suggests `pnpm lint --fix`) |
...
Notably absent from the pre-PR checklist in `CONTRIBUTING.md:76-85`: `pnpm test`.
...
| `check-package-version-bumps.mjs` | 685 | `<base-ref>` or `--release` | Diff mode: a package whose publishable inputs changed must have its version increased. Release mode: consults the npm registry and refuses to reuse or downgrade | `ci.yml:59-79`, `publish-packages.yml:114-115,375` |
| `check-internal-dependency-ranges.mjs` | 159 | no args | An owned package may appear as another's dependency exactly once, in `dependencies`, as `workspace:^` | `ci.yml:88-89` |
| `check-node-engine-contract.mjs` | 73 | no args | `semver.minVersion(root engines.node)` must satisfy every installed direct production dependency's own `engines.node` | `ci.yml:93-94` via `check:node-engine` |
| `check-i18n-keys.mjs` | 114 | no args | Exact leaf-key set equality across `lib/i18n/locales/*.json` against `en-US.json`; rejects arrays and empty objects as locale values | `ci.yml:131` via `check:i18n-keys` |
...
| `smoke-test-package-tarballs.mjs` | 206 | `<artifact-dir>` (shifts a leading bare `--`) | Installs the packed tarballs into a temp dir and asserts each dependent publishes `@openmaic/dsl` as a deduplicable caret | `publish-packages.yml:237` via `test:package-tarballs` |
| `ci-run-parallel.sh` | 58 | `NAME COMMAND [NAME COMMAND …]` | Runs pairs concurrently, buffers each to `${RUNNER_TEMP}/ci-parallel-$$/<i>.log`, replays them in argument order as Actions groups, exits 1 if any child failed | `ci.yml:41-50` (self-test), `ci.yml:125-131` |
...
| `generation-node-smoke-server.mjs` | 103 | `--port <n>` | A fake OpenAI-compatible endpoint returning a canned outline, scene-content and actions payload; also serves `/health` | `ci.yml:151-178` |
| `generation-node-smoke.mjs` | 109 | `--requirement --endpoint --model [--api-key]` | A pure-Node consumer of `@openmaic/generation` + `@openmaic/dsl` that runs outlines → content → actions → `buildCompleteScene` → `validateScene` and prints JSON | `ci.yml:173-178` |
...
`check-package-version-bumps.mjs:61-69` refuses to run outside a git worktree with
...
`check-package-version-bumps.mjs:8-53` defines "publishable input" as
...
- **`typecheck` is not uniform.** `dsl`, `renderer` and `editor` run a single
  ...
  separately. The comment at `ci.yml:181-184` gives the reason for storage: its
...
- **`importer` has no `typecheck` script at all**, which is why
  `publish-packages.yml:223-225` omits it from the typecheck `--filter` list, and
  why `openmaic-packages.mjs:132-139` checks `--filter` loosely rather than as an
  exact set.
- **Every owned package declares `prepublishOnly: pnpm run build`.** The release
  ...
  (`publish-packages.yml:176`) — because the build already happened in a verified
