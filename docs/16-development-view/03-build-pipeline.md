...
**Sources:** `package.json:9-33`, `scripts/sync-maic-importer.mjs`,
...
`next.config.ts`, `Dockerfile`, `vercel.json`, `.github/workflows/ci.yml:91-121`,
`.github/workflows/publish-packages.yml:117-157`. Evidence:
...
Properties of the chain itself, all readable from `package.json:10`:
...
- **It runs third-party build code**, which is the stated reason the release
  ...
  (`publish-packages.yml:78-81`).

It runs in six places: locally on `pnpm install`, in `ci.yml:91` (the `check`
job) and `ci.yml:239` (the `e2e` job's own install), in
`storage-pg-contract.yml:44`, in `publish-packages.yml:117`, and in the Docker
`deps` stage (`Dockerfile:46`). The `e2e` one matters: its `sync-maic-importer`
step produces the vendor bundle that `pnpm build` at `ci.yml:328` then asserts
...
`ci.yml:107-121` therefore diffs the tree after install, and the reference point
...
`publish-packages.yml:143-157` runs the same check with no pathspec — whole tree —
after the release build, and `publish-packages.yml:338-350` runs it a third time
...
(`package.json:16`). The assertion is 36 lines and checks one thing: that
...
commands (`assert-vendor-maic-importer.mjs:28-33`):

```
pnpm --filter @openmaic/importer build && pnpm run sync:maic-importer
```

`public/vendor/maic-importer` is gitignored (`.gitignore:37`), so it exists only
after `postinstall` has run — which is why the Docker `builder` stage re-copies
it out of the `deps` stage *after* `COPY . .` (`Dockerfile:74-77`), rather than
relying on the source checkout.

### Build-time TypeScript config swap

`next.config.ts:12-14` selects the tsconfig by `NODE_ENV`:
...
`npx tsc --noEmit` (the CI linter step, `ci.yml:130`) does.

### Output mode

`next.config.ts:4` — `output: process.env.VERCEL ? undefined : 'standalone'`.
The `standalone` bundle is what `Dockerfile:105-106` copies into the runner
...
`outputFileTracingIncludes` (`next.config.ts:5-11`) force-adds three paths that
...
`gen-schema.mjs:1-5`) over the TS contract and emits three artefacts, one per
root type in its `ROOTS` map (`gen-schema.mjs:15-19`):
...
(`gen-schema.mjs:21-24`) because historical stored documents carry app-owned
...
[`../09-media-and-export/index.md`](../09-media-and-export/index.md) and
`docker-compose.yml:72-78`).
...
gitignored (`.gitignore:37`), so a clean checkout does not contain it, and Docker
...
`ALLOWED_FRAME_ANCESTORS`, which `next.config.ts:38-56` reads inside `headers()`.
