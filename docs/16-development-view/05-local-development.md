...
**Sources:** [`README.md:104-360`](README.md#-quick-start), [`CONTRIBUTING.md:25-47`](CONTRIBUTING.md#prerequisites), `package.json:6-8,15-17,209`,
`.nvmrc`, `.env.example`, `docker-compose.yml`, `Dockerfile`,
[`playwright.config.ts:23-36`](playwright.config.ts#L23-L36), `next.config.ts`, `vercel.json`,
[`lib/config/feature-flags.ts:33`](lib/config/feature-flags.ts#L33), [`lib/server/config-validation.ts:181`](lib/server/config-validation.ts#L181).

## Prerequisites

| Requirement | Declared where | Value |
| --- | --- | --- |
| Node.js | [`package.json:6-8`](package.json#L6-L8) `engines.node` | `>=22.19.0` |
| Node.js (tool hint) | `.nvmrc` | `22` |
| pnpm | [`package.json:209`](package.json#L209) `packageManager` | `pnpm@10.28.0` with an embedded sha512 integrity hash |
| pnpm (doc floor) | [`README.md:109`](README.md#prerequisites) | `>= 10` |
| Provider credentials | `.env.example` | at least one LLM key; all 124 uncommented assignments are optional |

`corepack` is the intended activation path — [`Dockerfile:21-22`](Dockerfile#L21-L22) does
...
[`07-quality-gates.md`](docs/16-development-view/07-quality-gates.md).
...
`postinstall` chain described in [`03-build-pipeline.md`](docs/16-development-view/03-build-pipeline.md),
which builds two vendored forks and all six owned packages before the dev server
can start. `pnpm.ignoredBuiltDependencies` ([`package.json:203-207`](package.json#L203-L207)) suppresses
...
passed as a Docker build arg ([`Dockerfile:51-72`](Dockerfile#L51-L72)).

Two flags are also cross-checked at boot: [`lib/server/config-validation.ts:181`](lib/server/config-validation.ts#L181)
...
| --- | --- | --- |
| 3000 | `next dev` / `next start` / the Docker image | Next default; [`Dockerfile:90`](Dockerfile#L90) `ENV PORT=3000`, [`docker-compose.yml:22-23`](docker-compose.yml#L22-L23) maps `3000:3000` |
| 3002 | Playwright's web server | [`playwright.config.ts:35`](playwright.config.ts#L35) `env: { PORT: '3002' }`, `use.baseURL` at [`:13`](playwright.config.ts#L13) |
| 5432 | PostgreSQL under the `server-persistence` profile, container-internal only | [`docker-compose.yml:46-64`](docker-compose.yml#L46-L64) declares no `ports:`/`expose:`, so nothing is published to the host and `DATABASE_URL` must reach it as `postgres:5432`. 5432 is published to the host only by the CI service containers ([`storage-pg-contract.yml:24-25`](.github/workflows/storage-pg-contract.yml#L24-L25), [`publish-packages.yml:91-92`](.github/workflows/publish-packages.yml#L91-L92)) |
| 9000 | `render-service` under the `video-export` profile | [`docker-compose.yml:83-90`](docker-compose.yml#L83-L90), reached as `http://render-service:9000` |
| 43127 | the generation Node-consumer smoke server in CI | [`ci.yml:155`](.github/workflows/ci.yml#L155) |

`pnpm test:e2e` locally reuses an existing server on 3002 if one is running
([`playwright.config.ts:30`](playwright.config.ts#L30) `reuseExistingServer: !process.env.CI`) and otherwise
...
`pnpm dev` is bare `next dev` ([`package.json:15`](package.json#L15)), with no port flag and no
`--turbopack`/`--webpack` override. Two consequences documented elsewhere in the
tree:

- **The bundler is Turbopack.** [`scripts/sync-maic-importer.mjs:5-9`](scripts/sync-maic-importer.mjs#L5-L9) exists
...
and with `tsconfig.json` otherwise ([`next.config.ts:12-14`](next.config.ts#L12-L14)), so `pnpm dev` sees
...
- **`env_file: .env.local` is not optional.** Compose fails if the file is absent,
  which is why [`README.md:321-323`](README.md#docker-deployment) puts `cp .env.example .env.local` before
  `docker compose up`.
- **`RENDER_SERVICE_URL` is set in the compose `environment:` block**, so it wins
  over any value in `.env.local` ([`docker-compose.yml:26-33`](docker-compose.yml#L26-L33)). The comment there is
...
- **The PostgreSQL password default is labelled development-only**
  ([`docker-compose.yml:53-55`](docker-compose.yml#L53-L55)), overridable via `PERSISTENCE_POSTGRES_PASSWORD`.
- **`NEXT_PUBLIC_*` must be supplied as compose build args**, not through
  `.env.local`. [`docker-compose.yml:8-21`](docker-compose.yml#L8-L21) plumbs ten of them plus
  `ALLOWED_FRAME_ANCESTORS`. The server-persistence example at
  [`README.md:368-372`](README.md#server-backed-persistence-postgresql) demonstrates the pattern: `NEXT_PUBLIC_PERSISTENCE=1
...
- **Slow-network mirrors** are two optional build args, `ALPINE_MIRROR` (hostname,
  ...
  ([`Dockerfile:6-7`](Dockerfile#L6-L7), [`README.md:326-360`](README.md#slow-network--china-build-acceleration)). Both are trailing-slash-stripped in a
  shell loop. [`README.md:335-337`](README.md#slow-network--china-build-acceleration) warns against embedding credentials in them,
...
[`README.md:390-397`](README.md#server-backed-persistence-postgresql) is worth reading before any non-localhost use of server
...
CI mirrors exactly this in its own job ([`ci.yml:189-223`](.github/workflows/ci.yml#L189-L223)) with
...
- [`README.md:109`](README.md#prerequisites) says pnpm `>= 10` while `packageManager` pins `10.28.0` exactly.
