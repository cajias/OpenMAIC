...
`lib/server/provider-config.ts`, `lib/config/feature-flags.ts`, `lib/ai/providers.ts:2347-2365`,
`.env.example:340-372`;
[../appendix/research/ai-provider-layer/03-flows.md](../appendix/research/ai-provider-layer/03-flows.md),
[../appendix/research/ai-provider-layer/05-failure-modes.md](../appendix/research/ai-provider-layer/05-failure-modes.md).
...
1. Validation is imported **dynamically** (`instrumentation.ts:28`) precisely so the Edge bundle
...
dropped, see [./03-stage-routing.md](./03-stage-routing.md)), so a missing key there is a hard
warning. On `DEFAULT_MODEL` the same condition is only a note because the browser's own key can
still make the request work.

`warnBareModelIdDeprecation` (`lib/ai/providers.ts:2359`) dedupes on a module-level
...
a `defaultBaseUrl` (`lib/ai/providers.ts:1508`), so it counts as configured.

Because `TENCENT` and `TENCENT_HUNYUAN` both map to `tencent-hunyuan`, and `XIAOMI` and `MIMO` both
map to `xiaomi` (`lib/server/provider-config.ts:88`–`:91`), setting the pin under one prefix and
...
non-empty `DATABASE_URL` and is what `instrumentation.ts:38` gates the runner on.

## The one strict contract: the agent driver route

`assertAgentDriverRouteConfig` (`lib/server/agent-runtime/agent-driver-model.ts:14`) is the only
...
| --- | --- |
| `lib/server/config-validation.ts:192` | wrapped in try/catch → downgraded to a `[config]` warning; the server boots and the runner starts |
| `lib/server/agent-runtime/agent-driver-model.ts:92` | **not** caught → `resolveAgentDriverModel()` rejects, and `runSession` fails at `lib/server/agent-runtime/runner.ts:1262` |
...
notice. `.env.example:355`–`:365` calls the route "REQUIRED while the runtime is enabled" and spells
out the constraints.
...
| --- | --- |
| That a routed model id exists in the provider's catalog | Nowhere. An unknown model id is sent to the provider verbatim; a 404 comes back as `Upstream endpoint not found.` (`lib/server/llm-error-response.ts:53`) |
| That `server-providers.yml` parses | `loadYamlFile` returns an empty object on any error and warns from `provider-config.ts:226`; every provider then becomes unmanaged |
| That a managed provider's key is valid | Only `POST /api/verify-model` — see [./06-verify-routes.md](./06-verify-routes.md) |
| That `api`/`contextWindow` on a non-driver stage will ever be read | Nothing warns; they are inert by design (`lib/server/model-routes.ts:48`–`:51`) |
| That `DEFAULT_MODEL` is set at all | Only the request throws, from `lib/server/resolve-model.ts:66` |
| Anything on the Edge runtime | `instrumentation.ts:16` returns before validating |
...
`MODEL_ROUTES` produces **two** log lines — a `[config]` warning at boot (`config-validation.ts:110`)
and a `model-routes` `log.error` on first use (`model-routes.ts:237`). Same for an unknown stage key
(`:225`).
