...
1. **Untrusted HTML in a real browser.** The export ZIP is fully self-contained,
   ...
   at the kernel (`docker-entrypoint.sh:4-10`).
2. **Binary toolchain the app does not want.** Chromium headless shell + FFmpeg on
   ...
   (`Dockerfile:3-5`). Producer's BeginFrame capture requires the *old* headless
...
4. **Swappable internals behind a stable contract.** `main.ts:8-10` says the
...
(`main.ts:19-23`). The service uses producer as a library, so the entry is
`main.ts`.
...
Stated verbatim in the file header (`main.ts:12-17`):
...
`/health` (`main.ts:241-250`) publishes `accepting` as an **aggregate boolean
...
addresses (`render-coordinator.ts:123-129`). `versions` is `RuntimeVersions`:
`{ service, producer, node, chromium, chromiumPath, ffmpeg, ffmpegPath,
containerImage }` (`types.ts:26`), collected once at boot by
`collectRuntimeVersions()` and copied into every job's metrics.

`RenderOptions` (`types.ts:18`) is `{ fps: number; quality: 'draft' | 'standard'
| 'high'; format: 'mp4' }`. `format` is a single-member union stated as explicit
forward-compat.

## 3. Admission ordering is the security boundary

`createApp(deps)` (`main.ts:229`) documents the rule above itself
...
Archive rejection (`unzip.ts:31`) happens on **declared** sizes via a synchronous
...
The split is explicit and stated at `build-export-zip.ts:11-13`: the compile and
...
`lib/store/video-render.ts:243-269` is deliberately narrow, and the comment states
...
- `NEXT_PUBLIC_ENABLE_VIDEO_EXPORT` is read only in
  `lib/config/feature-flags.ts:122` (client). Nothing under
...
- `RENDER_SERVICE_URL` is deliberately exempt from `validateUrlForSSRF`
  (`lib/server/render-service.ts:25-35`) because it is meant to point at an internal
