...
| Topic | [../03-app-and-api/index.md](../03-app-and-api/index.md) · pack: [app-shell-and-routing](../appendix/research/app-shell-and-routing/00-overview.md) |
...
non-empty `DATABASE_URL` (`lib/config/feature-flags.ts:18-25`), and 26 routes gate
on the latter.
...
| Topic | [../03-app-and-api/index.md](../03-app-and-api/index.md), [../12-api-reference/index.md](../12-api-reference/index.md) · pack: [api-surface](../appendix/research/api-surface/00-overview.md) |
...
| Key artefacts | `PROVIDERS` (`lib/ai/providers.ts:75`), `getModel()` (`:2033`), `callLLM` / `streamLLM` (`lib/ai/llm.ts:325,397`), `lib/server/provider-config.ts`, `lib/server/model-routes.ts` |
...
| Topic | [../04-ai-provider-layer/index.md](../04-ai-provider-layer/index.md) · pack: [ai-provider-layer](../appendix/research/ai-provider-layer/00-overview.md) |

The boundary is machine-enforced in both directions: `eslint.config.mjs:608-667`
...
| Key artefacts | `startAgentRunner()` (`lib/server/agent-runtime/runner.ts:1861`), `runSession()` (`:889`), `buildAgent()` (`lib/agent/runtime/build-agent.ts:65`), `HOST_AGENT_LIFECYCLE` (`lib/agent-runtime/lifecycle.ts:37`), `STAGE_WRITER_TOOL_NAMES` (`lib/agent-runtime/stage-writer-tools.ts:20`) |
...
| Topic | [../05-agent-runtime/index.md](../05-agent-runtime/index.md) · pack: [agent-runtime](../appendix/research/agent-runtime/00-overview.md) |
...
| Key artefacts | `@openmaic/generation` (8 229 lines src, 26 Markdown prompt templates), `buildDocumentBundle` (`lib/document/bundle.ts:181`), the browser loop (`lib/hooks/use-scene-generator.ts`) and the headless twin (`lib/server/classroom-generation.ts`) |
...
| Topic | [../06-generation-pipeline/index.md](../06-generation-pipeline/index.md) · pack: [generation-pipeline](../appendix/research/generation-pipeline/00-overview.md) |
...
| Topic | [../07-dsl-renderer-editor/index.md](../07-dsl-renderer-editor/index.md) · pack: [dsl-renderer-editor](../appendix/research/dsl-renderer-editor/00-overview.md) |
...
envelope (`packages/@openmaic/dsl/src/version.ts:589`).
...
| Topic | [../08-classroom-runtime/index.md](../08-classroom-runtime/index.md) · pack: [classroom-runtime](../appendix/research/classroom-runtime/00-overview.md) |
...
| Key artefacts | `generateTTS()` (`lib/audio/tts-providers.ts:207`), `resolveActionTimeline()` (`lib/choreography/timeline.ts:282`), `compileVideoTimeline()` (`lib/video-export/compile.ts:152`), `foldWhiteboardRuntimeRecords()` (`lib/whiteboard/runtime/fold.ts:170`), `render-service/src/main.ts` |
...
| Topic | [../09-media-and-export/index.md](../09-media-and-export/index.md) · pack: [media-audio-video](../appendix/research/media-audio-video/00-overview.md) |

Purity here is enforced, not asserted: `eslint.config.mjs:254-492` fences
...
| Topic | [../10-persistence-and-state/index.md](../10-persistence-and-state/index.md) · pack: [persistence-storage-state](../appendix/research/persistence-storage-state/00-overview.md) |

## 10. Quality, testing, CI, dependencies

| | |
| --- | --- |
| Charter | Stated verbatim at `scripts/openmaic-packages.mjs:17-33`: gates exist to catch **mistakes that are silent by default**, not deliberate subversion, "because every new check arrives with its own editable configuration" |
...
| Topic | [../14-code-quality/index.md](../14-code-quality/index.md), [../16-development-view/index.md](../16-development-view/index.md) · pack: [quality-testing-ci-deps](../appendix/research/quality-testing-ci-deps/00-overview.md) |
...
| `AICallFn` (`packages/@openmaic/generation/src/pipeline-types.ts:60`) | generation ↔ provider layer | `(systemPrompt, userPrompt, images?) => Promise<string>` — the whole model dependency |
| `createCallLlmStreamFn()` (`lib/agent/runtime/stream-fn.ts:250`) | agent runtime ↔ provider layer | pi `StreamFn` adapted onto `streamLLM` with `stopWhen: stepCountIs(1)` |
| `resolveModelFromRequest()` (`lib/server/resolve-model.ts:183`) | API surface ↔ provider layer | header + stage-route arbitration; a routed stage discards client credentials |
| `HOST_AGENT_LIFECYCLE` (`lib/agent-runtime/lifecycle.ts:37`) | agent runtime ↔ browser | 13 durable event names the client subscribes to *by name* |
| `STAGE_WRITER_TOOL_NAMES` (`lib/agent-runtime/stage-writer-tools.ts:20`) | agent runtime ↔ DSL ↔ workbench client | 9 names, 3 consumers, one list |
...
| `createStorageHttpHandler` (`app/api/persistence/[...path]/route.ts:108-121`) plus the `ServerResponse` shim (`nodeRequest` at `:129-144`, `setHeaders` at `:146-154`, closed by the `as unknown as ServerResponse` cast at `:253`) | API surface ↔ persistence | a Node `RequestListener` adapted to a Web `Response` |
| `render-service` HTTP contract (`render-service/src/main.ts:12-17`) | video export ↔ the render container | six endpoints, deliberately minimal so internals can be swapped |
...
- `app/classroom/[id]/page.tsx` runs its own copy of the classroom load pipeline
  ...
  ([classroom-runtime](../appendix/research/classroom-runtime/00-overview.md)).
  Which one is intended to be canonical is not recorded in either file.
- `skills/openmaic/SKILL.md` is consumed by no runtime in this repository — it is
