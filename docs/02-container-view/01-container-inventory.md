...
— those are in [06-bounded-contexts.md](./06-bounded-contexts.md).
...
Evidence: [app-shell-and-routing/04](../appendix/research/app-shell-and-routing/04-dependencies-and-config.md),
[persistence-storage-state/00](../appendix/research/persistence-storage-state/00-overview.md),
[media-audio-video/02g](../appendix/research/media-audio-video/02g-interfaces-render-service.md).
...
`render-service/docker-entrypoint.sh:40-42` installs the iptables `OUTPUT DROP`
...
| Field | Value |
| --- | --- |
| Technology | Next.js 16.2.11, React 19.2.3, Node ≥ 22.19.0 (`package.json:7,119,143`) |
...
| How started | `node server.js` in the runner stage (`Dockerfile:112`) when `output: 'standalone'`; as Vercel functions when `VERCEL` is set (`next.config.ts:4`) |
| Listens on | `0.0.0.0:3000` (`Dockerfile:89-90`); mapped `3000:3000` in compose (`docker-compose.yml:22-23`) |
| Config source | Process env; `.env.local` via compose `env_file` (`docker-compose.yml:24-25`); optional `server-providers.yml`, which an operator must uncomment at `docker-compose.yml:38-39` before it is mounted read-only — the shipped file's only active volume is `openmaic-data:/app/data` at `:40` (parsed by `lib/server/provider-config.ts:8-10`) |
| Runtime flavour | Node only. No route in the tree declares `runtime = 'edge'`; only `middleware.ts` runs on the edge, and it branches on `process.env.NEXT_RUNTIME !== 'edge'` at `middleware.ts:53` because it cannot trust server-only variables there |
...
guarantee and no shutdown hook (`instrumentation.ts:1-12`):
...
pulls in `pg` (`instrumentation.ts:14-20`). The two `process.once` calls live in
...
| Config source | `NEXT_PUBLIC_*` values inlined at **build** time (`lib/config/feature-flags.ts:1-8`), plus the account-scoped settings store persisted to `localStorage` |
...
domain model. See [03-server-client-boundary.md](./03-server-client-boundary.md).

### 3. render-service

| Field | Value |
| --- | --- |
| Technology | Node 22.22.2 (digest-pinned, `render-service/Dockerfile:8`), Hono 4, `@hyperframes/producer` 0.7.107, `puppeteer-core` 25, Chromium headless shell + FFmpeg in-image |
| Responsibility | Frame capture and MP4 encoding only. `POST /render`, `POST /preview`, `GET /render/:jobId`, `GET /render/:jobId/download`, `DELETE /render/:jobId`, `GET /health` (`render-service/src/main.ts:12-17`) |
| How started | `ENTRYPOINT /usr/local/bin/docker-entrypoint.sh` (`render-service/Dockerfile:95`) → iptables lockdown → `setpriv --reuid=render … tsx src/main.ts` (`docker-entrypoint.sh:72`). Compose profile `video-export` only (`docker-compose.yml:81-82`) |
| Listens on | `9000`, `expose`d but **not** published to the host (`docker-compose.yml:83-84`, `render-service/Dockerfile:78,89`) |
| Config source | Its own env block in compose (`docker-compose.yml:89-123`); its own `package.json` / `tsconfig.json` / `vitest.config.ts`; `render-service/src/config.ts` |
| Isolation | On the `render` network, `internal: true` — no route to host or internet (`docker-compose.yml:142-143`); plus its own egress DROP; plus `mem_limit` default `8g` and `shm_size: 2gb` |
| Coupling to the app | Only `RENDER_SERVICE_URL` (`lib/server/render-service.ts:16`). Unset ⇒ the app degrades to a ZIP download rather than erroring (`lib/server/render-service.ts:4-7`) |

`RENDER_SERVICE_URL` is deliberately **not** run through the SSRF guard, with the
reason written down at `lib/server/render-service.ts:26-35`: the URL is *meant*
...
(`render-service/package.json:19-20`) — against `0.11.1` and `0.1.6` currently
on disk (`packages/@openmaic/dsl/package.json`, `.../renderer/package.json`).
Consequences in [05-workspace-packages.md](./05-workspace-packages.md).

### 4. PostgreSQL (relational store)

| Field | Value |
| --- | --- |
| Technology | `postgres:16` image (`docker-compose.yml:47`); `pg` 8.16 pool in-process (`lib/persistence/server-provider.ts:8,72`) |
...
| How started | Compose profile `server-persistence` (`docker-compose.yml:48-49`). Schema is provisioned lazily and idempotently on first use by five `ensure*` calls (`lib/persistence/server-provider.ts:43-47`) |
| Config source | `DATABASE_URL`. Absent ⇒ the persistence route answers `404 PERSISTENCE_NOT_CONFIGURED` (`app/api/persistence/[...path]/route.ts:271-274`) and `isAgentRuntimeConfigured()` is false (`lib/config/feature-flags.ts:23-25`) |
| Pooling | One `Pool` cached on a `globalThis` symbol, keyed by connection string (`lib/persistence/server-provider.ts:30-34,74-77`), closed on drain (`instrumentation.ts:82-92`) |
...
(`lib/persistence/owner-materials.ts:103`) — 16 in the diagram plus these four is
...
`lib/persistence/server-provider.ts:49` via
`lazyAssetByteStore(process.env.ASSET_S3_BUCKET, queryable)`:

| Layer | Selected when | Notes |
| --- | --- | --- |
| S3 bucket | `ASSET_S3_BUCKET` set | `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` are **optional peers** of `@openmaic/storage` (`packages/@openmaic/storage/package.json` `peerDependenciesMeta`), externalised in `next.config.ts:32-33`. Region/endpoint/credentials come from the standard AWS chain (`.env.example:505-507`) |
...
(`app/api/persistence/[...path]/route.ts:63-77`).

### 6. Server local filesystem

The compose volume `openmaic-data:/app/data` (`docker-compose.yml:40`) backs
three unrelated trees, all rooted at `process.cwd()/data`:

| Path | Written by | Content |
| --- | --- | --- |
| `data/usage/<YYYY>-<MM>.jsonl` | `lib/server/usage-storage.ts:10,17` | Append-only token-usage metering. The one entity with a filesystem backend |
| `data/classrooms/` | `CLASSROOMS_DIR`, `lib/server/classroom-storage.ts:6` | Persisted classroom JSON + media, served with Range support by `app/api/classroom-media/[classroomId]/[...path]/route.ts` |
| `data/classroom-jobs/` | `CLASSROOM_JOBS_DIR`, `lib/server/classroom-storage.ts:7` | Headless generation job rows for the `/api/generate-classroom` poll loop |

Writes go through `writeJsonFileAtomic` (temp file + `rename`,
`lib/server/classroom-storage.ts:21-29`).

### 7. Browser-local storage tier

Four independent browser databases plus two web-storage areas. This is the
**default** deployment mode: `lib/persistence/bootstrap.ts:15-17` only switches
...
| Route handoffs | `sessionStorage` | `generationSession`, `generationParams`, `workbench.launchPrompt` | none — neither is schema-validated ([app-shell-and-routing/00](../appendix/research/app-shell-and-routing/00-overview.md)) |

`initDatabase()` calls `navigator.storage?.persist?.()` to ask the browser not to
evict under storage pressure (`lib/utils/database.ts:578-584`) — the media blobs
...
`http://localhost:11434/v1`, `lib/ai/providers.ts:1508`; `lemonade-tts` at
`http://127.0.0.1:8000`, `lib/audio/constants.ts:794`; `comfyui-image` at
`http://localhost:8188`, `lib/media/image-providers.ts:132`). The two exceptions
...
server." (`lib/audio/tts-providers.ts:246-249`, same shape at
`lib/audio/asr-providers.ts:179`). Counts are from the registry object literals,
not from documentation:

| Family | Count | Registry | Selection |
| --- | --- | --- | --- |
| Text LLM | 19 built-in + `custom-*` | `PROVIDERS`, `lib/ai/providers.ts:75`; ids in `BuiltInProviderId`, `lib/types/provider.ts:8-27` | `lib/server/resolve-model.ts:63-65`: stage route > `x-model` > `DEFAULT_MODEL` |
| TTS | 10 — 9 server-reachable + `browser-native-tts` | `TTS_PROVIDERS`, `lib/audio/constants.ts:119` | `generateTTS(config, text)` |
| ASR | 6 — 5 server-reachable + `browser-native` | `ASR_PROVIDERS`, `lib/audio/constants.ts:1078` | per-request |
| Image generation | 8 | `IMAGE_PROVIDERS`, `lib/media/image-providers.ts:33` | `generateImage()` |
| Video generation | 6 | `VIDEO_PROVIDERS`, `lib/media/video-providers.ts:22` | per-request |
| Web search | 9 | exhaustive `switch` in `searchWeb()`, `lib/web-search/index.ts` | `providerId` argument |
| Document extraction | 4 PDF + 1 plain-text | `PDF_PROVIDERS`, `lib/pdf/constants.ts:14`; registry order in `lib/document/extractors/registry.ts` | first provider whose MIME + capability set matches |
...
where the URL really is client-supplied (`lib/server/resolve-model.ts:83-110`).

## Where configuration enters, per container

| Container | Build-time input | Run-time input | Notes |
| --- | --- | --- | --- |
| Browser bundle | 10 `NEXT_PUBLIC_*` build args plus `ALLOWED_FRAME_ANCESTORS`, 11 `ARG`s in all (`Dockerfile:51-72`), mirrored in compose `build.args` (`docker-compose.yml:5-21`). `ALLOWED_FRAME_ANCESTORS` is a server-side header input, not a bundle-inlined value | none | A `NEXT_PUBLIC_*` value supplied only via `env_file` reaches the *runtime* container, too late to be inlined |
| Next server | `tsconfig.build.json` selection at `next.config.ts:13`; `output` at `:4` | full process env + optional `server-providers.yml` | `vercel.json` sets `functions["app/api/**/*.ts"].maxDuration = 300` — pages get no such budget |
| render-service | its own image build | its own compose env block | Shares nothing with the app's env |
| PostgreSQL | image | `POSTGRES_DB/USER/PASSWORD` (`docker-compose.yml:50-55`) | Dev default password `openmaic-dev` |

Truthiness for every flag is strict: `readBoolean` accepts only the exact strings
`'true'` and `'1'` (`lib/config/feature-flags.ts:10-12`), so `'TRUE'`, `'yes'`
and `'on'` are all disabled.

Per-environment topology, resource sizing and the Vercel-versus-standalone split
are covered in [../17-deployment-view/index.md](../17-deployment-view/index.md);
the full env-var catalogue is in
[../15-cross-cutting/index.md](../15-cross-cutting/index.md) and
[../13-dependencies/index.md](../13-dependencies/index.md).

## Open questions

- **`NEXT_PUBLIC_PRO_WORKBENCH_ENABLED` has no build path in Docker.** It is
  read at `lib/config/feature-flags.ts:33` but appears in neither `Dockerfile:51-72`
  nor `docker-compose.yml:5-21`. Inferred consequence: a Compose build cannot
...
- No health check is declared for the `openmaic` service in compose (only
  `postgres` has one, `docker-compose.yml:56-61`), and `render-service` has none
...
- `docs/` used to be listed in `.gitignore`, making this documentation set untracked. That
  ...
  is [`../16-development-view/07-quality-gates.md`](../16-development-view/07-quality-gates.md)
