...
| --- | --- | --- | --- |
| **stage** | `interface Stage` | [`packages/@openmaic/dsl/src/stage.ts:141`](packages/@openmaic/dsl/src/stage.ts#L141) | The whole course document: `id`, `name`, optional `languageDirective` / `style`, a stage-level `whiteboard: Whiteboard[]`, a `videoManifest`, and the persisted agent roster (`agentIds` or `generatedAgentConfigs`). |
...
| **action** | `type Action` | [`packages/@openmaic/dsl/src/action.ts:235`](packages/@openmaic/dsl/src/action.ts#L235) | One playback verb. A 21-member discriminated union on `type`. |
| **utterance** | *not a domain type* | — | There is no `Utterance` in the codebase. The only identifier is the browser's `SpeechSynthesisUtterance` ([`lib/playback/engine.ts:798`](lib/playback/engine.ts#L798)). See [§ Utterance](#utterance-the-word-with-no-type) below. |

`SceneType` is a closed four-member union — `'slide' | 'quiz' | 'interactive' |
'pbl'` ([`stage.ts:22`](packages/@openmaic/dsl/src/stage.ts#L22)) — mirrored by a `SCENE_TYPES` tuple with a compile-time
exhaustiveness assertion ([`stage.ts:35-37`](packages/@openmaic/dsl/src/stage.ts#L35-L37)) so adding a member without updating
the tuple fails the build. `StageMode` is a separate three-member union
`'autonomous' | 'playback' | 'edit'` ([`stage.ts:45`](packages/@openmaic/dsl/src/stage.ts#L45)).
...
1. **`Scene` is not `SceneCore & { content }` by intersection.** It is a
   distributive conditional over the content union ([`stage.ts:281-283`](packages/@openmaic/dsl/src/stage.ts#L281-L283)), so each
   member ties *its own* `type` to *its own* `content` shape. A
   `{ type: 'quiz', content: SlideContent }` value is unrepresentable.
   `InteractiveContent` ([`packages/@openmaic/dsl/src/interactive.ts:51`](packages/@openmaic/dsl/src/interactive.ts#L51)) and
   `PBLContent` ([`packages/@openmaic/dsl/src/pbl.ts:151`](packages/@openmaic/dsl/src/pbl.ts#L151)) are contract-owned but
   composed in through `Scene`'s generic content parameter — the default is only
   `SlideContent | QuizContent` ([`stage.ts:280`](packages/@openmaic/dsl/src/stage.ts#L280)).
2. **`whiteboards` on a scene are `Slide[]`, not scene content.** A whiteboard is
   ...
   ([`stage.ts:51`](packages/@openmaic/dsl/src/stage.ts#L51)). Stage-level and scene-level whiteboards both exist; the
   whiteboard the runtime mutates is resolved through `lib/api/stage-api`'s
   `whiteboard.get()` ([`lib/action/engine.ts:301`](lib/action/engine.ts#L301)).

## The 21 actions, grouped by how playback treats them

`FIRE_AND_FORGET_ACTIONS = ['spotlight', 'laser']` and `SYNC_ACTIONS` (19
members) partition the union at [`action.ts:261`](packages/@openmaic/dsl/src/action.ts#L261) and `:267`. The partition is data,
not a hardcoded branch: [`lib/choreography/timeline.ts:51`](lib/choreography/timeline.ts#L51) builds its blocking set
from `FIRE_AND_FORGET_ACTIONS` precisely so the two cannot drift.

| Group | Members | Playback behaviour |
| --- | --- | --- |
| Fire-and-forget effects | `spotlight`, `laser` | Dispatched, then `queueMicrotask` straight to the next action ([`lib/playback/engine.ts:672`](lib/playback/engine.ts#L672)). Cleared by one shared 5 s timer. |
| Narration | `speech` | Handled by `PlaybackEngine` itself, never through `ActionEngine` on the playback path. Owns the clock ([`engine.ts:584`](lib/playback/engine.ts#L584)). |
| Whiteboard mutations (12) | `wb_open`, `wb_draw_text`, `wb_draw_shape`, `wb_draw_chart`, `wb_draw_latex`, `wb_draw_table`, `wb_draw_line`, `wb_draw_code`, `wb_edit_code`, `wb_clear`, `wb_delete`, `wb_close` | `await ActionEngine.execute(action)` ([`engine.ts:735`](lib/playback/engine.ts#L735)); each has a fixed animation dwell. The only group replayed during a seek. |
| Video | `play_video` | Blocking; waits for the media task to become playable, then for the element to stop playing, capped at `MAX_VIDEO_WAIT_MS` ([`lib/action/engine.ts:424`](lib/action/engine.ts#L424)). |
| Discussion | `discussion` | Blocking in a different sense: it schedules a 3 s timer, shows a card, and then the engine idles with **no timer of its own** ([`engine.ts:706-713`](lib/playback/engine.ts#L706-L713)). |
| Widget messages (4) | `widget_highlight`, `widget_setState`, `widget_annotation`, `widget_reveal` | `postMessage` into the interactive iframe plus a 300 ms dwell ([`lib/action/engine.ts:872-901`](lib/action/engine.ts#L872-L901)). |

`SLIDE_ONLY_ACTIONS` ([`action.ts:264`](packages/@openmaic/dsl/src/action.ts#L264)) is a third, smaller list — `spotlight` and
...
- **cursor** — the `(sceneIndex, actionIndex)` pair private to `PlaybackEngine`
  ([`engine.ts:64-65`](lib/playback/engine.ts#L64-L65)). Resolved to a concrete action by the pure
  `resolvePlaybackCursor` ([`lib/choreography/cursor.ts:44`](lib/choreography/cursor.ts#L44)), which advances past
  exhausted scenes and yields one synthetic `EMPTY_SCENE_DWELL` beat for a scene
  with `actions: []` ([`cursor.ts:19`](lib/choreography/cursor.ts#L19), `:54`) so a blank slide still shows.
  `sceneIndex` is effectively dead in the app: the only production constructor
  passes `[currentScene]` ([`components/edit/PlaybackChromeRoot.tsx:759`](components/edit/PlaybackChromeRoot.tsx#L759)). The
  multi-scene walk exists for the video exporter.
- **generation** — `playbackGeneration`, a monotonic counter
  ([`engine.ts:96`](lib/playback/engine.ts#L96), `:493`). Every state transition bumps it; every async
...
- **snapshot** — `PlaybackSnapshot` ([`lib/playback/types.ts:5`](lib/playback/types.ts#L5)), emitted through
  `onProgress` **before** the cursor advances ([`engine.ts:579`](lib/playback/engine.ts#L579)), so a restored
...
| --- | --- | --- | --- |
| A pre-authored narration line | `SpeechAction` ([`action.ts:47`](packages/@openmaic/dsl/src/action.ts#L47)) with an `audioId?: AssetRef` | inside `scene.actions` in the document | Pre-generated bytes resolved at play time by `AudioPlayer.play` ([`lib/utils/audio-player.ts:99`](lib/utils/audio-player.ts#L99)); falls back to browser TTS, then to a timer |
| A live agent's spoken segment | a `TextItem` in the `StreamBuffer` queue ([`lib/buffer/stream-buffer.ts:35`](lib/buffer/stream-buffer.ts#L35)), sealed then queued | in memory only, driven by SSE deltas | Synthesised per segment by `useDiscussionTTS` after `onSegmentSealed` ([`lib/hooks/use-discussion-tts.ts:351`](lib/hooks/use-discussion-tts.ts#L351)) |
...
`SpeechSynthesisUtterance` ([`engine.ts:757`](lib/playback/engine.ts#L757), `:798`), because Chrome silently
...
The canonical table for the whole set is [`../glossary.md`](docs/glossary.md); what follows
...
| Sense | What it is |
| --- | --- |
| Stage-the-document | the course document (`Stage`, [`packages/@openmaic/dsl/src/stage.ts:141`](packages/@openmaic/dsl/src/stage.ts#L141)) — the unit of persistence |
...
| --- | --- | --- |
| `LlmStage` | one of the 20 routable *model-selection* keys — `'scene-outlines-stream'`, `'scene-content:quiz'`, `'chat-adapter'`, `'maic-agent-driver'`, … `LLM_STAGES` at [`lib/server/model-routes.ts:131`](lib/server/model-routes.ts#L131), the type at [`:154`](lib/server/model-routes.ts#L154). It selects a **model**, not a document | [`../04-ai-provider-layer/03-stage-routing.md`](docs/04-ai-provider-layer/03-stage-routing.md) |
| stage-as-pipeline-step | prose only, no type: "stage 2 produces the outline". Four steps | [`../06-generation-pipeline/01-pipeline-overview.md`](docs/06-generation-pipeline/01-pipeline-overview.md) |
...
- [`./02-playback-state-machine.md`](docs/08-classroom-runtime/02-playback-state-machine.md) — what the
