...
`lib/server/agent-runtime/fetch-url.ts:140-174`,
`lib/server/web-search-config.ts:12-79`, `app/api/proxy-media/route.ts`,
`lib/server/resolve-model.ts:104-110`,
`lib/server/agent-runtime/generate-image.ts:106-120`,
`lib/server/render-service.ts:25-39`,
...
[`../appendix/research/quality-testing-ci-deps/01b-modules-ci-and-build.md`](../appendix/research/quality-testing-ci-deps/01b-modules-ci-and-build.md).
...
| Asset URL returned by an image/video provider | `generate-image.ts:111`, `generate-video.ts:155` | Z4 provider |
| URL handed to the agent's `fetch_url` tool | `fetch-url.ts:449` | Z6 model output |
...
(`lib/server/web-search-config.ts:12-44`) enumerates the accepted official
...
(`web-search/route.ts:98`, `web-search-config.ts:121`), so a SearXNG base URL can
...
| --- | --- | --- |
| `app/api/proxy-media/route.ts:33` + per-redirect `:55` | `validateUrlForSSRF` | No — always |
| `app/api/azure-voices/route.ts:29` | `validateUrlForSSRF` | No — always |
| `app/api/provider/probe-models/route.ts:34` (both `baseUrl` and `modelsUrl`) | `validateUrlForSSRF` | No — always |
| `app/api/generate/tts/route.ts:98` | `validateUrlForSSRF` | No — always |
| `app/api/generate/voice/route.ts:126` | `validateUrlForSSRF` | No — always |
| `lib/server/agent-runtime/generate-image.ts:111` (per redirect hop) | `validateUrlForSSRF` | No — always |
| `lib/server/agent-runtime/generate-video.ts:155` (per redirect hop) | `validateUrlForSSRF` | No — always |
| `app/api/generate/image/route.ts:70` | `validateUrlForSSRF` | **Yes** — `production` only |
| `app/api/generate/video/route.ts:65` | `validateUrlForSSRF` | **Yes** |
| `app/api/transcription/route.ts:57` | `validateUrlForSSRF` | **Yes** |
| `app/api/parse-pdf/route.ts:47` | `validateUrlForSSRF` | **Yes** |
| `app/api/extract-document/route.ts:258` (media) and `:386` (document) | `validateUrlForSSRF` | **Yes** |
| `app/api/verify-image-provider/route.ts:57` | `validateUrlForSSRF` | **Yes** |
| `app/api/verify-video-provider/route.ts:52` | `validateUrlForSSRF` | **Yes** |
| `app/api/verify-pdf-provider/route.ts:58`, `:83`, `:132` | `validateUrlForSSRF` | **Yes** (all three) |
| `lib/server/resolve-model.ts:105` — covers `chat`, `chat/pi`, `verify-model`, `quiz-grade`, `pbl/v2/*`, `generate/scene-*` | `validateUrlForSSRF` | **Yes** |
| `lib/server/agent-runtime/fetch-url.ts:449` + `:479` | strict + pinned lookup | No — always |
| `app/api/web-search/route.ts:109` and `lib/server/web-search-config.ts:122` (classroom) | per-provider exact allowlist | No — always |
...
| --- | --- |
| `RENDER_SERVICE_URL` | Operator deployment config, meant to name an internal host (`http://render-service:9000`). Guarding it would force operators to weaken SSRF globally. `lib/server/render-service.ts:25-35` |
| A **managed** provider's `baseUrl` | Comes from `server-providers.yml` / `<PREFIX>_BASE_URL`, i.e. the operator. `lib/server/resolve-model.ts:83-87`, `lib/server/provider-config.ts:679-687` |
| `SEARXNG_BASE_URL` | Same reasoning; the route additionally discards any client-supplied SearXNG base URL. `app/api/web-search/route.ts:97-98` |
| `POST /api/web-search` — no `validateUrlForSSRF` call anywhere in the file | Deliberate, not an omission: the allowlist above is stronger than the classifier. There is nothing for `validateUrlForSSRF` to add to a value that already had to be one of nine hard-coded vendor endpoints. `lib/server/web-search-config.ts:74-77` |

## The classifier

`isPrivateIP` (`lib/server/ssrf-guard.ts:178-244`) is unusually thorough for a
...
`CLOUD_METADATA_ADDRESSES` and `CLOUD_METADATA_HOSTNAMES` (`ssrf-guard.ts:11-12`)
...
(`lib/server/ssrf-guard.ts:246-247`), which is honest but means the first thing a
...
- [`01-trust-boundaries.md`](./01-trust-boundaries.md) — crossings C8 and C9.
- [`06-configuration.md`](./06-configuration.md) — `ALLOW_LOCAL_NETWORKS`, `NODE_ENV`, `RENDER_SERVICE_URL`.
- [`../12-api-reference/index.md`](../12-api-reference/index.md) — the `INVALID_URL` / 403 responses these produce.
