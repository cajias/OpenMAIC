...
**Sources:** [`lib/config/feature-flags.ts:1-12`](lib/config/feature-flags.ts#L1-L12), [`lib/persistence/bootstrap.ts:31-39`](lib/persistence/bootstrap.ts#L31-L39),
[`lib/persistence/server-auth.ts:1-13`](lib/persistence/server-auth.ts#L1-L13), [`Dockerfile:51-72`](Dockerfile#L51-L72), [`docker-compose.yml:8-21`](docker-compose.yml#L8-L21),
[`lib/server/provider-config.ts:693-706`](lib/server/provider-config.ts#L693-L706), [`lib/server/proxy-fetch.ts:134`](lib/server/proxy-fetch.ts#L134),
`app/api/materials/route.ts`, `lib/server/capped-stream.ts`,
[`lib/constants/generation.ts:16`](lib/constants/generation.ts#L16), [`render-service/src/config.ts:100-110`](render-service/src/config.ts#L100-L110),
...
[`lib/config/feature-flags.ts:1-8`](lib/config/feature-flags.ts#L1-L8) states the rule: public flags use
...
> `x-learner-key`.
> — [`lib/persistence/server-auth.ts:3-8`](lib/persistence/server-auth.ts#L3-L8)
...
([`Dockerfile:51-72`](Dockerfile#L51-L72), mirrored at [`docker-compose.yml:11-21`](docker-compose.yml#L11-L21)).

`NEXT_PUBLIC_PRO_WORKBENCH_ENABLED` is documented in `.env.example` and read by
`isProWorkbenchEnabled()` ([`lib/config/feature-flags.ts:33`](lib/config/feature-flags.ts#L33)) but appears in
...
([`lib/server/provider-config.ts:693-706`](lib/server/provider-config.ts#L693-L706)). The client learns *that* a provider is
managed (by its presence in the map) and nothing more. `GET /api/health` reports
four booleans and a version string ([`app/api/health/route.ts:12-23`](app/api/health/route.ts#L12-L23)).
...
| Path | Mechanism |
| --- | --- |
| Forward-proxy URL | [`lib/server/proxy-fetch.ts:134`](lib/server/proxy-fetch.ts#L134) does `log.info('Using proxy', cachedProxyUrl, 'for:', url)`. `cachedProxyUrl` is the raw `https_proxy` value, so `http://user:pass@proxy:3128` is written verbatim. `LOG_LEVEL` defaults to `info` ([`lib/logger.ts:5`](lib/logger.ts#L5)), so this fires by default. |
| Error objects | `formatLine` stringifies an `Error` as `a.stack ?? a.message` ([`lib/logger.ts:18`](lib/logger.ts#L18)). A provider SDK that embeds a request URL — including a query-string key — in its message will have it logged. Not traced to a specific provider that does. |
...
[`03-threat-injection.md`](docs/15-cross-cutting/03-threat-injection.md) is a key-exfiltration
...
1. **The cap is enforced on streamed bytes, not on `Content-Length`.**
   `readMeteredBody` ([`app/api/materials/route.ts:400-416`](app/api/materials/route.ts#L400-L416)) adds up each chunk as
   ...
   ([`lib/server/capped-stream.ts:19-46`](lib/server/capped-stream.ts#L19-L46)) wraps the body in a `TransformStream` and
   `controller.error(...)`s over the cap, and its docstring states exactly that
   threat. It has one caller in the repo,
   [`app/api/export-video/render/route.ts:70`](app/api/export-video/render/route.ts#L70), which is where it matters — a
...
2. **A missing `Content-Length` reserves the per-file maximum** rather than zero
   ([`app/api/materials/route.ts:228-229`](app/api/materials/route.ts#L228-L229)), so an unmeasured stream cannot slip past
...
| --- | --- | --- | --- |
| Audio/video material | `OPENMAIC_AGENT_MAX_UPLOAD_BYTES` | 50 MiB | [`lib/server/agent-runtime/config.ts:46`](lib/server/agent-runtime/config.ts#L46) |
| Document/image material | `MATERIALS_MAX_DOCUMENT_BYTES` | 50 MiB, `min`-ed with the above | [`config.ts:48`](lib/server/agent-runtime/config.ts#L48), [`app/api/materials/route.ts:70-73`](app/api/materials/route.ts#L70-L73) |
| Materials per owner | `MATERIALS_MAX_COUNT_PER_OWNER` | 100 → 429 | [`config.ts:50`](lib/server/agent-runtime/config.ts#L50) |
| Bytes per owner | `MATERIALS_MAX_TOTAL_BYTES_PER_OWNER` | 2 GiB → 429 | [`config.ts:52-55`](lib/server/agent-runtime/config.ts#L52-L55) |
| Extraction upload | — (hardcoded) | 50 MiB | [`lib/constants/generation.ts:16`](lib/constants/generation.ts#L16), checked at [`extract-document/route.ts:481`](app/api/extract-document/route.ts#L481), [`:572`](app/api/extract-document/route.ts#L572), [`:602`](app/api/extract-document/route.ts#L602) |
| Proxy-media response | — (hardcoded) | 25 MiB, checked on `content-length` **and** on the buffered blob | [`app/api/proxy-media/route.ts:67-75`](app/api/proxy-media/route.ts#L67-L75) |
| Export archive (app side) | — (hardcoded) | 300 MiB, streamed cap | [`app/api/export-video/render/route.ts:18`](app/api/export-video/render/route.ts#L18), [`:70`](app/api/export-video/render/route.ts#L70) |
| Next proxy body | — | `proxyClientMaxBodySize: '200mb'` | [`next.config.ts:36`](next.config.ts#L36) |
| Preview JSON | `RENDER_PREVIEW_MAX_JSON_BYTES` | 32 MiB → 413 | [`render-service/src/config.ts:96`](render-service/src/config.ts#L96) |
...
decompression ([`render-service/src/config.ts:100-110`](render-service/src/config.ts#L100-L110), applied in
...
- Pathological raster dimensions are refused in the text-only path
  ([`lib/pdf/pdf-providers.ts:256`](lib/pdf/pdf-providers.ts#L256)).
- Documents never silently leave the operator's infrastructure: a self-hosted
  ...
  ([`app/api/extract-document/route.ts:144-147`](app/api/extract-document/route.ts#L144-L147), checked at [`:369`](app/api/extract-document/route.ts#L369), 422 at [`:375`](app/api/extract-document/route.ts#L375)).
...
not configurable ([`lib/server/classroom-storage.ts:6`](lib/server/classroom-storage.ts#L6)).
