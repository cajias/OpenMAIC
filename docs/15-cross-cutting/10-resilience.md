...
`lib/ai/llm.ts:264-276`, `lib/audio/tts-providers.ts:145-218`,
`lib/server/agent-runtime/fetch-url.ts:62-67`, `lib/agent/runtime/tool-timeout.ts`,
`lib/server/agent-runtime/{resume,tool-call-integrity,config}.ts`,
`lib/whiteboard/runtime/store.ts:165-265`, `app/api/materials/route.ts:231-330`,
`render-service/src/{config,render-coordinator}.ts`, `instrumentation.ts:57-95`,
...
exponential delay (`generation-retry.ts:166-175`). The sleep itself is
...
- Pause saves the remaining browser-TTS chunks and **cancels**, because Firefox's
  ...
  (`lib/playback/engine.ts:243-246`, `:773`).
...
| Mechanism | Where |
| --- | --- |
| Required by contract, at-least-once execution | every agent tool (`resume.ts:34`) |
...
| Transcript dedupe as "the idempotence judge" | stage-writer patches (`runner.ts:1690`) |
