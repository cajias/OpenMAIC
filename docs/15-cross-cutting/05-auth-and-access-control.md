...
**Sources:** [`middleware.ts:18-90`](middleware.ts#L18-L90), `lib/server/access-token.ts`,
...
- **The access code IS the HMAC key.** `createAccessToken(accessCode)` uses it
  directly as the HMAC secret ([`lib/server/access-token.ts:6`](lib/server/access-token.ts#L6)). Rotating
...
- **`AccessCodeGuard.onSuccess` re-fetches server providers** because
  ...
  ([`components/access-code-guard.tsx:45-54`](components/access-code-guard.tsx#L45-L54)). On a `fetch` failure the guard
...
literal path `/api/health` ([`middleware.ts:66`](middleware.ts#L66)). No route re-checks the gate; a
...
| Comparison of the submitted code is timing-safe | Yes | `timingSafeEqual` after a length check ([`verify/route.ts:23-28`](app/api/access-code/verify/route.ts#L23-L28)). |
...
| --- | --- |
| **Per-user identity.** One shared password; everyone who knows it is the same principal. | [`middleware.ts:60-74`](middleware.ts#L60-L74) |
| **Server-side expiry.** The token carries a timestamp and `verifyToken` never compares it to `Date.now()`. The 7-day `Max-Age` is a browser-side hint a client can ignore; a captured cookie is valid forever until the code rotates. | [`middleware.ts:18-44`](middleware.ts#L18-L44) vs [`verify/route.ts:36`](app/api/access-code/verify/route.ts#L36) |
| **Per-session revocation.** No token store, no jti, no denylist. | whole of `lib/server/access-token.ts` |
| **Truly constant-time cookie comparison in middleware.** The Edge verifier does a length check then an XOR-accumulate loop, with an inline comment admitting it is "not truly constant-time in JS". (The Node path in [`access-token.ts:24`](lib/server/access-token.ts#L24) does use `timingSafeEqual`.) | [`middleware.ts:37-43`](middleware.ts#L37-L43) |
...
| **Any test coverage.** `middleware.ts` has no test file. | [`../appendix/research/quality-testing-ci-deps/05-failure-modes.md`](docs/appendix/research/quality-testing-ci-deps/05-failure-modes.md) |
| **Multi-tenant isolation.** See the identity section below. | [`lib/persistence/server-auth.ts:1-13`](lib/persistence/server-auth.ts#L1-L13) |
...
- **M1** is an unauthenticated bearer of its own id: possession of the cookie
  ...
  ([`lib/server/agent-runtime/owner.ts:33-36`](lib/server/agent-runtime/owner.ts#L33-L36)). It is not an authentication claim.
- **M1 has a dead parameter.** `resolveRequestOwnerId` accepts
  ...
  ([`owner.ts:38-51`](lib/server/agent-runtime/owner.ts#L38-L51)). **No call site anywhere supplies it** — verified by grep
...
- **The consequence:** `POST /api/stages/[id]/publish` and `.../unpublish` refuse
  ...
  ([`app/api/stages/[id]/publish/route.ts:27-32`](app/api/stages/[id]/publish/route.ts#L27-L32)). Since every owner is `anon:`,
...
- **M2 is documented as providing no isolation at all.** Its `x-learner-key` is
  ...
  ([`lib/persistence/server-auth.ts:38-53`](lib/persistence/server-auth.ts#L38-L53)). Document requests are the one
  exception — they use the server-resolved M1 owner instead
  ([`app/api/persistence/[...path]/route.ts:109-112`](app/api/persistence/[...path]/route.ts#L109-L112)).
...
and stated at [`lib/server/agent-runtime/route-response.ts:36-40`](lib/server/agent-runtime/route-response.ts#L36-L40). Owner-bound
...
3. **Do not enable server persistence on a public deployment.**
   ...
   development-only, and the token is in the client bundle. [`lib/persistence/server-auth.ts:10-12`](lib/persistence/server-auth.ts#L10-L12)
...
4. **Terminate TLS.** The access cookie only gets `Secure` when `NODE_ENV` is
   `production` ([`verify/route.ts:37`](app/api/access-code/verify/route.ts#L37)), and the token is a bearer credential.
5. **Do not set `ALLOW_LOCAL_NETWORKS`.** It disables the SSRF guard for 13 route
   files at once — see [`02-threat-ssrf.md`](docs/15-cross-cutting/02-threat-ssrf.md).
6. **Leave `TRUST_PROXY_HEADERS` unset unless a proxy really overwrites
   ...
   ([`app/api/export-video/render/route.ts:23-38`](app/api/export-video/render/route.ts#L23-L38)).
...
8. **Set `ALLOWED_FRAME_ANCESTORS` only if you actually embed OpenMAIC.**
   ...
   form ([`next.config.ts:46-48`](next.config.ts#L46-L48)).
