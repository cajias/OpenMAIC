...
files), `app/page.tsx:671`, `app/generation-preview/page.tsx:1040-1051`,
`lib/playback/engine.ts:62`, `lib/chat/agent-loop.ts:154`,
`lib/chat/pi/director-loop.ts:29`, `lib/server/agent-runtime/runner.ts:889,1861`,
`lib/pbl/v2/api/sse.ts`, `app/api/pbl/v2/task/update/route.ts`,
`lib/import/use-import-pptx.ts`, `lib/export/use-export-pptx.ts:497`,
`lib/video-export/compile.ts:152`, `lib/store/video-render.ts:121`,
`app/api/generate-classroom/route.ts`, `skills/openmaic/references/generate-flow.md`.
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../06-generation-pipeline/index.md`, `../03-app-and-api/index.md` |
...
(`store.setGeneratingOutlines(remaining)`, `app/generation-preview/page.tsx:1037-1038`)
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../06-generation-pipeline/index.md` |
...
| **Exercised** | `runClassroomLoad()` (19 injected dependencies, IndexedDB then server fallback) → `Stage` chrome dispatch (`components/stage.tsx`) → `PlaybackChromeRoot` (1 848 lines, the real orchestrator) → `PlaybackEngine` (`lib/playback/engine.ts:62`) → `ActionEngine` (`lib/action/engine.ts:178`) → `SlideCanvas` / whiteboard overlays / `InteractiveIframeHost` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../08-classroom-runtime/index.md`, `../07-dsl-renderer-editor/index.md` |
...
| **Exercised** | `runAgentLoop()` (`lib/chat/agent-loop.ts:154`, browser-owned) → `POST /api/chat` (LangGraph `createOrchestrationGraph`, default) **or** `POST /api/chat/pi` (`runPiDirectorLoop`, flagged) → `StatelessEvent` SSE frames → `StreamBuffer` 30 ms/char pacing → `useDiscussionTTS` hold protocol → `ActionEngine` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../05-agent-runtime/index.md`, `../08-classroom-runtime/index.md` |

Both chat paths ship today. `NEXT_PUBLIC_PI_CHAT_ENABLED` selects, and it is
commented out in `.env.example:324`, so the LangGraph path is what a stock
deployment runs.
...
| **Fails how** | **Silently awards 50 % partial credit when the grading response fails to parse** (`app/api/quiz-grade/route.ts:95-98`, `score: Math.round(points * 0.5)`), with no test coverage |
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../06-generation-pipeline/index.md`, `../08-classroom-runtime/index.md` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../08-classroom-runtime/index.md` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../05-agent-runtime/index.md` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../07-dsl-renderer-editor/index.md`, `../05-agent-runtime/index.md` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../07-dsl-renderer-editor/index.md` |
...
| **Exercised** | `.pptx`: `buildPptxBlob()` (`lib/export/use-export-pptx.ts:497`) through the **vendored** pptxgenjs 4.0.1, with LaTeX going temml → MathML → `mathml2omml` → OMML. `.maic.zip`: `buildClassroomExportZip()` (`lib/export/use-export-classroom.ts:80`), format version 1. Video: `compileVideoTimeline()` (9 pure passes → zod-authored `VideoTimeline` IR, schema v4, 13 diagnostic codes) → `emitHyperframes()` → `buildExportZip()` → either a ZIP download or `startRender()` (`lib/store/video-render.ts:121`) relaying to `render-service` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../09-media-and-export/index.md`, `../07-dsl-renderer-editor/index.md` |
...
| **Trace** | `../11-data-flows/index.md` |
| **Components** | `../06-generation-pipeline/index.md`, `../12-api-reference/index.md` |
...
| `stages/[id]/publish`, `stages/[id]/unpublish` | **none** — the handlers run, but their *success* path is unreachable: `withRequestOwnerId` only ever yields an `anon:`-prefixed id, and `publish/route.ts:26-31` refuses those with `401 login_required` before any access lookup. The owner-match and already-public branches below it (`:37-39`, `:41-45`) are therefore dead in this build |
